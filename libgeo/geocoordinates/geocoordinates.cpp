...
void geocoordinates::set_lat_lon_strs(const QString &lag, const QString &lam, const QString &las, const QString &LatH,
  ...
  if (!ok) {
    qDebug() << this->myfunc << "can't convert" << log << " to number";
    errors++;
  }
  ok = true;
  int tlom = lom.toInt(&ok, 10);
  if (!ok) {
    qDebug() << this->myfunc << "can't convert" << lom << " to number";
    errors++;
  }
  ok = true;
  double tlos = los.toDouble(&ok);
  if (!ok) {
    qDebug() << this->myfunc << "can't convert" << los << " to number";
    errors++;
  }
  ok = true;
  int tlag = lag.toInt(&ok, 10);
  if (!ok) {
    qDebug() << this->myfunc << "can't convert" << lag << " to number";
    errors++;
  }
  ok = true;
  int tlam = lam.toInt(&ok, 10);
  if (!ok) {
    qDebug() << this->myfunc << "can't convert" << lam << " to number";
    errors++;
  }
  ok = true;
  double tlas = las.toDouble(&ok);
  if (!ok) {
    qDebug() << this->myfunc << "can't convert" << las << " to number";
    errors++;
  }
  ok = true;
  ...
  if (elevation.size()) {
    ...
    if (!ok) {
      qDebug() << this->myfunc << "can't convert" << telev << " to number";
      errors++;
    }
    ok = true;
  }
...
bool geocoordinates::fetch_arguments(QStringList &qargs) {
  ...
  if (((cmdpos = qargs.indexOf("-deg")) > 0) && (qargs.size() > cmdpos + 8)) {
    ...
    if (!this->is_init)
      qDebug() << this->myfunc << "can't convert" << qargs[cmdpos + 1] << " to number";

    tlom = qargs[cmdpos + 2].toLong(&this->is_init, 10);
    if (!this->is_init)
      qDebug() << this->myfunc << "can't convert" << qargs[cmdpos + 2] << " to number";

    tlos = qargs[cmdpos + 3].toDouble(&this->is_init);
    if (!this->is_init)
      qDebug() << this->myfunc << "can't convert" << qargs[cmdpos + 3] << " to number";

    this->LonH = qargs[cmdpos + 4];
    tlag = qargs[cmdpos + 5].toLong(&this->is_init, 10);
    if (!this->is_init)
      qDebug() << this->myfunc << "can't convert" << qargs[cmdpos + 4] << " to number";

    tlam = qargs[cmdpos + 6].toLong(&this->is_init, 10);
    if (!this->is_init)
      qDebug() << this->myfunc << "can't convert" << qargs[cmdpos + 5] << " to number";

    tlas = qargs[cmdpos + 7].toDouble(&this->is_init);
    if (!this->is_init)
      qDebug() << this->myfunc << "can't convert" << qargs[cmdpos + 6] << " to number";
  ...
  if ((cmdpos = qargs.indexOf("-elevation")) > 0) {
    ...
    if (!this->is_init)
      qDebug() << myfunc << "can't convert" << qargs[cmdpos + 1] << " to number";
  }

  if (this->is_init)
    set_lat_lon_rad(rad_lat, rad_lon, (long double)televation, tutm_meridian, tgk_meridian);
...

{
  ...
  M = this->eqradius * ((1.L - this->eccSquared / 4.L - 3.L * this->eccSquared * this->eccSquared / 64. - 5.L * this->eccSquared * this->eccSquared * this->eccSquared / 256.L) * this->latitude - (3.L * this->eccSquared / 8.L + 3.L * this->eccSquared * this->eccSquared / 32.L + 45.L * this->eccSquared * this->eccSquared * this->eccSquared / 1024.L) * sinl(2.L * this->latitude) + (15.L * this->eccSquared * this->eccSquared / 256.L + 45.L * this->eccSquared * this->eccSquared * this->eccSquared / 1024.L) * sinl(4.L * this->latitude) - (35.L * this->eccSquared * this->eccSquared * this->eccSquared / 3072.L) * sinl(6.L * this->latitude));

  this->easting = (long double)(k0 * N * (A + (1.L - T + C) * A * A * A / 6.L + (5.L - 18.L * T + T * T + 72.L * C - 58.L * eccPrimeSquared) * A * A * A * A * A / 120.L) + 500000.0L);

  this->northing = (long double)(k0 * (M + N * tanl(this->latitude) * (A * A / 2.L + (5.L - T + 9.L * C + 4.L * C * C) * A * A * A * A / 24.L + (61.L - 58.L * T + T * T + 600.L * C - 330.L * eccPrimeSquared) * A * A * A * A * A * A / 720.L)));
...
*/
  ...
  this->latitude = bf - yN * tanl(bf) * (1.L + eta2 - yN / 6.L * (5.L + 3.L * t2 + 6.L * eta2 * (1.L - t2) - yN * (4.L + 3.L * t2 * (2.L + t2))));
  this->longitude = lh + y / N / cosl(bf) * (1.L - yN / 3.L * (1.L + 2.L * t2 + eta2 - yN / 10.L * (5.L + t2 * (28.L + 24.L * t2))));
...
void geocoordinates::UTM2geo() {
  ...
  phi1Rad = mu + (3.L * e1 / 2.L - 27.L * e1 * e1 * e1 / 32.) * sinl(2.L * mu) + (21.L * e1 * e1 / 16.L - 55.L * e1 * e1 * e1 * e1 / 32.L) * sinl(4.L * mu) + (151.L * e1 * e1 * e1 / 96.L) * sinl(6.L * mu);
  ...
  this->latitude = phi1Rad - (N1 * tanl(phi1Rad) / R1) * (D * D / 2.L - (5.L + 3.L * T1 + 10.L * C1 - 4.L * C1 * C1 - 9.L * eccPrimeSquared) * D * D * D * D / 24. + (61.L + 90.L * T1 + 298.L * C1 + 45.L * T1 * T1 - 252.L * eccPrimeSquared - 3.L * C1 * C1) * D * D * D * D * D * D / 720.L);

  this->longitude = (D - (1.L + 2.L * T1 + C1) * D * D * D / 6.L + (5.L - 2.L * C1 + 28.L * T1 - 3.L * C1 * C1 + 8.L * eccPrimeSquared + 24.L * T1 * T1) * D * D * D * D * D / 120.L) / cosl(phi1Rad);
