...
void xmlcallib::set_file(const QFileInfoList &qfx) {
  ...
  for (auto &test : qfx) {
    if (!test.fileName().endsWith("xml", Qt::CaseInsensitive))
      all_is_xml = false;
  }
  ...
  if (all_is_xml) {
    ...
    for (auto &chan : channels) {
      ...
      if (chan->sensortype.contains("ADB"))
        chan->gen_adb_pre_cal(chan->f_off);
      else
        chan->gen_theo_cal(chan->f_off);
      chan->chopper = cal::chopper_on;
      if (chan->sensortype.contains("ADB"))
        chan->gen_adb_pre_cal(chan->f_on);
      else
        chan->gen_theo_cal(chan->f_on);
    }
  ...
  for (auto &chan : this->channels) {
    if (chan->f_on.size())
      has_chopper_on = true;
    if (chan->f_off.size())
      has_chopper_off = true;
  }

  if (!has_chopper_on)
    this->ui->pushButton_Chopper_Off_2->click();
  else if (!has_chopper_off)
    this->ui->pushButton_Chopper_On_2->click();
}
...
void xmlcallib::dropEvent(QDropEvent *event) {
  ...
  if (urls.isEmpty())
    return;
  QFileInfoList qfx;
  for (auto &urlls : this->urls)
    qfx.append(QFileInfo(urlls.toLocalFile()));
  this->qdw->show();
  this->qlw->clear();
  this->set_file(qfx);
}
  ...
  for (i = 0; i < this->channels.size(); ++i) {
    ...
    if (this->ui->pushButton_Chopper_Both->isChecked() || this->ui->pushButton_Chopper_On->isChecked()) {
      ...
      if (this->ui->checkBox_un->isChecked()) {
        ...
        size_t ff = 0;
        for (auto &u : unn)
          u *= this->channels.at(i)->f_on[ff++];
        plt->graph(j)->setData(this->channels.at(i)->f_on, unn);
      } else
        plt->graph(j)->setData(this->channels.at(i)->f_on, this->channels.at(i)->ampl_on);
      plt->graph(j)->setPen(QPen(Qt::blue));
      if (j == 0)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssSquare, 12));
      if (j == 1)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCross, 12));
      if (j == 2)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssPlus, 12));
      if (j == 3)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssDiamond, 12));
      if (j == 4)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssStar, 12));
      if (j == 5)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCrossCircle, 12));
      if (j == 6)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssDisc, 12));
      if (j == 7)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssTriangle, 12));
      if (j == 8)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssTriangleInverted, 12));
      if (j == 9)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssPlusSquare, 12));
      if (j == 10)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCrossCircle, 12));
      if (j == 11)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssPlusCircle, 12));
      if (j == 12)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssPeace, 12));

      if (this->channels.at(i)->sensortype.contains("ADB")) {
        plt->graph(j)->setName(this->channels.at(i)->get_adb_name() + " (on)");
      } else
        plt->graph(j)->setName(this->channels.at(i)->sensortype + " " + QString::number(channels.at(i)->sernum) + " (on)");
    ...
    if (this->ui->pushButton_Chopper_Both->isChecked() || this->ui->pushButton_Chopper_Off->isChecked()) {
      ...
      if (this->ui->checkBox_un->isChecked()) {
        ...
        size_t ff = 0;
        for (auto &u : unn)
          u *= this->channels.at(i)->f_off[ff++];
        plt->graph(j)->setData(this->channels.at(i)->f_off, unn);
      } else
        plt->graph(j)->setData(this->channels.at(i)->f_off, this->channels.at(i)->ampl_off);
      if (j == 0)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssSquare, 12));
      if (j == 1)
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCross, 12));
      ...
      if (this->channels.at(i)->sensortype.contains("ADB")) {
        ...
        plt->graph(j)->setName(this->channels.at(i)->sensortype + " " + QString::number(channels.at(i)->sernum) + " (off)");
    ...
    if ((this->add_master_plot == 2) || (this->add_master_plot == 3)) {
      ...
      if (this->ui->checkBox_un->isChecked()) {
        ...
        size_t ff = 0;
        for (auto &u : unn)
          u *= this->channels.at(i)->f_off_master[ff++];
        plt->graph(j)->setData(this->channels.at(i)->f_off_master, unn);
      } else
        plt->graph(j)->setData(this->channels.at(i)->f_off_master, this->channels.at(i)->ampl_off_master);
    ...
    if ((this->add_master_plot == 1) || (this->add_master_plot == 3)) {
      ...
      if (this->ui->checkBox_un->isChecked()) {
        ...
        size_t ff = 0;
        for (auto &u : unn)
          u *= this->channels.at(i)->f_on_master[ff++];
        plt->graph(j)->setData(this->channels.at(i)->f_on_master, unn);
      } else
        plt->graph(j)->setData(this->channels.at(i)->f_on_master, this->channels.at(i)->ampl_on_master);
      plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssSquare, 10));
    ...
    if (this->channels.at(i)->f_off_theo.size()) {
      if ((this->add_theo_plot == 2) || (this->add_theo_plot == 3)) {
        ...
        if (this->ui->checkBox_un->isChecked()) {
          ...
          size_t ff = 0;
          for (auto &u : unn)
            u *= this->channels.at(i)->f_off_theo[ff++];
          plt->graph(j)->setData(this->channels.at(i)->f_off_theo, unn);
        } else
          plt->graph(j)->setData(this->channels.at(i)->f_off_theo, this->channels.at(i)->ampl_off_theo);
        ...
        if (this->channels.at(i)->sensortype.contains("ADB")) {
          ...
          plt->graph(j)->setName(this->channels.at(i)->sensortype + " theo off");
    ...
    if (this->channels.at(i)->f_on_theo.size()) {
      if ((this->add_theo_plot == 1) || (this->add_theo_plot == 3)) {
        ...
        if (this->ui->checkBox_un->isChecked()) {
          ...
          size_t ff = 0;
          for (auto &u : unn)
            u *= this->channels.at(i)->f_on_theo[ff++];
          plt->graph(j)->setData(this->channels.at(i)->f_on_theo, unn);
        } else
          plt->graph(j)->setData(this->channels.at(i)->f_on_theo, this->channels.at(i)->ampl_on_theo);
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssTriangle, 10));
    ...
    if (this->channels.at(i)->f_off_ipl.size()) {
      if ((this->add_ipl_plot == 2) || (this->add_ipl_plot == 3)) {
        ...
        if (this->ui->checkBox_un->isChecked()) {
          ...
          size_t ff = 0;
          for (auto &u : unn)
            u *= this->channels.at(i)->f_off_ipl[ff++];
          plt->graph(j)->setData(this->channels.at(i)->f_off_ipl, unn);
        } else
          plt->graph(j)->setData(this->channels.at(i)->f_off_ipl, this->channels.at(i)->ampl_off_ipl);
    ...
    if (this->channels.at(i)->f_on_ipl.size()) {
      if ((this->add_ipl_plot == 1) || (this->add_ipl_plot == 3)) {
        ...
        if (this->ui->checkBox_un->isChecked()) {
          ...
          size_t ff = 0;
          for (auto &u : unn)
            u *= this->channels.at(i)->f_on_ipl[ff++];
          plt->graph(j)->setData(this->channels.at(i)->f_on_ipl, unn);
        } else
          plt->graph(j)->setData(this->channels.at(i)->f_on_ipl, this->channels.at(i)->ampl_on_ipl);
        plt->graph(j)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssTriangleInverted, 10));
...
void xmlcallib::on_pushButton_Chopper_On_clicked() {
  ...
  if (this->add_master_plot)
    this->add_master_plot = 1;
  if (this->add_ipl_plot)
    this->add_ipl_plot = 1;
  if (this->add_theo_plot)
    this->add_theo_plot = 1;
...
void xmlcallib::on_actionExtendFull_triggered() {
  ...
  for (auto &chan : channels) {
    ...
    chan->chopper = cal::chopper_off;
    if (chan->sensortype.contains("ADB"))
      chan->gen_adb_pre_cal(fsoff);
    else
      chan->gen_theo_cal(fsoff);
    chan->chopper = cal::chopper_on;
    if (chan->sensortype.contains("ADB"))
      chan->gen_adb_pre_cal(fson);
    else
      chan->gen_theo_cal(fson);
    chan->chopper = chop;
    fson.clear();
    fsoff.clear();
  }
...
void xmlcallib::slot_x_y_pos(const double &x, const double &y) {
  if (x == DBL_MAX)
    ...
    this->ui->x_y_label->setText("x:" + QString::number(x) + "  y:" + QString::number(y));
}
