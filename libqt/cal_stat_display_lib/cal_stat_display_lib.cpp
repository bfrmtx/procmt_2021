...
cal_stat_display_lib::cal_stat_display_lib(const QFileInfo &info_db, const QFileInfo &master_cal_db, QWidget *parent) :
  ...
  // prepare the types
  this->plots[0]->chopper = cal::chopper_on;
  this->plots[0]->plot_type = cal::amplitude;
  this->plots[1]->chopper = cal::chopper_on;
  this->plots[1]->plot_type = cal::phase_grad;
  this->plots[2]->chopper = cal::chopper_off;
  this->plots[2]->plot_type = cal::amplitude;
  this->plots[3]->chopper = cal::chopper_off;
  this->plots[3]->plot_type = cal::phase_grad;
...
int cal_stat_display_lib::set_calstat(const QList<calstat> &in) {
  ...
  for (auto &ins : in)
    this->calstats.push_back(calstat(ins));
  ...
  for (auto &cals : calstats) {
    ...
    if (cals.chopper == cal::chopper_on)
      vfon.push_back(cals.f);
    if (cals.chopper == cal::chopper_off)
      vfoff.push_back(cals.f);
  }
...
void cal_stat_display_lib::slot_plot(const double &f) {
  ...
  for (this->citb = this->calstats.cbegin(); this->citb != this->calstats.cend(); ++this->citb) {
    if ((this->citb->f == f) && (this->citb->chopper == this->chopper) && (citb->variance_ampls_data.size() == g_stat::xstat_size)) {
      ...
      if (chopper == cal::chopper_on)
        this->anyplot->graph(this->anyplot->graphCount() - 1)->setName("amplitude chopper on");
      else
        this->anyplot->graph(this->anyplot->graphCount() - 1)->setName("amplitude chopper off");
  ...
  for (this->citb = this->calstats.cbegin(); this->citb != this->calstats.cend(); ++this->citb) {
    if ((this->citb->f == f) && (this->citb->chopper == this->chopper) && (citb->variance_phzs_data.size() == g_stat::xstat_size)) {
      ...
      // this->anyplot->graph(this->anyplot->graphCount()-1)->removeFromLegend();
      if (chopper == 1)
        this->anyplot->graph(this->anyplot->graphCount() - 1)->setName("phase chopper on");
      else
        this->anyplot->graph(this->anyplot->graphCount() - 1)->setName("phase chopper off");
...
void cal_stat_display_lib::on_chopper_chk_clicked(bool checked) {
  ...
  for (auto &plot : plots) {
    ...
    if (plot->plot_type == cal::amplitude) {
      ...
      if (this->chopper == cal::chopper_on) {
        if (plot->chopper == cal::chopper_off)
          plot->foff_ranges_area->hide();
        if (plot->chopper == cal::chopper_on)
          plot->fon_ranges_area->show();
        this->slot_plot(this->sumcal->f_on_ipl.at(0));
      }
      if (this->chopper == cal::chopper_off) {
        if (plot->chopper == cal::chopper_on)
          plot->fon_ranges_area->hide();
        if (plot->chopper == cal::chopper_off)
          plot->foff_ranges_area->show();
  ...
  if (!this->count_me)
    ...
    this->resize(this->wsize.width(), this->wsize.height());
  this->count_me++;
}
