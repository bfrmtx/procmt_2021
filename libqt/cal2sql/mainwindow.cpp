...
void MainWindow::scan_db() {
  ...
  if (this->mode != -2)
    qfi.setFile(this->dialog->selectedFiles().at(0));
  if (this->mode == -2)
    this->mode = 0; // swtich from drag & drop to open mode

  if (mode == 1) {
    if (qfi.completeSuffix().size() == 0)
      qfi.setFile(this->dialog->selectedFiles().at(0) + ".sql3");
  }
...
void MainWindow::dlg_coil_selected(const QString &coil, const int &serno) {
  ...
  if (!this->cal->select_coil(coil, serno, true))
    return;
