...
frequencies_combo::frequencies_combo(const QString &dbname, QComboBox *box) {
  ...
  if (this->db->open()) {
    ...
    if ((this->names.size() == this->values.size()) && this->names.size())
      this->box->setCurrentText(this->names.at(0));
    else
      qDebug() << "db err: query failed";
    this->db->close();
    *this->db = QSqlDatabase();
    ...
    qDebug() << "can not open" << this->db->databaseName();
  this->db.reset();
}
...
single_column_combo::single_column_combo(const QString &dbname, const QString &table_name, const QString &double_int_string,
...
void single_column_combo::init_data_base(const QString &dbname, const QString &table_name, const QString &double_int_string) {
  ...
  if (this->db->open()) {
    ...
    if (query.exec("select *from " + table_name + ";")) {
      while (query.next()) {
        this->names.append(query.value(0).toString());
        if (double_int_string == "double")
          this->values.append(query.value(0).toDouble());
        else if (double_int_string == "int")
          this->values.append(query.value(0).toInt());
        else if (double_int_string == "string")
          this->values.append(query.value(0).toString());
        else
          this->values.append(query.value(0));
      }
...
void single_column_combo::add_to_gui(QBoxLayout *layout, const QString horizontal_vertical, QMap<QString, int> &map_scc) {
  if (this->parent) {
    if (horizontal_vertical == "horizontal") {
      ...
      if (this->box)
        this->hzl->addWidget(this->box);
      if (this->label)
        this->hzl->addWidget(this->label);
    }
    if (horizontal_vertical == "verical") {
      this->vtl = new QVBoxLayout();
      layout->addLayout(this->vtl);
      if (this->box)
        this->vtl->addWidget(this->box);
      if (this->label)
        this->vtl->addWidget(this->label);
    }
    if (map_scc.isEmpty()) {
...
checkbox_list::checkbox_list(QObject *parent) :
    QObject(parent) {
}
...
radiobutton_list::radiobutton_list(QObject *parent) :
    QObject(parent) {
}
...
void radiobutton_list::slot_re_check() {
  ...
  if (index < vradio_names.size())
    emit this->btn_name_checked(vradio_names.at(index));
}

hz_s_spinbox::hz_s_spinbox(bool is_lowpass, QWidget *parent) :
    QWidget(parent) {
...
hz_s_spinbox::~hz_s_spinbox() {
  if (this->fsp != Q_NULLPTR)
    delete this->fsp;
  if (this->Hz_s_btn != Q_NULLPTR)
    delete this->Hz_s_btn;
}
...
void hz_s_spinbox::toggle_btn(const bool is_hz) {
  ...
  if (this->is_hz) {
    ...
    if (this->is_lowpass)
      this->Hz_s_btn->setText("[Hz] LP");
    else
      this->Hz_s_btn->setText("[Hz] HP");
  } else {
    this->Hz_s_btn->setDown(false);
    if (this->is_lowpass)
      this->Hz_s_btn->setText("[s] LP");
    else
      this->Hz_s_btn->setText("[s] HP");
  }
}
  ...
  if (this->db->open()) {
    ...
    if (query.exec("select *from " + table_name + ";")) {
      while (query.next()) {
        this->names.append(query.value(0).toString());
        if (double_int_string == "double")
          this->values.append(query.value(1).toDouble());
        else if (double_int_string == "int")
          this->values.append(query.value(1).toInt());
        else if (double_int_string == "string")
          this->values.append(query.value(1).toString());
        else
          this->values.append(query.value(0));
      }
...
check_boxes_scoll_area::check_boxes_scoll_area(QWidget *parent) :
    QScrollArea(parent) {
...
void check_boxes_scoll_area::check_all(const bool checked) {
  ...
  for (auto &btn : this->check_boxes) {
    if (btn->isChecked())
      fs.push_back(btn->property(prop_frequency).toDouble());
  }
...
push_buttons_scoll_area::push_buttons_scoll_area(QWidget *parent) :
    QScrollArea(parent) {
  vl = new QVBoxLayout(this);
  this->setLayout(vl);
}
...
pmt_open_help::pmt_open_help(QObject *parent) :
    QObject(parent) {
}
