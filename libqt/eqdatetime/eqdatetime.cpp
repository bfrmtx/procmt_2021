...
bool eQDateTime::cuts(quint64 const &starts, const quint64 &use, const bool &force_min_change)
{
    ...
    if (!this->num_samples) return false;
    if (( starts == this->start_sample) && ( use == this->use_samples)) return true;
    ...
    if (this->fullsec_mode && this->samples_per_sec) {
        ...
        if (force_min_change && !diff_steps && diff_start) diff_steps = 1;
        if (diff_steps) {
            if (inc) {
                this->nsteps_start += diff_steps;
            }
            else if (dec){
                if (this->nsteps_start > diff_steps) this->nsteps_start -= diff_steps;
                else this->nsteps_start = 0;
            }
            ...
            else this->nsteps_start = old_steps_start;
        }
    ...
    if (this->start_sample == starts && (this->use_samples == use)) return true;
    else return false;
...
eQDateTime::eQDateTime(): QDateTime()
{
    this->setTimeSpec(Qt::UTC);
...
eQDateTime::eQDateTime(const eQDateTime &edt): QDateTime()
{
    this->setTimeSpec(Qt::UTC);
...
eQDateTime::eQDateTime(const QDateTime &date): QDateTime(date)
{
    this->setTimeSpec(Qt::UTC);
}

eQDateTime::eQDateTime(const QDate &date): QDateTime(date, QTime(0,0,0))
{
    this->setTimeSpec(Qt::UTC);
}

eQDateTime::eQDateTime(const QDate &date, const QTime &time, const Qt::TimeSpec spec) :
...
qint64 eQDateTime::check_get_change_stop_time(const qint64 &new_stop_time_msecs_since_1970)
{
    ...
    if (!this->fullsec_mode) qFatal("check_get_change_stop_time is not in full seconds mode; unpredictable results");
    ...
    if ( (newtime > use) && (newtime < latest) ) {
        if (newtime < (use + this->secs_per_sample)) {
            if((use + this->secs_per_sample) < latest)    return use + this->secs_per_sample;
            else return latest;
        }
        else {
            qint64 diff = (newtime - use) / this->secs_per_sample;
            if ((use + diff * this->secs_per_sample) < latest) return use + diff * this->secs_per_sample;
            else return latest;
        }
    }
    ...
    if ( (newtime < use) && (newtime > earliest) ) {
        if (newtime > (use - this->secs_per_sample)) {
            if((use - this->secs_per_sample) > earliest)    return use - this->secs_per_sample;
            else return earliest;
        }
        else {
            qint64 diff = (use - newtime) / this->secs_per_sample;
            if ((use - diff * this->secs_per_sample) > earliest) return use - diff * this->secs_per_sample;
            else return earliest;
        }
    }
    return 0;
...
qint64 eQDateTime::check_get_change_start_time(const qint64 &new_start_time_msecs_since_1970)
{
    ...
    if ( (newtime > starts) && (newtime < latest) ) {
        if (newtime < (starts + this->secs_per_sample)) {
            if ((starts + this->secs_per_sample) < latest) return starts + this->secs_per_sample;
            else return latest;

        }
        else {
            qint64 diff = (newtime - starts) / this->secs_per_sample;
            if ((starts + diff * this->secs_per_sample) < latest) return starts + diff * this->secs_per_sample;
            else return latest;
        }
    }
    ...
    if ( (newtime < starts) && (newtime > earliest) ) {
        if (newtime > (starts - this->secs_per_sample)) {
            if((starts - this->secs_per_sample) > earliest) return starts - this->secs_per_sample;
            else return earliest;
        }
        else {
            qint64 diff = (starts - newtime) / this->secs_per_sample;
            if ((starts - diff * this->secs_per_sample) > earliest) return starts - diff * this->secs_per_sample;
            else return earliest;
        }
    }
    return 0;
...
{
    if (!use_samples) return this->cuts(start_sample, this->use_samples);
    else if (use_samples == ULLONG_MAX)  return this->cuts(start_sample, this->num_samples);

    else return this->cuts(start_sample, use_samples);
...
bool eQDateTime::set_use_samples(const quint64 &use_samples)
{
    ...
    if (use_samples == ULLONG_MAX) return this->cuts(this->start_sample, this->num_samples);
...
{
    if (this->sample_freq == 0.0) return false;
...
eQDateTime eQDateTime::sample_time(const quint64 &nsamples) const
{
    ...
    if (this->sample_freq == 0.0) return *this;

    eQDateTime qdtnew;
    quint64 sam = this->num_samples;  // return stop time in case
    if (nsamples <= this->num_samples ) sam = nsamples;
...
{
    if (this->sample_freq == 0.0) return 0;
    double len = 0.0;
    len = double(num_samples);

    if (use_half_for_filter) len *= 0.5;
...
{
    if (!nsamples) return this->toMSecsSinceEpoch() / 1000;
    if (this->sample_freq == 0.0) return this->toMSecsSinceEpoch() / 1000;
...
quint64 eQDateTime::samples_to(const eQDateTime &stops, eQDateTime *corrected_time) const
{
    ...
    if (startsat > stopsat) return 0;
    if (this->num_samples) {
        if (stops > this->sample_time(this->num_samples)) return 0;
    }
    if (this->sample_freq > 0.99) return quint64( ( ((stopsat - startsat) / 1000.) * this->sample_freq));
    ...
    if (corrected_time != nullptr) *corrected_time = this->sample_time(quint64(fullsteps));
...
void eQDateTime::to_full_minute(const bool round_up)
{
    ...
    msecs_since -=  newtime;
    this->frac_msecs = 0.0;
    if (round_up) msecs_since += qint64(60000);
