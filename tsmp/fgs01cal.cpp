...
/*!
    \fn FGS01Cal::read(string filename, string chopper)
     reads from a file; chopper: on, off, auto, none
     simple check for double entry,
     makes sure that f[0] < f[1]
 */
size_t FGS01Cal::read(const valarray<double>& newfreq, string chopper = "on", const double& samplefreq = 0, const double& gain = 0,
    ...
    if (!ifs) {
        ...
        if (!ifs) {
            cerr << "FGS01Cal::read -> can nor open file " << filename << " neither " << share_dir << " exit" << endl;
            exit(0);
        } else
            cerr << "FGS01Cal::read -> local file not found, taking " << share_dir << " instead" << endl;
    }
    ...
    if (ifs.eof()) {
        cerr << "FGS01Cal::read -> can not find key words chopper on or chopper off; I try a file with numbers only now.... check!!!" << endl;
    ...
    cerr << "FGS01Cal::read -> using " << chopper << endl;
    while(ifs) {
    ...
    if (i != freqs.size()) {
        cerr << "FGS01Cal::read -> at least on frequency with double entry!!! " << endl;
        exit(0);
    }
...
/*!
    \fn FGS01Cal::dump(string filename)
     debug function; just dump values to file
 */

size_t FGS01Cal::dump(const string& filename) {

    ofstream ofs;
    size_t i;
    if (!filename.size()) {
        cerr << "FGS01Cal::dump -> no filename given" << endl;
        return 0;
    }
    ofs.open(filename.c_str());
    if (!ofs) {
        cerr << "FGS01Cal::dump -> not open output stream" << endl;
        return 0;

    }

    if (!freqs.size() || !trf.size()) {
        cerr << "FGS01Cal::dump -> no frequencies / complex data to dump" << endl;
        return 0;
...
for ( i = 0; i < f.size(); i++) f[i] = i * samplefreq / wl;
 ...
 multiply by f again
*/
size_t FGS01Cal::interpolate(const valarray<double>& newfreq) {
    ...
    if (trf.size()) {
        ...
        cerr << "FGS01Cal::interpolate -> interpolating sensor trf (" << newfreq[1] << " - "
        ...
        cerr << "FGS01Cal::interpolate -> interpolating ERROR" << endl;
...
*/
size_t FGS01Cal::interpolate_extend_theoretical(const valarray<double>& newfreq, string chopper = "on", const double& samplefreq = 0,
    ...
    if (freqs[freqs.size()-1] < 513) {
        cerr << "FGS01Cal::interpolate_extend_theoretical -> extrapolating trf function for FGS01" << endl;
...
*/
size_t FGS01Cal::hardwired(const valarray<double>& newfreq, string chopper, const double& samplefreq, const double& gain, const double& factor,
...
*/
size_t FGS01Cal::theoretical(const valarray<double>& newfreq, string chopper = "on", const double& samplefreq = 0, const double& gain = 0,
                             const double& factor = 0,  const double& offset = 0, string adc = "", string tags = "") {

    cerr << "FGS01Cal::theoretical -> trf function for FGS01 " << newfreq.size() << endl;
    freqs.resize(newfreq.size());
    freqs = newfreq;
    trf.resize(newfreq.size());
    for (size_t i = 0; i < freqs.size(); i++) {
...
/*!
    \fn FGS01Cal::set_calib(const double& f, const complex<double>& trfn)
 */
size_t FGS01Cal::set_calib(const valarray<double>& f, const valarray<complex<double> >& trfn) {
    if (f.size() != trfn.size()) {
        cerr << "FGS01Cal::set_calib -> f and trf are not of the same size, exit!" << endl;
        exit(0);
    }
 ...
 */
size_t FGS01Cal::set_calib(const valarray<double>& f, const valarray<double>& ampl, const valarray<double>& ph_deg) {
    /// @todo implement me
    return 0;
