...
/*!
    \fn EFP05Cal::read(string filename, string chopper)
     reads from a file; chopper: on, off, auto, none
     simple check for double entry,
     makes sure that f[0] < f[1]
 */
size_t EFP05Cal::read(const valarray<double>& newfreq, string chopper = "on", const double& samplefreq = 0, const double& gain = 0,
    ...
    if (!ifs) {
        ...
        if (!ifs) {
            cerr << "EFP05Cal::read -> can nor open file " << filename << " neither " << share_dir << " exit" << endl;
            exit(0);
        } else
            cerr << "EFP05Cal::read -> local file not found, taking " << share_dir << " instead" << endl;
    }
    ...
    if (ifs.eof()) {
        cerr << "EFP05Cal::read -> can not find key words chopper on or chopper off; I try a file with numbers only now.... check!!!" << endl;
    ...
    cerr << "EFP05Cal::read -> using " << chopper << endl;
    while(ifs) {
    ...
    if (i != freqs.size()) {
        cerr << "EFP05Cal::read -> at least on frequency with double entry!!! " << endl;
        exit(0);
    }
    ...
    cerr << "EFP05Cal:: read trf: " << freqs[0] << " " << trf[0] << " " << freqs[freqs.size()-1] << " "  << trf[freqs.size()-1] << endl;
    return freqs.size();
...
/*!
    \fn EFP05Cal::dump(string filename)
     debug function; just dump values to file
 */

size_t EFP05Cal::dump(const string& filename) {

    ofstream ofs;
    size_t i;
    if (!filename.size()) {
        cerr << "EFP05Cal::dump -> no filename given" << endl;
        return 0;
    }
    ofs.open(filename.c_str());
    if (!ofs) {
        cerr << "EFP05Cal::dump -> not open output stream" << endl;
        return 0;

    }

    if (!freqs.size() || !trf.size()) {
        cerr << "EFP05Cal::dump -> no frequencies / complex data to dump" << endl;
        return 0;
...
for ( i = 0; i < f.size(); i++) f[i] = i * samplefreq / wl;
 ...
 multiply by f again
*/
size_t EFP05Cal::interpolate(const valarray<double>& newfreq) {
    ...
    if (trf.size()) {
        ...
        cerr << "EFP05Cal::interpolate -> interpolating sensor trf (" << newfreq[1] << " - "
        ...
        cerr << "EFP05Cal::interpolate -> interpolating ERROR" << endl;
    ...
    cerr << "EFP05Cal::interpolate -> " << freqs[0] << " " << trf[0] << ", " << freqs[1] << " " << trf[1] << " " << freqs[freqs.size()-1] << " "  << trf[freqs.size()-1] << flush << endl;
...
*/
size_t EFP05Cal::interpolate_extend_theoretical(const valarray<double>& newfreq, string chopper = "on", const double& samplefreq = 0,
    ...
    if (freqs[freqs.size()-1] < 513) {
        cerr << "EFP05Cal::interpolate_extend_theoretical -> extrapolating trf function forEFP05" << endl;
...
*/
size_t EFP05Cal::hardwired(const valarray<double>& newfreq, string chopper, const double& samplefreq, const double& gain, const double& factor,
...
*/
size_t EFP05Cal::theoretical(const valarray<double>& newfreq, string chopper = "on", const double& samplefreq = 0, const double& gain = 0,
                           const double& factor = 0,  const double& offset = 0, string adc = "", string tags = "") {
    ...
    if (freqs[freqs.size()-1] < 513) {
        cerr << "EFP05Cal::theoretical -> trf function for mfs coils 05 06 08U KIM" << endl;
        for (size_t i = 0; i < freqs.size(); i++) {
            p1 = (freqs[i]/4.) * im;
            p2 = (freqs[i]/8192.) * im;
            trf[i] = 1000. * 0.8 * (  (p1 / ( 1. + p1)) * (1. / (1. + p2))  );
...
/*!
    \fn EFP05Cal::set_calib(const double& f, const complex<double>& trfn)
 */
size_t EFP05Cal::set_calib(const valarray<double>& f, const valarray<complex<double> >& trfn) {
    if (f.size() != trfn.size()) {
        cerr << "EFP05Cal::set_calib -> f and trf are not of the same size, exit!" << endl;
        exit(0);
    }
 ...
 */
size_t EFP05Cal::set_calib(const valarray<double>& f, const valarray<double>& ampl, const valarray<double>& ph_deg) {
    /// @todo implement me
    return 0;
