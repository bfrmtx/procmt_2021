...
bool Plot::showData(QVector<QVector<QVector<QString>>> &qvecTFHeader, QVector<QVector<QVector<QVector<double>>>> &qvecTFData,
  ...
  for (CHCounter = 0; CHCounter < qvecTFData[0].size(); CHCounter++) {
    ...
    if ((pclMagPlot != Q_NULLPTR) && qvecMag.size()) {
      ...
      if ((amin < minf_ampl) || minf_ampl == 0.0)
        minf_ampl = amin;
    ...
    if ((pclCalChopperOnAmplPlot != Q_NULLPTR) && (qvecCalChopperOnAmpl.size() > CHCounter)) {
      ...
      pclCalChopperOnAmplPlot->graph(uiGraphCount)->addData(qvecCalChopperOnFreq[CHCounter], qvecCalChopperOnAmpl[CHCounter]);
    ...
    if ((pclCalChopperOnPhasePlot != Q_NULLPTR) && (qvecCalChopperOnPhase.size() > CHCounter)) {
      ...
      pclCalChopperOnPhasePlot->graph(uiGraphCount)->addData(qvecCalChopperOnFreq[CHCounter], qvecCalChopperOnPhase[CHCounter]);
    ...
    if ((pclCalChopperOffAmplPlot != Q_NULLPTR) && (qvecCalChopperOffAmpl.size() > CHCounter)) {
      ...
      pclCalChopperOffAmplPlot->graph(uiGraphCount)->addData(qvecCalChopperOffFreq[CHCounter], qvecCalChopperOffAmpl[CHCounter]);
    ...
    if ((pclCalChopperOffPhasePlot != Q_NULLPTR) && (qvecCalChopperOffPhase.size() > CHCounter)) {
      ...
      pclCalChopperOffPhasePlot->graph(uiGraphCount)->addData(qvecCalChopperOffFreq[CHCounter], qvecCalChopperOffPhase[CHCounter]);
...
void Plot::on_pbPrint_clicked(void) {
  ...
  for (Counter = 0; Counter < this->qvecCursorMagPlot.size(); Counter++) {
    if (this->qvecCursorMagPlot.at(Counter).hLine)
      this->ui->plotMag->removeItem(this->qvecCursorMagPlot.at(Counter).hLine);
    if (this->qvecCursorMagPlot.at(Counter).vLine)
      this->ui->plotMag->removeItem(this->qvecCursorMagPlot.at(Counter).vLine);
  }

  for (Counter = 0; Counter < this->qvecCursorCohPlot.size(); Counter++) {
    if (this->qvecCursorCohPlot.at(Counter).hLine)
      this->ui->plotCoh->removeItem(this->qvecCursorCohPlot.at(Counter).hLine);
    if (this->qvecCursorCohPlot.at(Counter).vLine)
      this->ui->plotCoh->removeItem(this->qvecCursorCohPlot.at(Counter).vLine);
  }

  for (Counter = 0; Counter < this->qvecCursorNoisePlot.size(); Counter++) {
    if (this->qvecCursorNoisePlot.at(Counter).hLine)
      this->ui->plotNoise->removeItem(this->qvecCursorNoisePlot.at(Counter).hLine);
    if (this->qvecCursorNoisePlot.at(Counter).vLine)
      this->ui->plotNoise->removeItem(this->qvecCursorNoisePlot.at(Counter).vLine);
  }
