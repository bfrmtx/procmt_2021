...
void prc_com::set_data(const QMap<QString, QVariant> &in_map, const bool strip_classname, const bool check_options) {
  ...
  while (citer != in_map.constEnd()) {
    ...
    if (strip_classname && new_key.contains(this->myname) && this->myname.size()) {
      ...
      this->set_key_value(new_key, citer.value(), check_options);
    } else if (!strip_classname)
      this->set_key_value(citer.key(), citer.value(), check_options);
    ++citer;
  }
}
  ...
  while (citer != this->constEnd()) {
    if (show_values) {
      if (prepend_leading_minus) {
        ...
        qInfo() << citer.key() << " -> " << citer.value().toString();
    }
    if (!show_values) {
      if (prepend_leading_minus) {
...
void prc_com::scan_cmdline(const QStringList &argues, QStringList &files, QStringList &atsfiles, QStringList &measdocs, const bool remove_leading_minus, const bool check_and_replace, const bool auto_remove) {
  ...
  for (i = 0; i < argues.size(); ++i) {
    arg = argues.at(i);
    if (remove_leading_minus)
      if (arg.startsWith("-"))
        arg.remove(0, 1); // remove the leading minus
    if (i < argues.size()) {
      if (check_and_replace) {
        ...
        if (tmp_map.contains(arg) && (arg.compare("files") != 0)) {
          ...
          if ((tmp_map[arg].metaType().id() == QMetaType::Double))
            QMap<QString, QVariant>::insert(arg, argues.at(i + 1).toDouble());
          else if ((tmp_map[arg].metaType().id() == QMetaType::Int))
            QMap<QString, QVariant>::insert(arg, argues.at(i + 1).toInt());
          else if ((tmp_map[arg].metaType().id() == QMetaType::UInt))
            QMap<QString, QVariant>::insert(arg, argues.at(i + 1).toUInt());
          else if ((tmp_map[arg].metaType().id() == QMetaType::ULongLong))
            QMap<QString, QVariant>::insert(arg, argues.at(i + 1).toULongLong());
          else if ((tmp_map[arg].metaType().id() == QMetaType::LongLong))
            QMap<QString, QVariant>::insert(arg, argues.at(i + 1).toLongLong());
          else if ((tmp_map[arg].metaType().id() == QMetaType::QString) && !argues.at(i + 1).compare("true"))
            QMap<QString, QVariant>::insert(arg, true);
          else if ((tmp_map[arg].metaType().id() == QMetaType::QString) && !argues.at(i + 1).compare("false"))
            QMap<QString, QVariant>::insert(arg, false);
          else
