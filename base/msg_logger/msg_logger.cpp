...
QStringList msg_logger::get_all_as_single_lines() {
  ...
  if (this->c0.empty())
    return strlst;
  while (!this->c0.empty()) {
    line.append(this->c0.front().toString());
    line.append(" ");
    line.append(this->c1.front().toString());
    line.append(" ");
    line.append(this->c2.front().toString());
    line.append(" ");
    line.append(this->c3.front().toString());
    line.append(" ");
    this->c0.pop();
    this->c1.pop();
    this->c2.pop();
...
void msg_logger::parzen_radius_changed(const double &old_prz, const double &prz) {
  ...
  QString m("from ");
  m.append(QString::number(old_prz, 'f', 3));
  m.append(" to ");
