...
emerald_xtrx::emerald_xtrx(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba, QObject *parent) : measdocxml(qfi, qurl, qba, parent) {
}
...
*/
  ...
  if (emits)
    emit this->signal_emerald_channel_data_qmap_created(this->emerald_Channel_data);
}
...
void emerald_xtrx::create_emerald(const bool emits) {
  ...
  if (emits)
    emit this->signal_emerald_emerald_data_qmap_created(this->emerald_EmeraldData_data);
}
