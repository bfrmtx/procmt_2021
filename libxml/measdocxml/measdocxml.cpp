...
measdocxml::measdocxml(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba, QObject *parent) :
    QObject(parent) {

  this->open(qfi, qurl, qba);
}

measdocxml::measdocxml(QObject *parent) :
    QObject(parent) {
  this->clear();
}
...
bool measdocxml::open(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba) {
  ...
  // avoid that qfi AND qurl are both set
  if (qfi != nullptr && qurl != nullptr) {
    std::cerr << " measdocxml::measdocxml -> failed to initialize class - both QFileInfo and QUrl are set! " << std::endl;
    return false;
  }
  // if qfi is nullptr and qurl is set, we need to convert the QUrl to QFileInfo
  if (qurl != nullptr && qfi == nullptr) {
  ...
  } else if (qfi != nullptr && qurl == nullptr) {
  ...
  // for MSVC++ we have to open the LoadFile with fp and use _wfopen_s
  // check if MSVC++ is used
#ifdef _MSC_VER
  FILE *fp = nullptr;
  errno_t err = _wfopen_s(&fp, this->qfi.absoluteFilePath().toStdWString().c_str(), L"rb");
  if (err != 0 || fp == nullptr) {
    std::cerr << "measdocxml::measdocxml -> can not open file " << this->qfi.absoluteFilePath().toStdString() << std::endl;
    return false;
  }
  // load the file into the XMLDocument
  tinyxml2::XMLError loaded_doc = this->doc.LoadFile(fp);
  fclose(fp);
  // check if we could load the file
  if (loaded_doc != tinyxml2::XML_SUCCESS) {
    this->doc.Clear();
    std::cerr << "measdocxml::measdocxml -> can not parse file " << this->qfi.absoluteFilePath().toStdString() << std::endl;
    return false;
  }
  ...
  // we need to convert the absolute path to be openable by tinyxml2 XMLDocument::LoadFile(const char *filename)
  std::string filenameStr = this->qfi.absoluteFilePath().toStdString();
  const char *filename = filenameStr.c_str();
  if (filename == nullptr) {
    std::cerr << "measdocxml::measdocxml -> can not convert file name to char* " << this->qfi.absoluteFilePath().toStdString() << std::endl;
    return false;
  }
  tinyxml2::XMLError loaded_doc = this->doc.LoadFile(filename);
  // check if we could load the file
  if (loaded_doc != tinyxml2::XML_SUCCESS) {
    this->doc.Clear();
    std::cerr << "measdocxml::measdocxml -> can not parse file " << this->qfi.absoluteFilePath().toStdString() << std::endl;
...
bool measdocxml::save(const QFileInfo *qfi) {
  ...
  if (!this->root)
    return false;
#ifdef _MSC_VER
  // On MSVC/Windows, use wide-character file paths for Unicode support
  FILE *fp = nullptr;
  errno_t err = _wfopen_s(&fp, this->qfi.absoluteFilePath().toStdWString().c_str(), L"wb");
  if (err != 0 || fp == nullptr) {
    std::cerr << "measdocxml::save -> can not open file " << this->qfi.absoluteFilePath().toStdString() << std::endl;
    return false;
  }
  tinyxml2::XMLError er = doc.SaveFile(fp);
  fclose(fp);
#else
  tinyxml2::XMLError er;
  if (!this->qfi.baseName().size())
    return false;
  else {
    std::string filenameStr = this->qfi.absoluteFilePath().toStdString();
    const char *filename = filenameStr.c_str();
    if (filename == nullptr) {
      std::cerr << "measdocxml::save -> can not convert file name to char* " << this->qfi.absoluteFilePath().toStdString() << std::endl;
      return false;
    }
    er = doc.SaveFile(filename);
  }
#endif
  if (er == 0)
    return true;
...
int measdocxml::change_all_elements_text(const QString &element_name, const QString &value) {
  ...
  while (xlement) {
    if (xlement->Value() && !std::string(xlement->Value()).compare(sSearchString)) {
      if (this->verbose)
        qInfo() << "---> measdocxml::change_all_element_text setting" << xlement->Value() << "-> " << value;
      xlement->SetText(strval.c_str());
      ++n;
    } else {
      if (this->verbose)
        qInfo() << "measdocxml::change_all_element_text scanning -> " << xlement->Value() << xlement->GetText();
    }
    /*
...
int measdocxml::pvt_set_section(const QString &section_name, const int &nth_section, const bool second_call_for_subsection) {
  ...
  if (second_call_for_subsection && !this->section)
    return 0;
  ...
  if (second_call_for_subsection)
    xmlelement = this->section->FirstChildElement();
  else
    xmlelement = doc.FirstChildElement();
  while (xmlelement) {
    ...
    if ((xmlelement->Value()) && (!std::string(xmlelement->Value()).compare(sSearchString)) && (xmlelement->FirstChildElement())) {
      if (this->verbose)
        qInfo() << " ---> measdocxml::set_section got section" << xmlelement->Value();
      ...
      if (n == nth_section) {
        if (second_call_for_subsection)
          this->subsection = xmlelement;
        else
          this->section = xmlelement;
        return n;
      }
    } else {
      if (this->verbose)
        qInfo() << "measdocxml::set_section reaching" << xmlelement->Value() << xmlelement->GetText();
    }
    ...
    if (xmlelement->FirstChildElement()) //(1)
      ...
      while (xmlelement->Parent() && !xmlelement->Parent()->NextSiblingElement()) { //(3)
        ...
        if (second_call_for_subsection && xmlelement == this->section)
          break;
        else if (xmlelement == this->root)
          break;
      }
...
int measdocxml::get_siblings_data(tinyxml2::XMLElement *start_sibling, QMap<QString, QVariant> &data, const bool insert_undefinded) {
  ...
  do {
    if (!xmlelement->FirstChildElement()) {
      if (this->verbose)
        qInfo() << "measdocxml::get_siblings_data getting : " << xmlelement->Value() << xmlelement->GetText();
...
int measdocxml::get_siblings_data_txm(tinyxml2::XMLElement *start_sibling, QMap<QString, QVariant> &data, const QString &idstr,
  ...
  do {
    ...
    if (!xmlelement->FirstChildElement()) {
      ...
      if (this->verbose)
        qInfo() << "measdocxml::get_siblings_data_txm getting : " << xmlelement->Value() << xmlelement->GetText() << iattr;
      // E3_Position_0_Elevation
      if (use_topnode.size() && use_node.size() && (position_id != -1))
        element_str = use_topnode + "_" + use_node + "_" + QString::number(position_id) + "_" + element_str;
      else if (use_topnode.size() && use_node.size())
        element_str = use_topnode + "_" + use_node + "_" + element_str;
      if (iattr != -1 && (position_id == -1))
        element_str += "_" + idstr + "_" + QString::number(iattr);
      if (conv) {
        if (data.contains(element_str)) {
          QVariant in = element_value;
          data.insert(element_str, this->get_native(in, data.value(element_str)));
  ...
  do {
    ...
    if (data.contains(name)) {
      ...
      if (data.value(name).metaType().id() == QMetaType::Double) {
        ...
        xmlelement->SetText(data.value(name).toString().toStdString().c_str());
    }
    xmlelement = xmlelement->NextSiblingElement();
    ++n;
...
QStringList measdocxml::get_atsfiles(QList<int> &channel_nos) {
  ...
  if (channel_nos.size() == filenames.size())
    return filenames;
  return QStringList();
}
...
void measdocxml::query_default_attributes(const tinyxml2::XMLElement *xmlelement) {
  ...
  if (this->verbose)
    qInfo() << "GMS =" << this->GMS << ", Type =" << this->Type << ", Version =" << this->Version << ", Name =" << this->Name;
}
...
int measdocxml::get_txm_section(const QString &topnode, const QString &node, QMap<QString, QVariant> &data_map,
  ...
  if (this->goto_sub_section(topnode, node, 1) == 1) {
    ...
    if (position_id != -1)
      this->get_siblings_data_txm(xmlelement->FirstChildElement(), data_map, idstr, use_topnode, use_node, iattr);
    else
      this->get_siblings_data_txm(xmlelement->FirstChildElement(), data_map, idstr, use_topnode, use_node, -1);
    xmlelement = xmlelement->NextSiblingElement();
    if (!xmlelement)
      return 0;
    iattr = -1;
...
void measdocxml::create_sensor(const bool emits) {
  ...
  if (emits)
    emit this->signal_sensor_data_qmap_created(this->sensor_data);
}
...
void measdocxml::create_atswriter(const bool emits) {
  ...
  if (emits)
    emit this->signal_atswriter_configuration_qmap_created(atswriter_configuration);
  ...
  if (emits)
    emit this->signal_atswriter_data_qmap_created(this->atswriter_data);
...
void measdocxml::create_gps_status(const bool emits) {
  ...
  if (emits)
    this->signal_gps_status_qmap_created(this->gps_status);
}
...
void measdocxml::create_global_config(const bool emits) {
  ...
  if (emits)
    emit this->signal_global_config_qmap_created(this->global_config);
}
...
void measdocxml::create_adbboard_autogain(const bool emits) {
  ...
  if (emits)
    emit this->signal_adbboard_autogain_qmap_created(this->adbboard_autogain);
}

QString measdocxml::filename_from_atswriter_section(const QMap<QString, QVariant> &atswriter_section) {
  if (atswriter_section.value("ats_data_file").toString() == this->qfi.fileName())
    return this->qfi.fileName();
...
void measdocxml::slot_update_atswriter(const QMap<QString, QVariant> &atswriter_section, const int &channel_id, const QString &measdir, const QString &measdoc) {
  ...
  if (atsw.contains("sensor_type")) {
    ...
    if (exchanged)
      this->set_channel_section("ATSWriter", atsw, channel_id, true);
  }
  if (!exchanged)
