...

{
    ...
    this->csv_header << "serial" << "chopper" << "f" << "a" << "p" << "stddev a" << "stddev p";
...
{
    if (this->master_calibration_db.isOpen()) this->master_calibration_db.close();
    // destroy the reference
    this->master_calibration_db = QSqlDatabase();
    QSqlDatabase::removeDatabase(this->master_calibration_db_connection_name);

    if (this->sensor_db.isOpen()) this->sensor_db.close();
    // destroy the reference
...
bool calibration::operator ==(const calibration &rhs)
{
    ...
    if (ret_ser && ! ret_type) return true;
...
void calibration::read_measdoc(const int &channel, const cal::cal &chopper, const QFileInfo &qfi)
{
    ...
    if (this->f_on.size()) this->sort_lower_to_upper_check_duplicates(cal::calibration, cal::chopper_on);
    if (this->f_off.size()) this->sort_lower_to_upper_check_duplicates(cal::calibration, cal::chopper_off);
...
size_t calibration::interploate(const std::vector<double> &newfreqs, const int channel, const cal::cal chopper, const bool overlap, const bool extend_theo, const bool limit_upper_to_cal)
{
    ...
    if (this->is_linear_cal) return 0;
    if (!newfreqs.size()) return 0;
    ...
    if (chopper == cal::chopper_on) {
        ...
        if (!this->f_on_master.size()) this->get_master_cal();
        ...
        if ((this->f_on.size()) && (this->f_on.size() == this->ampl_on.size()) && (f_on.size() == this->phase_grad_on.size()) ) {
            ...
            if (extend_theo) {
                if (extend_theo && (limit_upper_to_cal == false)) this->gen_theo_cal(newfreqs);
            ...
            for (auto &f : newfreqs) {
                if ((f >= *min_max.first) && (f <= *min_max.second)) accept_new_freqs.emplace_back(f);
            }
    ...
    if (chopper == cal::chopper_off) {
        ...
        if (!this->f_off_master.size()) this->get_master_cal();
...
void calibration::eliminate_except_complex()
{
    ...
    this->f_on.clear(); this->phase_grad_on.clear(); this->ampl_on.clear();
    this->f_off.clear(); this->phase_grad_off.clear(); this->ampl_off.clear();
    this->phase_grad_on_stddev.clear(); this->ampl_on_stddev.clear();
    this->phase_grad_off_stddev.clear(); this->ampl_off_stddev.clear();

    this->phase_grad_on_master_stddev.clear(); this->ampl_on_master_stddev.clear();
    this->phase_grad_off_master_stddev.clear(); this->ampl_off_master_stddev.clear();

    this->f_on_theo.clear(); this->phase_grad_on_theo.clear(); this->ampl_on_theo .clear();
    this->f_off_theo.clear(); this->phase_grad_off_theo.clear(); this->ampl_off_theo.clear();
    this->phase_grad_on_theo_stddev.clear(); this->ampl_on_theo_stddev.clear();
    this->phase_grad_off_theo_stddev.clear(); this->ampl_off_theo_stddev.clear();

    this->f_on_master.clear(); this->phase_grad_on_master.clear(); this->ampl_on_master .clear();
    this->f_off_master.clear(); this->phase_grad_off_master.clear(); this->ampl_off_master.clear();

    this->f_on_ipl.clear(); this->phase_grad_on_ipl.clear(); this->ampl_on_ipl.clear();
    this->f_off_ipl.clear(); this->phase_grad_off_ipl.clear(); this->ampl_off_ipl.clear();
}
...
bool calibration::prepare_cal_db()
{
    ...
    if (!this->sensor_db.isOpen()) return false;
    QString message;
    QString query_str;
...
int calibration::insert_into_cal_db()
{
    ...
    if (!this->sensor_db.isOpen()) return 0;
    QString message;
    ...
    if ((this->f_on.size() == this->ampl_on.size()) && (this->f_on.size() == this->phase_grad_on.size()) && f_on.size() ) {
        for (size_t i = 0; i < this->f_on.size(); ++i) {
            ...
            values.append( "') ");
            if (i < this->f_on.size()-1) values.append( ", ");
        }
    ...
    if ((this->f_off.size() == this->ampl_off.size()) && (this->f_off.size() == this->phase_grad_off.size()) && f_off.size() ) {
        for (size_t i = 0; i < this->f_off.size(); ++i) {
            ...
            values.append( "') ");
            if (i < this->f_off.size()-1) values.append( ", ");
        }
...
bool calibration::select_coil(const QString &sensortype, const int &sernum, bool b_get_cal)
{
    ...
    if (!this->coil_exist(false)) return false;
    if (b_get_cal) {
        if (this->get_cal()) return true;
...
{
    if (!sensortype.size()) return QString("empty");
    if (sernum == INT_MAX) return QString("empty");

    QStringList entries;
    entries << "*.txt" << "*.TXT" << "*.cal" << "*.CAL" ;
...
size_t calibration::get_master_cal(const bool no_cal_found)
{
    ...
    // check again
    if (!this->ukn_sensor) this->sensortype = this->set_atsheader_sensor(this->sensortype);
    else return 0;
...
QMultiMap<QString, int> calibration::all_sensors_in_db()
{
    ...
    if (!this->sensor_db.isOpen()) return alldb;
...
size_t calibration::write_std_txt_file(const QFileInfo &qfi)
{
    ...
    if (this->f_on.size() || this->f_off.size()) {
        ...
        if (calfile.open(QIODevice::WriteOnly)) {
            ...
            qts << "Metronix GmbH, Kocherstr. 3, 38120 Braunschweig" << Qt::endl << Qt::endl;
            qts << " Magnetometer: " << this->sensortype << " #" << QString::number(this->sernum) << "   Date: " << this->caltime.toString("dd/MM/yy") << "  Time:" << this->caltime.toString("HH:mm:ss") << Qt::endl << Qt::endl;
            if (this->f_on.size()) {
                qts << Qt::endl;
                ...
                for (size_t i = 0; i < f_on.size(); ++i) {
                    ...
                    if (this->phase_grad_on.at(i) > 0.0) qts << " ";
            ...
            if (this->f_off.size()) {
                ...
                for (size_t i = 0; i < f_off.size(); ++i) {
                    ...
                    if (phase_grad_off.at(i) > 0.0)  qts << " ";
...
{
    if ( !(this->f_on.size() + f_off.size())) return 0;
...
QString calibration::create_xlmstr() const
{
    ...
    if (this->sensortype.startsWith("fg", Qt::CaseInsensitive)) tixw.element("ci", "identifier=\"fluxgate\"", this->sensortype);
    if (this->sensortype.contains("squid", Qt::CaseInsensitive)) tixw.element("ci", "identifier=\"squid\"", this->sensortype);
    if (this->sensortype.contains("efp", Qt::CaseInsensitive)) tixw.element("ci", "identifier=\"electrode\"", this->sensortype);
    else tixw.element("ci", "identifier=\"coil\"", this->sensortype);
...
{
    if (this->sensortype.startsWith("fg", Qt::CaseInsensitive)) this->calinfo.insert("ci_sensortype", QString("fluxgate"));
    if (this->sensortype.contains("squid", Qt::CaseInsensitive)) this->calinfo.insert("ci_sensortype", QString("squid"));
    if (this->sensortype.contains("efp", Qt::CaseInsensitive)) this->calinfo.insert("ci_sensortype", QString("electrode"));
    else this->calinfo.insert("ci_sensortype", "coil");
...
int calibration::sensors_summary()
{
    ...
    while (i != this->sensor_types_tables.constEnd()) {
        ...
        if (!query.exec(query_str)) {
            ...
            isum += isize;
            if (isize > 0) {
                if (!this->selectable_sensors.contains(i.key())) this->selectable_sensors.append(i.key());
            }
            isize = 0;
...
size_t calibration::gen_theo_cal(const std::vector<double> &fin_f, const double dad, const double dpg, const bool limit_fcal_high)
{
    ...
    if (limit_fcal_high && !this->sensortype.contains("FGS")) {
        if (this->chopper == cal::chopper_on) {
            for ( auto &f : fin_f) {
                if (f <= this->f_on.back()) fin.emplace_back(f);
            }
        }
        if (this->chopper == cal::chopper_off) {
            for ( auto &f : fin_f) {
                if (f <= this->f_off.back()) fin.emplace_back(f);
            }
        }
    ...
    if (this->chopper == cal::chopper_on) {
        ...
        if (dad > 0.0001) this->ampl_on_theo_stddev.reserve(fin.size());
        else this->ampl_on_theo_stddev.clear();
        if (dpg > 0.0001) this->phase_grad_on_theo_stddev.reserve(fin.size());
        else this->phase_grad_on_theo_stddev.clear();
    }
    if (this->chopper == cal::chopper_off) {
        ...
        if (dad > 0.0001) this->ampl_off_theo_stddev.reserve(fin.size());
        else this->ampl_off_theo_stddev.clear();
        if (dpg > 0.0001) this->phase_grad_off_theo_stddev.reserve(fin.size());
        else this->phase_grad_off_theo_stddev.clear();
    }
        ...
        if (this->chopper == cal::chopper_on) {
            for (auto &f : fin) {
                ...
                if (f == 0.0) {
                    ...
                    if (dad > 0.0001) this->ampl_on_theo_stddev.emplace_back(this->ampl_on_theo.back() * dad);
                    if (dpg > 0.0001) this->phase_grad_on_theo_stddev.emplace_back(dpg);
                }
        ...
        if (this->chopper == cal::chopper_off) {
            for (auto &f : fin) {
                ...
                if (f == 0.0) {
                    ...
                    if (dad > 0.0001)  this->ampl_off_theo_stddev.emplace_back(this->ampl_off_theo.back() * dad);
                    if (dpg > 0.0001) this->phase_grad_off_theo_stddev.emplace_back(dpg);
                }
            }
    ...
    if ((this->sensortype == "SHFT-02") || (this->sensortype == "SHFT-02e")) {
        ...
        if (this->chopper == cal::chopper_on) {
            for (auto &f : fin) {
                ...
                if (f > 99)  this->f_on_theo.emplace_back(f);
                if (f == 0.0) {
                    this->zerof(this->ampl_on_theo, this->phase_grad_on_theo, 1.0, 90.0);
                    this->zerof(this->ampl_on_theo_stddev, this->phase_grad_on_theo_stddev, 1.0, 90.0);
        ...
        if (this->chopper == cal::chopper_off) {
            for (auto &f : fin) {
                // theo does not work below 100 - master cal has some more
                if (f > 99)  this->f_off_theo.emplace_back(f);
        ...
        if (this->chopper == cal::chopper_on) {
            for (auto &f : fin) {
                ...
                if (f == 0.0) {
                    ...
                    if (dad > 0.0001) this->ampl_on_theo_stddev.emplace_back(this->ampl_off_theo.back() * dad);
                    if (dpg > 0.0001) this->phase_grad_on_theo_stddev.emplace_back(dpg);
                }
            }
            message = "gen theo cal " + this->sensortype + " #" + QString::number(this->sernum) + " -> " + QString("chopper on: ") +
                                         ...
                                         const double &dc_ampl, const double &dc_phase_deg, const double &mul_by, const bool &mul_by_f, const bool &invert) const
{
    ...
    if (stop_idx <= start_idx) return;
    if ((stop_idx - start_idx) != psrc.size()/2) return;
    if (f_sample == 0.0) return;
...
{
    if (c_0_off_1_on == 1) this->chopper = cal::chopper_on;
...
{
    if (this->input_div == cal::div_1) return;
    double mulme = 1.0;
    if (this->input_div == cal::div_8) mulme = 8.0;
...
std::vector<double> calibration::gen_cal_freqs(const double start, const double stop, const int steps_per_decade)
{
    ...
    if (cf.size() == 0) {
        ...
        if (stop < start) message.append(" wrong order, stop < start !");
        qDebug() << message;
        emit this->tx_cal_message(this->channel, this->slot, message);
    }
...
bool calibration::parse_filename_other_cal(const QFileInfo &qfi)
{
    ...
    for (auto const &tag : tags ) {
        ...
        if (tag.contains("SensorResistance", Qt::CaseInsensitive)) {
            ...
            if (values.size() == 2) {
                int d = values.at(1).toInt(&ok);
                if (ok) this->sensor_resistance = double(d);
            }
        }
...
QString calibration::get_adb_name() const
{
    ...
    if (this->input_div == cal::div_1) name.append(", DIV 1, ");
    if (this->input_div == cal::div_8) name.append(", DIV 8, ");
    name.append("Gain1 ");
    name.append(QString::number(int(this->gain_1)));
    name.append(", ");

    if (this->dac_on) name.append("DAC on, ");
    ...
    if (this->adu08e_rf_1_2 == cal::adu08e_rf_1) name.append("RF-1, ");
    if (this->adu08e_rf_1_2 == cal::adu08e_rf_2) name.append("RF-2, ");
    if (this->adu08e_rf_1_2 == cal::adu07e_rf_3) name.append("RF-3, ");
    if (this->adu08e_rf_1_2 == cal::adu07e_rf_4) name.append("RF-4, ");
    if (this->adu08e_rf_1_2 == cal::adu08e_rf_off) name.append("RF-off, ");

    if (this->adu08e_lp == cal::adu08e_lp4hz_on) name.append("LF-LP-4Hz");
    if (this->adu08e_hp == cal::adu08e_hp500hz_on) name.append("HF-HP-500Hz");
    if (this->adu08e_hp == cal::adu08e_hp1hz_on) name.append("HF-HP-1Hz");
...
{
    if (other_cal == nullptr) return false;
