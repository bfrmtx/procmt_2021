 ...
 */
template<typename T> struct mt_data_t

{
    mt_data_t(const size_t sz, const double &f, const size_t &min_stacks = _min_stacks, const size_t &vector_size = 0) {
        ...
        if (vector_size) {
            for (auto &x : this->coh) {
                if ((i == xy) || (i == yx)) x.resize(vector_size, 0.0);
                ++i;
            }
        }
        ...
        else this->min_stacks = _min_stacks;
    ...
    bool resize(const double &f, const size_t &newsize) {
        ...
        int i = 0;
        for (auto &x : this->coh) {
            if ((i == xy) || (i == yx))  x.resize(newsize, 0.0);
            ++i;
        }
        return true;
    }
   ...
   */
    bool is_true_tensor_min_stacks(const bool count_first) {
        ...
        if (count_first) this->count_true_selections();

        for (auto &tr : this->trues) {
            if (tr < this->min_stacks) return false;
        }
        return true;
   ...
   */
    ...
    double phi_deg(const size_t &elem, const size_t &idx, const bool swap = false) const {
        if (swap) {
            if (elem == tns::yx) return ((arg(this->d[elem][idx] ) * 180.) / M_PI) + 180.0;
            if (elem == tns::yy) return ((arg(this->d[elem][idx] ) * 180.) / M_PI) + 180.0;
 ...
 */
template<typename T> struct mt_data_res {
   ...
   */
     ...
     */
    bool test_tensor() {
        if (!this->freqs.size()) return false;
        int valid = 0;
        // test without tipper
        for (size_t i = 0; i < tns::tns_size; ++i) {
            if (this->d.at(i).size() == this->freqs.size()) valid += 1;
    ...
    bool test_full_tensor() {
        ...
        for (size_t i = 0; i < tns::tns_size; ++i) {
            ...
            if (this->freqs.size() == this->err[i].size()) {
                ...
                for (auto &xe : this->err[i]) {
                    if (xe < 0) xe = 0.0;
                    if (xe > 1.0E31) xe = 0.0; // avoid display crash
                }
            }
        }
    ...
    bool test_full_tensor_tipper()  {
        ...
        if (valid != tns::tns_tip_size) return false;
        return true;
    }
    ...
    bool test_tipper() const {
        ...
        if ( (this->freqs.size() == this->d[tx].size()) && (this->freqs.size() == this->d[ty].size()) ) return true;
        return false;
    }
    ...
    void remove_fake_errors() {
        ...
        for (auto &e : this->err) {
            ...
            if (count == e.size()) e.clear(); // vector to zero size
        }
    ...
    std::vector<double> phi_vector_deg(const size_t &elem, const bool swap = false) const {
        std::vector<double> data;
        if (d.size() < elem) return data;
        if (!this->freqs.size()) return data;

        data.reserve(this->d[elem].size());
        if (swap) {
            for (auto const &dat : this->d[elem]) {
                if (elem == tns::yx) data.push_back(((arg(this->dat ) * 180.) / M_PI) + 180.0);
                else if (elem == tns::yy) data.push_back( ((arg(this->dat ) * 180.) / M_PI) + 180.0);
                else data.push_back((arg(dat) * 180.) / M_PI);
            }
        }
        else {
            for (auto const &dat : this->d[elem]) {
    ...
    double coherency(const size_t &elem, const size_t &idx) const {
        ...
        if ( (elem < tns::xxxy) || (elem > tns::yxyy) ) return 0.0;
        if (!this->freqs.size()) return 0.0;
    ...
    std::vector<double> coherency_vector(const size_t &elem) const {
        std::vector<double> data;
        if (coh.size() < elem) return data;
        if (!this->freqs.size()) return data;

        if ( (elem < tns::xxxy) || (elem > tns::yxyy) ) return data;
    ...
    std::vector<double> rho_vector_err(const size_t &elem, const bool bsqrt) const {
        std::vector<double> data;
        if (d.size() < elem) return data;
        if (err.size()  < elem) return data;
        if (!this->freqs.size()) return data;


        if (this->d[elem].size() != this->err[elem].size()) return data;
        data.reserve(this->d[elem].size());
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            if (data[i] > vals[i]) data[i] = vals[i] / 2.0;
        }
    ...
    double phi_deg_err(const size_t &elem, const size_t &idx, const bool bsqrt) const {
        ...
        double xerr = 0.0;
        if (bsqrt) {
            if (sqrt(this->err[elem][idx]) / abs(this->d[elem][idx]) > 0.99) xerr = 0.0;
            else xerr = (asin (sqrt(this->err[elem][idx]) / abs(this->d[elem][idx]))) / 2.0;
        }
        else {
            if (sqrt(this->err[elem][idx]) / abs(this->d[elem][idx]) > 0.99) xerr = 0.0;
            else xerr = (asin (this->err[elem][idx] / abs(this->d[elem][idx]))) / 2.0;
        }
        if (std::isnan(xerr)) xerr = M_PI_2;
        xerr *= (180. / M_PI);
        return xerr;
    }
    ...
    std::vector<double> phi_vector_deg_err(const size_t &elem, const bool bsqrt) const {
        ...
        for (auto const &dat : this->d[elem]) {
            ...
            if (bsqrt) {
                if (*iter_err < 0) x = 0.0;
                else if (sqrt(*iter_err) / abs(dat) > 0.99) x = 0.0;
                else x = (asin (sqrt(*iter_err) / abs(dat))) / 2.0;
            }
            else {
                if (*iter_err < 0) x = 0.0;
                else if (sqrt(*iter_err) / abs(dat) > 0.99) x = 0.0;
                else x = (asin (*iter_err / abs(dat))) / 2.0;
            }
            if (std::isnan(x)) x = M_PI_2;
            x *= (180. / M_PI);
            data.push_back(x);
            iter_err++;
    ...
    size_t get_rho_star_z_star(std::vector<double_t> &rs, std::vector<double_t> &rs_err, std::vector<double_t> &zs, std::vector<double_t> &zs_err, const size_t &elem) const {
        if (!this->freqs.size()) return 0;
        rs.clear(); rs_err.clear(); zs.clear(); zs_err.clear();
        if ( (elem < tns::xxxy) || (elem > tns::yxyy) ) return 0;
        if (!this->d[elem].size()) return 0;
        if (!this->err[elem].size()) return 0;
        if (this->d[elem].size() != this->err[elem].size()) return 0;
    ...
    bool toASCII_file(const QFileInfo &qfi) const{
        if (!freqs.size()) return false;
        QFile file(qfi.absoluteFilePath());
        QStringList header;
        if (tns_size <= d.size()) {
            header << "#f" << "zxx.re" << "zxx.im" << "delta_zxx";
            header << "zxy.re" << "zxy.im" << "delta_zxy";
            header << "zyx.re" << "zyx.im" << "delta_zyx";
            header << "zyy.re" << "zyy.im" << "delta_zyy";
        }
        if (tns_tip_size == d.size()) {
            header << "tx.re" << "tx.im" << "delta_tx";
            header << "ty.re" << "ty.im" << "delta_ty";
        }

        if (tns_size <= d.size()) {
            header << "rho_xx" << "delta_rho_xx";
            header << "rho_xy" << "delta_rho_xy";
            header << "rho_yx" << "delta_rho_yx";
            header << "rho_yy" << "delta_rho_yy";
            header << "phi_deg_xx" << "delta_phi_deg_xx";
            header << "phi_deg_xy" << "delta_phi_deg_xy";
            header << "phi_deg_yx" << "delta_phi_deg_yx";
            header << "phi_deg_yy" << "delta_phi_deg_yy";
        }
        ...
        for (auto &x : this->err) {
            ...
            for (auto &y : x) {
                if (y == DBL_MAX) {
                    ...
                    if (std::find(idx_del.begin(), idx_del.end(), i) == idx_del.end()) idx_del.push_back(i);
                }
                ++i;
            }
        }
   ...
   */
    void resize(const size_t sz, const std::vector<double> &freqs) {
        ...
        if (this->d.size() != sz) this->clear(true);
        if (!this->d.size()) {
            this->d.resize(sz);
            this->err.resize(sz);
            this->coh.resize(sz);
 ...
 */
template<typename T> struct channel_single_spc_collector_buff
{
     ...
     */
    explicit channel_single_spc_collector_buff(const size_t cht = SIZE_MAX, const size_t spcet = SIZE_MAX ) : chan_type(cht), spce(spcet) { }
   ...
   */
    ...
    void to_vector_vector(const size_t max_size = SIZE_MAX) {
        if (max_size != SIZE_MAX) this->vdata.reserve(max_size);
        else  this->vdata.reserve(this->data.size());
        size_t i = 0;
        while (!data.empty()) {
            if (i < max_size) vdata.emplace_back(std::move(this->data.front()));
            this->data.pop();
            ++i;
   ...
   */
    void to_map() {
        ...
        for (auto &f : this->freqs) this->data_map.emplace(f, std::vector<T>());
        // take the queus
 ...
 *
 */
template<typename T> struct channel_single_spc_collector_mutli_runs_buff
{

    explicit channel_single_spc_collector_mutli_runs_buff(const size_t cht = SIZE_MAX) : chan_type(cht){ }
    ...
    QString get_channel_name() const {
        ...
        if (this->chan_type < size_t(pmt::channel_types.size())) return pmt::channel_types.at(this->chan_type);
        // spectra data? spce should be size:max if not used
        if (this->spce < size_t(pmt::ac_spectra_names.size())) return pmt::ac_spectra_names.at(this->spce);
        return QString("nil");
    }
    ...
    void append_clear_data(channel_single_spc_collector_buff<T> &input) {
        ...
        for (auto &f : input.freqs) this->data.emplace(f, std::vector<T>());
        // resvere new size in order to avoid too many resize
        for (auto &f : input.freqs) this->data[f].reserve(this->data[f].size() + input.data.size());
   ...
   */
    void reserve_mem_freqs(const std::vector<double> target_freqs) {
        // should only create a NEW queue only when f NOT exists
        for (auto &f : target_freqs) this->data.emplace(f, std::vector<T>());
    }
    ...
    void append(const std::vector<T> &data, const double &freq) {
        ...
        for (auto &d : data) this->data[freq].emplace_back(d);
    }
 ...
 */
template<typename T> struct all_spectra_collector
{

    explicit all_spectra_collector(bool use_ac_spectra_names = true) {
        if (!use_ac_spectra_names) data.resize(size_t(pmt::channel_types.size()));
        else {
            data.resize(size_t(pmt::ac_spectra_names.size()));
        }
    }
    ~all_spectra_collector() {
        for (auto &dat : this->data) dat.clear();
 ...
 */
template<typename T> struct f_spectra {
