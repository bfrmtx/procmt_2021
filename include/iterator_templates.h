...
std::vector<T> vector_from_indices(const_iterator v, const std::vector<std::size_t> &sel_size, const std::vector<std::size_t> &indices, const size_t &sel_size_index_pos) {
  ...
  std::size_t sum = 0;
  for (size_t i = 0; i < sel_size_index_pos; ++i)
    sum += sel_size[i];
 ...
 */
template <typename T, typename Iterator>
void simple_detrend(Iterator first, const Iterator last) {
 ...
 */
template <typename T, typename Iterator>
void hanning(Iterator first, const Iterator last) {
 ...
 */
template <typename T, typename Iterator>
void simple_detrend_and_hanning(Iterator first, const Iterator last) {
 ...
 */
template <typename T, typename Iterator>
T mean(Iterator first, const Iterator last, const T &add = T(0)) {
 ...
 */
template <typename T, typename Iterator>
T median(Iterator first, const Iterator last) {
...
template <typename T, typename Iterator>
T median_if(Iterator first, const Iterator last, std::vector<bool>::const_iterator selected_cbeg, std::vector<bool>::const_iterator selected_cend) {
...
template <typename T, typename Iterator>
T variance_old(Iterator first, const Iterator last, T &mean) {
...
template <typename T, typename Iterator>
T stddev(Iterator first, const Iterator last, T &mean) {
...
bool variance2D(Iterator firstx, const Iterator lastx, Iterator firsty, const Iterator lasty,
...
template <typename T, typename Iterator>
inline T fold(Iterator first, const Iterator last, Iterator coef_first, const Iterator coef_last) {

  if (std::distance(first, last) < std::distance(coef_first, coef_last))
    return T(0);
 ...
 */
template <typename T, typename Iterator>
std::size_t fir_dsgn(Iterator first, Iterator last, const T samplefreq, T CutLow, T CutHigh) {
 ...
 */
template <class T, class Iterator>
T seval_const_borders(const Iterator u, Iterator x_first, const Iterator x_last,
