...
void complex_vector_to_real(const std::vector<T> &c, std::vector<S> &r, std::vector<S> &i) {
  r.resize(c.size());
  i.resize(c.size());
  auto ci = c.cbegin();
  auto ce = c.cend();
 ...
 */
template <typename T, typename S>
void complex_vector_to_real(const std::vector<T> &c, std::vector<S> &r) {
  r.resize(c.size());
  auto ci = c.cbegin();
  auto ce = c.cend();
  auto ri = r.begin();
 ...
 */
template <typename T, typename S>
void complex_vector_to_imag(const std::vector<T> &c, std::vector<S> &i) {
  i.resize(c.size());
  auto ci = c.cbegin();
  auto ce = c.cend();
  auto ii = i.begin();
 ...
 */
template <typename T, typename S>
void vector_to_complex(const std::vector<S> &r, const std::vector<S> &i, std::vector<T> &c) {
  if (r.size() != i.size())
    return;
  c.resize(r.size());
  auto ci = c.begin();
...
template <typename T>
std::pair<std::complex<double>, std::complex<double>> minmax_element_cplx(const std::vector<T> &v) {

  std::pair<std::complex<double>, std::complex<double>> min_max;
  if (!is_complex(*v.cbegin()))
    return min_max;
  min_max.first = *v.begin();  // will be min real, min imag
  min_max.second = *v.begin(); // will be max real, max imag

  for (auto &cplx : v) {
    if (std::real(cplx) < std::real(min_max.first))
      min_max.first.real(std::real(cplx));
    if (std::imag(cplx) < std::imag(min_max.first))
      min_max.first.imag(std::imag(cplx));

    if (std::real(cplx) > std::real(min_max.second))
      min_max.second.real(std::real(cplx));
    if (std::imag(cplx) > std::imag(min_max.second))
      min_max.second.imag(std::imag(cplx));
  }
 ...
 */
template <typename T>
std::pair<T, T> min_max2v(const std::vector<T> &x1, const std::vector<T> &x2) {
  std::pair<T, T> min_max;
  auto r1 = std::minmax_element(x1.cbegin(), x1.cend());
  auto r2 = std::minmax_element(x2.cbegin(), x2.cend());
  if (*r1.first < *r2.first)
    min_max.first = *r1.first;
  else
    min_max.first = *r2.first;

  if (*r1.second > *r2.second)
    min_max.second = *r1.second;
  else
    min_max.second = *r2.second;
 ...
 */
template <typename T>
std::pair<double, double> min_max2v_cplx(const std::vector<T> &x1, const std::vector<T> &x2,
  ...
  if (std::real(r1.first) < std::real(r2.first))
    min_max_r.first = std::real(r1.first);
  else
    min_max_r.first = std::real(r2.first);

  if (std::real(r1.second) > std::real(r2.second))
    min_max_r.second = std::real(r1.second);
  else
    min_max_r.second = std::real(r2.second);

  if (std::imag(r1.first) < std::imag(r2.first))
    min_max_i.first = std::imag(r1.first);
  else
    min_max_i.first = std::imag(r2.first);

  if (std::imag(r1.second) > std::imag(r2.second))
    min_max_i.second = std::imag(r1.second);
  else
    min_max_i.second = std::imag(r2.second);

  if (min_max_r.first < min_max_i.first)
    abs_min_max.first = min_max_r.first;
  else
    abs_min_max.first = min_max_i.first;

  if (min_max_r.second > min_max_i.second)
    abs_min_max.second = min_max_r.second;
  else
    abs_min_max.second = min_max_i.second;

  if (real_true_imag_false)
    return min_max_r; // min and max of the real parts
  return min_max_i;
}
...
std::pair<double, double> inner_product_ri(const_iterator first1, const_iterator last1, const_iterator first2) {
  double re(0);
  double im(0);
