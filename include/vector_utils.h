...
bool sort_by_first_mod_second_only(const std::vector<T> &f, std::vector<S> &a) {

  if (f.size() != a.size())
    return false;

  std::vector<std::pair<T, S>> v(f.size());
  size_t i;
  ...
  std::sort(v.begin(), v.end(), iter::cmp_pair_first_lt<std::pair<T, S>>());
...
size_t write_vector(const QString filename, const std::vector<T> &v,
  ...
  if ((skip_first + skip_last) == v.size())
    return 0;
  b += skip_first;
  e -= skip_last;
  ...
  if (filename == "console") {
    ...
    QFile data(filename);
    if (append)
      opensuccess = data.open(QFile::WriteOnly | QFile::Append | QFile::Text);
    else
...
template <typename T, typename S>
size_t write_vector(const QString filename, const std::vector<T> &v, const std::vector<S> &w,
                    const size_t skip_first = 0, const size_t skip_last = 0, const int append = 0) {
...
template <typename T>
size_t write_vector(const QString filename, const std::vector<T> &v, const std::vector<T> &w, const std::vector<T> &err,
                    const size_t skip_first = 0, const size_t skip_last = 0, const int append = 0) {
...
*/
template <typename T>
QString cplx_to_reals(const T c, const int which) {
  if (which == 1)
    return QString::number(real(c), 'E', 8);
  if (which == 2)
    return QString::number(imag(c), 'E', 8);
  if (which == 3) {
  ...
  if (which == 4)
    return QString::number(abs(c), 'E', 8);
  if (which == 5)
    return QString::number(arg(c), 'E', 8);
  if (which == 6)
    return QString::number(((arg(c) * 180.) / M_PI), 'E', 8);
...
*/
template <typename T, typename S>
size_t write_vector_real_cplx(const QString filename, const QString type, const std::vector<T> &v, const std::vector<S> &w,
                              const size_t skip_first = 0, const size_t skip_last = 0, const int append = 0) {
...
size_t read_xy_columns(QFileInfo *qfi, std::vector<T> &x, std::vector<T> &y, const size_t colx = 0, const size_t coly = 1, const bool check_isnumber = true, const size_t size_hint = 0) {
  ...
  if (opensuccess) {
    ...
    while (!in.atEnd()) {
      ...
      // qDebug() << items;
      if (!idx) {
        if (((size_t)items.size() - 1 < colx) || ((size_t)items.size() - 1 < coly))
          return 0;
      }
...
size_t read_xy_columns_min_max_f(QFileInfo *qfi, const double min_f, const double max_f, std::vector<T> &x, std::vector<T> &y,
  ...
  if (opensuccess) {
    ...
    while (!in.atEnd()) {
      ...
      if (!idx) {
        if (((size_t)items.size() - 1 < colx) || ((size_t)items.size() - 1 < coly) || ((size_t)items.size() - 1 < colf))
          return 0;
      }
