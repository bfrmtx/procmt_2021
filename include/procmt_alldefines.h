...
enum threadctrl : std::int32_t {
    ...
    threadctrl_single = 0,                     //!<  thread needs to be triggerd as we have in slider bar; so start/stop on TRIGGER
...
template<class T> bool create_survey_subdirs(const QDir &base_dir) {
...
template <typename T> bool is_E(const size_t chtype) {
    if      (chtype == 0) return true;
    else if (chtype == 1) return true;
    else if (chtype == 5) return true;
    else if (chtype == 6) return true;
    return false;
...
static QMap<QString, double> auto_bw( {{"off", 0},
                                       {"1/8 (larger FFT)", 0.125}, {"1/4 (larger FFT)", 0.25}, {"1/2 (larger FFT)", 0.5},
                                       {"1 (normal FFT)", 1},
                                       {"2 (smaller FFT)", 2}, {"4 (smaller FFT)", 4}, {"8 (smaller FFT)", 8} });

static  QMap<QString, double> auto_prz ({{"off", 0},
                                         {"1/8 (less smoothing)", 0.125}, {"1/4 (less smoothing)", 0.25}, {"1/2 (less smoothing)", 0.5},
                                         {"1 (normal parzen", 1},
                                         {"2 (more smoothing)", 2}, {"4 (more smoothing)", 4}, {"8 (more smoothing)", 8} });
 ...
 *
 */
template<class T> size_t spc_to_auto_cross_spc (const size_t lhs, const size_t rhs){
    ...
    else return pmt::spce::spce_size;
 ...
 */
template<class T> size_t db_cols_to_auto_cross_ssp_index (const QString str_re_im, int &is_ac_1_is_ssp_2_fail_0){
