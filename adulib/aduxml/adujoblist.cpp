...
adujoblist::adujoblist(qint64 grid_time, QObject *parent): QAbstractTableModel(parent)
{
...
QVariant adujoblist::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        if (index.row() < this->jobs.size()) {
            if (index.column() == 0) return this->jobs[index.row()].get("sample_freq", 0);
            if (index.column() == 1) return this->jobs[index.row()].get_start_time().toString("HH:mm:ss   yyyy-MM-dd");
            if (index.column() == 2) {
                QTime st(0,0,0,0);
                         ...
                         const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba)
{
    ...
    if (jobs.size()) {
        ...
        if ((jobs.size() == 1) && (duration < 0) && ( (qfi != nullptr) ||  (qurl != nullptr) || (qba != nullptr) ) && this->want_set_first) {
            ...
            else jobs.last().on_set_duration(duration, false);
    ...
    if (special_list == "JLL") {
        for (auto &jb : jobs) jb.set_special_list(special_list);
...
void adujoblist::set_start_time(const QDateTime &dt, const uint jobno)
{
    ...
    if ( ((this->jobs[0].get_special_list() == "JLL") ||  (this->jobs[0].get_special_list() == "CEA-daily")) && this->jobs.size() == 2) {
        ...
        if (this->jobs[0].get_special_list() == "JLL") this->jobs[0].on_set_duration(jobs[0].seconds_to_midnight( -((9*3600) + 120)) );
        else this->jobs[0].on_set_duration(jobs[0].seconds_to_midnight( (-120)) );
        QDateTime xdt(jobs[0].get_stop_time());
        xdt.setTimeSpec(Qt::UTC);
        xdt = xdt.addSecs(120);
        ...
        if (this->grid_time > 1) {
            ...
            if (newstart < dt) newstart = newstart.addSecs(grid_time);
            this->jobs[jobno].set_start_time(newstart, true);
        }
        else this->jobs[jobno].set_start_time(dt, true);
    }
}
...
int adujoblist::on_save(const QFileInfo *qfiHwConfig, const QUrl *qurlHwConfig, const QByteArray *qbaHwConfig)
{
    ...
    if (this->HWConfig_stream_string.size()) {
        ...
        else qDebug() << "adujoblist::on_save -> HWConfig not written!";
    }
    ...
    if (this->HwDatabase_stream_string.size()) {
        ...
        else qDebug() << "adujoblist::on_save -> HWDatabase not written!";
    }
...
void adujoblist::on_set_TargetDirectory(const QString &TargetDirectory)
{
    ...
    if (!this->TargetDirectory.compare("default")) this->TargetDirectory = "";
}
...
void adujoblist::on_selectable_frequencies_changed(const std::vector<double> &selectable_frequencies)
{
    ...
    for (auto &v : this->selectable_frequencies) dbg << v << " ";
    qDebug() << Qt::endl;
#endif
