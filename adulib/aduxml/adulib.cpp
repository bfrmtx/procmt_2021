...
adulib::adulib(qint64 grid_time, QObject *parent) : QAbstractTableModel(parent) {


    this->values_needs_individual_channel_setting << "dac_val" << "gain_stage1" << "gain_stage2" << "ts_lsb" << "active" << "xmlcal_channel" << "xmlcal_sensor";
    this->values_needs_mapping_channel_setting << "pos_x1" << "pos_x2"
                                               << "pos_y1" << "pos_y2" << "pos_z1" << "pos_z2"
                                               << "channel_type" << "ext_gain"  << "sensor_type" << "ats_data_file"
                                               << "sensor_sernum" << "length" << "North" << "East" << "West" << "South";
...
adulib::~adulib()
{
    ...
    if (this->measdoc_template != nullptr) this->measdoc_template.reset();
...
adulib::adulib(const adulib &newjob) : QAbstractTableModel(newjob.parent())
{
    *this = newjob;
...
{
    if (this->channels.size()) return channels.at(0).get_data_size();
...
QVariant adulib::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    if (orientation == Qt::Horizontal) {
        if ((channels.at(0).get_Type("adbboard") == 10) && (channels.at(5).get_Type("adbboard") == 11)) {
            if (section < 5) return QString("channel %1").arg(section) + QString(" / slot %1").arg(section) ;
            else return QString("channel %1").arg(section - 5) + QString(" / slot %1").arg(section) ;
        }
        else {
...
quint64 adulib::operator - (const adulib &rhs) {
...
//! @todo mutex if delete is active
{
    if (this->atswriter_comments.contains(what)) {
        ...
        if (!what.compare("sample_freq")) {
            ...
            for (auto &chan : channels) {
                if (chan.is_active()) return chan.get(what).toDouble();
            }
        }

        if (ichan == -1 ) {
            for (auto &chan : channels) {
                if (chan.is_active()) return chan.get(what);
            }
        }
    ...
    if (!what.compare("sample_freq")) {
        ...
        if (tmp_sample_freq != value) {
            ...
            for (auto &chan : channels) {
                ...
                if (!ok) qFatal("checking frequency in set / adulib failed!!");
            }
...
bool adulib::fetch_hwcfg(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba) {
    ...
    if (this->hwcfg == nullptr) this->hwcfg =  std::make_shared<measdocxml>(this);
...
bool adulib::fetch_hwdb(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba) {
    ...
    if (!this->hwdb->open(qfi, qurl, qba)) return false;
...
bool adulib::fetch_hwstat(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba)
{
    ...
    if (!this->hwstat->open(qfi, qurl, qba)) return false;
...
QMap<QString, QVariant> adulib::fetch_selftest(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba)
{
    ...
    for (auto &channel : this->channels) {
        if (channel.is_active()) {
            ...
            if (channel.get_selftest("Severity").toString().contains("Warning")) {
                if (severity_errors != 0) severity_errors = 1;
            }
            ...
            if (channel.get("channel_type").toString().startsWith("E")) {
                ...
                else if (!this->resistivity_mode.compare("normal")
                         && ((channel.get_selftest("Resistivity").toDouble() > 5000) ||
    ...
    if (resistivityerrors < severity_errors) selftest_result.insert("selftest_summary", QVariant::fromValue(resistivityerrors));
...
QMap<QString, QVariant> adulib::fetch_global_config(const QFileInfo *qfi, const QUrl *qurl, const QByteArray *qba)
{
    ...
    if (tmpmap.contains("sample_freq")) this->set("sample_freq", tmpmap.value("sample_freq"), -1);
...
{
    if (is_checked) this->keep_grid = true;
    else this->keep_grid = false;
...
void adulib::build_hardware()
{
    ...
    for(auto &chan : channels) chan.replace_UNKN_E();
    for(auto &chan : channels) chan.set_dac_steps();

    if (this->GMS == 7) {
        for(auto &chan : channels) chan.set("filter_type", "ADU07_LF_RF_4");
        for(auto &chan : channels) chan.set("filter_type", "ADU07_HF_HP_1HZ");
    }
    ...
    for (auto &chan : channels) {
        ...
        if ( (chan.get_Name("sensor").contains("MFS",Qt::CaseInsensitive))  ||
            ...
            if (i == 2 || i == 7) chan.set("channel_type", "Hx");
            if (i == 3 || i == 8) chan.set("channel_type", "Hy");
            if (i == 4 || i == 9) chan.set("channel_type", "Hz");
        }
        ...
        if ((chan.get_Name("sensor").contains("EFP",Qt::CaseInsensitive)) ||
            ...
            if (i == 0 || i == 5) chan.set("channel_type", "Ex");
            if (i == 1 || i == 6) chan.set("channel_type", "Ey");
        }
        ++i;
    }
    ...
    for(auto &chan : channels) {
        ...
        if (chan.is_active())   sensor_names << chan.get_Name("sensor");
    }
    ...
    for (auto const &d : pfreq) this->selectable_frequencies.append(d);
...
void adulib::set_start_time(const QDateTime &start_time, const bool emits)
{
    if (this->start_time != start_time) {
        ...
        if (emits) emit this->start_time_changed(this->index, this->start_time, this->duration, this->get("sample_freq", 0).toDouble());
...
QDateTime adulib::set_min_start_time(const int &index, const QDateTime &previous_stop_time, const qint64 &previous_min_delta_start, const qint64 &previous_diff_secs)
{
    ...
    if (newstart < stops) newstart = newstart.addSecs(grid_time);
    ...
    if (old_start_time != this->start_time) emit this->timing_changed();
...
void adulib::update_channel_map(bool emits)
{
    ...
    if (emits) {
        if (this->channel_map != tmp_channel_map) emit this->channel_map_changed(this->channel_map);
    }
}
...
void adulib::sensor_calibration()
{
    ...
    for (auto &chan : channels) {
        if (chan.is_active()) {
            ...
            else xcal.emptyelement("calibration");
            xcal.pop();
        }
    }
...
void adulib::set_min_delta_start(const bool emits)
{
    ...
    if (this->min_delta_start != tmp)  emit this->timing_change_attempt();
    if (emits) {
        if (this->min_delta_start != tmp) emit this->min_delta_start_changed(this->index, this->start_time, this->duration, this->get("sample_freq", 0).toDouble());
    }
}
...
QString adulib::on_build_xml(const bool set_astwriter_finhished)
{
    ...
    if ((this->special_list == "JLL") || (this->special_list == "CEA-daily")) {
        ...
        if (this->special_list == "JLL") {
            ...
            for (int ii = 1; ii < 6; ++ii) {
                ...
                if (new_start_datetime > this->start_time) break;
            }
    ...
    if (this->special_list == "InSituCal")  {
        ...
        if (sfreq == 131072) calfreq = 256.0;
        if (sfreq == 32768)  calfreq = 64.0;
        if (sfreq == 4096)   calfreq = 8.0;
        if (sfreq == 256)    calfreq = 0.125;
...
void adulib::on_set_duration(const qint64 &duration, const bool emits)
{
    if (duration && (this->duration != duration )) {
        ...
        emit this->timing_changed();
        if (emits) emit this->duration_changed(this->index, this->start_time, this->duration, this->get("sample_freq", 0).toDouble());
    }
}
...
QStringList adulib::get_H_sensors(QStringList *alias_names, QList<int> *input_divider) const
{
    ...
    if (db.open()) {
        ...
        if (alias_names == nullptr) {
            ...
            else qDebug() << "db err: could not get H sensors" ;
        }
    ...
    else qDebug() << "can not open" << db.databaseName();
    return sensorlist;
...
QStringList adulib::get_E_sensors(QStringList *alias_names, QList<int> *input_divider) const
{
    ...
    if (db.open()) {
        ...
        if (alias_names == nullptr) {
            ...
            else qDebug() << "db err: could not get E sensors " ;
        }
        else if ((alias_names != nullptr) && (input_divider != nullptr)) {
