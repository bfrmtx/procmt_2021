...
bool adbboard::operator==(const adbboard &rhs) {
  return ((GMS == rhs.GMS) && (RevMain == rhs.RevMain) && (RevSub == rhs.RevSub) && (Type == rhs.Type) && aduslot::operator==(rhs));
}
...
bool adbboard::set(const QString &what, const QVariant &value) {
  ...
  // Other base classes can set the frequency again ...
    ...
    if (this->data.value("sample_freq").toDouble() < 1024) {
      ...
      if (this->is_active())
        this->prvdata.insert("hchopper", 1);

    } else {
      // if (this->is_active()) qDebug() << "chopper off";
      if (this->is_active())
        this->prvdata.insert("hchopper", 0);
    }
...
void adbboard::traverse(const int &tinput, const QString &tinput_circuitry, const QString &tfilter_type_main) {
  ...
  if (this->auto_filter_type_main.size() == 0)
    return;
  if ((this->auto_filter_type_main.size() != this->auto_input.size()) || (this->auto_filter_type_main.size() != this->auto_input_circuitry.size()) || (this->auto_filter_type_main.size() != this->auto_gain_stage1.size()) || (this->auto_filter_type_main.size() != this->auto_gain_stage2.size()) || (this->auto_filter_type_main.size() != this->auto_filter_type.size()) || (this->auto_filter_type_main.size() != this->auto_dac_val.size()) || (this->auto_filter_type_main.size() != this->auto_dac_on.size())) {
    return;
  }
