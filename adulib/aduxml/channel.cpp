...
void channel::replace_UNKN_E() {
  int i = this->get_slot();
  if (i == 0 || i == 5 || i == 1 || i == 6) {
    if (!QString::compare(this->get_Name("sensor"), "UNKN_E"))
      this->set_Name("sensor", "EFP-06");
  } else {
    if (!QString::compare(this->get_Name("sensor"), "UNKN_E"))
      this->set_Name("sensor", "MFS-06");
  }
}
...
void channel::slot_fetch_atswriter(const QMap<QString, QVariant> &xmlpart, const int &id_slot) {
  ...
  if (id_slot != this->get_slot())
    return;
  if (xmlpart.contains("pos_x1"))
    this->set("pos_x1", xmlpart.value("pos_x1"));
  if (xmlpart.contains("pos_x2"))
    this->set("pos_x2", xmlpart.value("pos_x2"));
  if (xmlpart.contains("pos_y1"))
    this->set("pos_y1", xmlpart.value("pos_y1"));
  if (xmlpart.contains("pos_y2"))
    this->set("pos_y2", xmlpart.value("pos_y2"));
  if (xmlpart.contains("pos_z1"))
    this->set("pos_z1", xmlpart.value("pos_z1"));
  if (xmlpart.contains("pos_z2"))
    this->set("pos_z2", xmlpart.value("pos_z2"));

  if (xmlpart.contains("ats_data_file"))
    this->set("ats_data_file", xmlpart.value("ats_data_file"));
  if (xmlpart.contains("ts_lsb"))
    this->set("ts_lsb", xmlpart.value("ts_lsb"));
  if (xmlpart.contains("num_samples"))
    this->set("num_samples", xmlpart.value("num_samples"));
