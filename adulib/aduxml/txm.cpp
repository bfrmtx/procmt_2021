...
bool TXM::is_active() {
  if (this->txm_cycle.value("BaseFrequency").toDouble() < 0.0000001)
    this->is_set = false;
  return this->is_set;
}
...
bool TXM::contains(const QString &what) {
  if (this->txm_cycle.contains(what))
    return true;
  if (this->txm_waveform.contains(what))
    return true;
  if (this->txm_sequence.contains(what))
    return true;
  if (this->txm_epos.contains(what))
    return true;
  return false;
}
