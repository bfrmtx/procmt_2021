...
adu_gui::adu_gui(QWidget *parent) : QMainWindow(parent), ui(new Ui::adu_gui) {
  ui->setupUi(this);
...
void adu_gui::clear_gui() {
  ...
  for (auto &item : this->dbl_spbxs)
    delete item;
  this->dbl_spbxs.clear();

  for (auto &item : this->dbl_spbxs_labels)
    delete item;
  this->dbl_spbxs_labels.clear();

  // reset all local data
  for (auto &item : this->spbxs)
    delete item;
  this->spbxs.clear();

  for (auto &item : this->spbxs_labels)
    delete item;
  this->spbxs_labels.clear();

  // reset all local data
  for (auto &item : this->cbboxs)
    delete item;
  this->cbboxs.clear();

  for (auto &item : this->cbboxs_labels)
    delete item;
  this->cbboxs_labels.clear();

  // reset all local data
  for (auto &item : this->lineetds)
    delete item;
  this->lineetds.clear();
  ...
  for (auto &item : this->rb_click_E)
    delete item;
  this->rb_click_E.clear();

  for (auto &item : this->rb_click_H)
    delete item;
...
QVariant adu_gui::get_label(const adu_defs::adu_defs varmap, const QString &key) {
  QString querystr;
  if (varmap == adu_defs::channel_map)
    querystr = adu_row.select_substr_channel() + " WHERE key = '" + key + "' LIMIT 1";
  if (varmap == adu_defs::system_map)
    querystr = adu_row.select_substr_system() + " WHERE key = '" + key + "' LIMIT 1";
...
void adu_gui::gen_str_results() {
  ...
  QStringList header;
  header << "key";
  if (!this->adu->get_selftest_channels(txt_values, out_keys, out_tooltips))
    return;
  if (!out_keys.size())
    return;

  if (txt_values.size() != this->adu->size())
    return;
  for (int i = 0; i < this->adu->size(); ++i) {
    if (txt_values.at(i).size() != out_keys.size())
      return;
  ...
  for (int i = 0; i < out_keys.size(); ++i) {
    ...
    for (int j = 0; j < this->adu->size(); ++j)
      table_SRT->setItem(i, j + 1, new QTableWidgetItem(txt_values.at(j).at(i)));
    table_SRT->setItem(i, this->adu->size() + 1, new QTableWidgetItem(out_tooltips.at(i)));
  }

  table_SRT->setAlternatingRowColors(true);
  for (int i = 0; i < table_SRT->rowCount(); ++i)
    table_SRT->resizeColumnToContents(i);
