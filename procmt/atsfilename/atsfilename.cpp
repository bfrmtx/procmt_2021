...
QString atsfilename::get_filename(const QString type) {
  if (is_init) {
    ...
    if (!QString::compare("07", type)) {
      name = this->svalue_rformat("fname_aduser", 3, '0') + "_V" + this->svalue_rformat("fname_version", 2, '0') + "_C" + this->svalue_rformat("fname_channel", 2, '0') + "_R" + this->svalue_rformat("fname_run", 3, '0') + "_T" + this->svalue("fname_type") +
    ...
    if (!QString::compare("06", type)) {
      name = this->svalue_rformat("fname_aduser", 3, '0') + this->svalue("fname_06_channel") + this->svalue_rformat("fname_run", 2, '0') + this->svalue("fname_06type") + this->svalue("fname_band06") + "." + suffix;
    }
    return name;
  }
...
void atsfilename::slot_set_filename_for_measdoc_from_ats(const QString &filename, const QString &start_datetime, const QString &stop_datetime, const QString &absolute_measdir) {
  if ((this->start_datetime == start_datetime) && (this->stop_datetime == stop_datetime) && (this->atsfile.fileName() == filename) && (this->absolute_measdir == absolute_measdir))
    return;
  this->clear_fname();
