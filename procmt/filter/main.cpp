...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2) {
    ...
    //   w.set_xmlfiles(qxml);
    // }
    QList<QFileInfo> qxml = {QFileInfo(a.arguments().at(1))};
    // if qxml is a directory, get all xml files in it
    if (qxml.at(0).isDir()) {
      QDir dir(qxml.at(0).absoluteFilePath());
      QStringList filters;
      filters << "*.xml";
      dir.setNameFilters(filters);
      dir.setFilter(QDir::Files | QDir::NoSymLinks);
      qxml = dir.entryInfoList();
    }
    w.set_xmlfiles(qxml);
