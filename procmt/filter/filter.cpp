...
filter::filter(QWidget *parent) :
  ...
  this->ui->dateTimeEdit_start->setTimeZone(QTimeZone::utc());
  this->ui->dateTimeEdit_stop->setTimeZone(QTimeZone::utc());
...
void filter::open() {
  if (this->qdirs.size() || this->qxml.size())
    this->qats.clear();
...
void filter::set_decimation_filter(const QString &str_x) {
  ...
  if (str_x == "4x")
    ...
    this->ui->rb_8x->setChecked(true);
  else if (str_x == "10x")
    this->ui->rb_10x->setChecked(true);
  else if (str_x == "15x")
    this->ui->rb_15x->setChecked(true);
  else if (str_x == "25x")
    this->ui->rb_25x->setChecked(true);
  else if (str_x == "32x")
    this->ui->rb_32x->setChecked(true);
}
...
void filter::set_ascii_output(const QString &str_asc) {
  ...
  if (str_asc == "raw")
    this->ui->rb_to_ascii->setChecked(true);
  else if (str_asc == "scaled")
    this->ui->rb_to_ascii_scaled->setChecked(true);
  else if (str_asc == "nt")
    this->ui->rb_to_ascii_scaled_nt->setChecked(true);
  else if (str_asc == "nt_theo")
    this->ui->rb_to_ascii_scaled_nt_theo->setChecked(true);
}
...
void filter::on_runbuton_clicked() {
  ...
  if (this->ui->rb_low_pass->isChecked() && (this->extrafilters_status != nullptr)) {
    ...
    cmdline->insert("lower_freq", double(0.0)); // filter tuning
    cmdline->insert("ats_scale", "none");       // filter does not want to scale
  } else if (this->ui->rb_16x->isChecked()) {
    cmdline->insert("ats", "ts");               // wants ats timeseries
    cmdline->insert("task", "filter");          // want to filter
...
void filter::slot_to_ascii_ts() {
  if ((this->cmdline->svalue("task") != "convert") && (cmdline->svalue("ascii") != "ts"))
    return;
  ...
  for (i = 0; i < this->qfis.size(); ++i) {
    ...
    if (this->ui->rb_to_ascii_scaled_nt_theo->isChecked())
      use_theo_only = true;
    // if (mcd->in_atsfiles.at(0)->get_sample_freq() > 512.01) use_theo_only = false;
    if (use_theo_only == true)
      cmdline->insert("calibration", "theo");
    ...
    if (this->ui->checkBox_have32GB->isChecked())
      mcd->slot_set_GB_mem(32);
...
void filter::slot_gui_fir_filter() {
  ...
  if (this->ui->checkBox_have_copy->isChecked() && this->ui->rb_remap->isChecked()) {
    ...
    for (auto &xmls : this->qxml) {
      ...
      if (this->in_channels_labels.size() != mcd->in_atsfiles.size())
        return;
      ...
      if (expected_size != mapfiles.size())
        return;
...
void filter::slot_single_sub_fir_notch_filter() {
  if (cmdline->svalue("filter_name") != "notch")
    return;
  if (!this->qfisxmlats_sub.size())
    return;
...
void filter::slot_single_sub_fir_filter() {
  if (!cmdline->svalue("filter_name").startsWith("mtx"))
    return;
  if (!this->qfisxmlats_sub.size())
    return;
  if (!this->ui->spinBoxCascades->value() && !this->ui->cb_auto_extend->isChecked())
    return;
  if (!this->remaining_cascades && !this->ui->cb_auto_extend->isChecked())
    return;
  ...
  if (this->qfisats_sub.size() && this->ui->cb_auto_extend->isChecked()) {
    ...
    if (dbmcd.open()) {
      ...
      if (query.exec(query_str)) {
        ...
        qDebug() << "slot_single_sub_fir_filter -> db err: could not get choppers for filtering";
    ...
    if (this->ui->rb_2x->isChecked())
      fct = 2.0;
    if (this->ui->rb_8x->isChecked())
      fct = 8.0;
    if (this->ui->rb_10x->isChecked())
      fct = 10.0;
    if (this->ui->rb_15x->isChecked())
      fct = 15.0;
    if (this->ui->rb_25x->isChecked())
      fct = 25.0;
    if (this->ui->rb_32x->isChecked())
      fct = 32.0;
    ...
    if ((dsamples / xwl) > min_stacks)
      this->remaining_cascades = 1;
    else {
      this->remaining_cascades = 0;
      return;
    }
...
void filter::delete_files() {
  for (auto &q : this->delete_xmlfiles)
    qDebug() << q.absoluteFilePath();
  for (auto &q : this->delete_atsfiles)
    qDebug() << q.absoluteFilePath();
  ...
  for (auto &q : this->delete_xmlfiles)
    strdirs.append(q.dir().absolutePath());
  for (auto &q : this->delete_atsfiles)
    strdirs.append(q.dir().absolutePath());
  ...
  for (auto &q : strdirs)
    delete_dirs.append(QDir(q));

  for (auto &q : delete_dirs)
    qDebug() << q.absolutePath();
