...
emerald::emerald(const QFileInfo &qfi, const int &skip_bytes, QObject *parent)  :  QObject(parent), QFileInfo(qfi), prc_com(false)
{
...
*/
    ...
    if (close_again) this->file.close();
...
size_t emerald::to_textfile(const QFileInfo &name_qfi)
{
    ...
    do {
        ...
        out << lines << " ";
        for (j = 0; j < data.size(); ++j) out << data[j] << " ";
        out << Qt::endl;
...
bool emerald::set_survey_basedir(const QDir &basedir, const QString &site_str, const bool create)
{
    ...
    if (basedir.exists()) {
        for (auto &str : pmt::survey_dirs) {
            ...
            if (dirs.exists()) qDebug() << "exists";
            if (create) {
                if (!dirs.exists()) {
                    if (dirs.mkpath(dirs.absolutePath())) qDebug() << "created";
                    else qDebug() << "creating" << str << "failed!";

                }
                if  (str == "ts") {
                    this->sdir.setPath(basedir.absolutePath() + "/" + str + "/" + site_str);
                    if (this->sdir.exists()) qDebug() << "exists";
                    else if (this->sdir.mkpath(sdir.absolutePath())) qDebug() << "created";
                    else qDebug() << "creating" << this->sdir.absolutePath() << "failed!";
...
bool emerald::dip_to_pos(const double length, const double decangle)
{
    ...
    if  (fabs(this->x1) <  assume_zero_length) this->x1 = 0.;
    if  (fabs(this->x2) <  assume_zero_length) this->x2 = 0.;
    if  (fabs(this->y1) <  assume_zero_length) this->y1 = 0.;
    if  (fabs(this->y2) <  assume_zero_length) this->y2 = 0.;
    if  (fabs(this->z1) <  assume_zero_length) this->z1 = 0.;
    if  (fabs(this->z2) <  assume_zero_length) this->z2 = 0.;
...
{
    if (this->data_in.size() != xdata_in.size()) return false;
    if (this->channel_map.size() != xchannel_map.size()) return false;
    else {
        QList<QString> mykeys = this->channel_map.keys();
        QList<QString> your_keys = xchannel_map.keys();
        int j = 0;
        for (int i = 0; i < this->channel_map.size(); ++i ) {
            if (mykeys.at(i).compare(your_keys.at(i))) ++j;
        }
        if (j) return false;
    }
...
void emerald::write_ats()
{
    ...
    for (auto &a : atsfiles) {
        ...
        double lsb;
        if (std::abs(min) > std::abs(max)) lsb = std::abs(min) * 6 /  ((double)INT32_MAX);
        else lsb = std::abs(max) * 6 /  ((double)INT32_MAX);
...
{
    for (size_t i = 0; i < data_in.size(); ++i) data_in[i].clear();
    data_in.clear();
}
...
void emerald::slot_convert(const quint64 new_guessed_samples)
{
    ...
    for (auto &chan : this->emerald_channels) {
        ...
        if (chan_name.contains("Bx", Qt::CaseInsensitive)) { chan_name = "Hx"; i = 2; this->channel_map.insert(chan_name, i);}
        if (chan_name.contains("BY", Qt::CaseInsensitive)) { chan_name = "Hy"; i = 3; this->channel_map.insert(chan_name, i);}
        if (chan_name.contains("Bz", Qt::CaseInsensitive)) { chan_name = "Hz"; i = 4; this->channel_map.insert(chan_name, i);}
        if (chan_name.contains("Ex", Qt::CaseInsensitive)) { chan_name = "Ex"; i = 0; this->channel_map.insert(chan_name, i);}
        if (chan_name.contains("Ey", Qt::CaseInsensitive)) { chan_name = "Ey"; i = 1; this->channel_map.insert(chan_name, i);}

        QString board("LF");
        if (this->emerald_site.value("SampleRate").toDouble() > 4096) board = "HF";
        ...
        QFileInfo qfi;
        if (this->sdir.exists()) qfi.setFile(this->sdir, name.fileName());
        else qfi.setFile(name.fileName());
    ...
    for (auto &a : atsfiles) {
        ...
        if (this->dip_to_pos(length, angle)) {
            ...
            if (coilserial) a->set_key_value("sensor_sernum", coilserial);
    ...
    for (auto &a : this->atsfiles) {
        ...
        if(i < this->atsfiles.size()) channelchk.append(", ");
    ...
    if (new_guessed_samples) for (auto &v : this->data_in) v.reserve(new_guessed_samples);
    else for (auto &v : this->data_in) v.reserve(guessed_samples);
    float f;
    do {
...
void emerald_header::buffer_to_map(const QString &buffer)
{
    ...
    if (data.size() >= items.size()) {
        for (auto &str : items) {
            ...
            if (!str.compare("f0") || !str.compare("f1") || !str.compare("f2") ) header.insert(str,std::fabs(data.at(i++).toDouble()));

            else if (!str.compare("start") || !str.compare("stop") || !str.compare("startUs")  || !str.compare("stopUs") ) header.insert(str, ((qint64)(data.at(i++).toLongLong())));
            else if (!str.compare("recLen") || !str.compare("gh_totalRecs") || !str.compare("firstEvent")  || !str.compare("totalEvents") ) header.insert(str, ((qint64)(data.at(i++).toLongLong())));
            else if (!str.compare("nextEvent") || !str.compare("prevEvent") || !str.compare("ev_totalRecs")  || !str.compare("dataStartRec") ) header.insert(str, ((qint64)(data.at(i++).toLongLong())));

            else header.insert(str, (QString)data.at(i++));
        }
    }
