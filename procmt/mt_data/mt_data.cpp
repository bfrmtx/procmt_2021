        ...
        if (cplxv2.size() && (k >= i)) {
          ...
          if (j < pmt::spce::spce_size) {
            ...
            if (this->ac[j].size() != cplxv1.size()) {
              ...
              for (size_t sj = 0; sj < g_stat::statmap_size; ++sj)
                this->ac_vals[j][sj] = 0.0;
              this->ac_ampl_vals[j].reserve(g_stat::statmap_size);
              for (size_t sj = 0; sj < g_stat::statmap_size; ++sj)
                this->ac_ampl_vals[j][sj] = 0.0;
            }
          }
        }
        ++k;
...
void acsp::normalize_ssp(const size_t spce_numerator, const size_t spce_denominator) {
  if (this->ssp[spce_numerator].size() != this->ssp[spce_denominator].size())
    return;
...
void acsp::reset_cross_remote_residuals() {
  if (this->ac[pmt::hxrhx].size()) {
    for (auto &c : this->ac[pmt::hxrhx])
      c = std::complex<double>(c.real(), 0.0);
  }
  if (this->ac[pmt::hyrhy].size()) {
    for (auto &c : this->ac[pmt::hyrhy])
      c = std::complex<double>(c.real(), 0.0);
  }
}
...
void acsp::reset_auto_residuals() {
  if (this->ac[pmt::exex].size()) {
    for (auto &c : this->ac[pmt::exex])
      c = std::complex<double>(c.real(), 0.0);
  }
  if (this->ac[pmt::eyey].size()) {
    for (auto &c : this->ac[pmt::eyey])
      c = std::complex<double>(c.real(), 0.0);
  }
  if (this->ac[pmt::hxhx].size()) {
    for (auto &c : this->ac[pmt::hxhx])
      c = std::complex<double>(c.real(), 0.0);
  }
  if (this->ac[pmt::hyhy].size()) {
    for (auto &c : this->ac[pmt::hyhy])
      c = std::complex<double>(c.real(), 0.0);
  }
  if (this->ac[pmt::hzhz].size()) {
    for (auto &c : this->ac[pmt::hzhz])
      c = std::complex<double>(c.real(), 0.0);
  }
}
      ...
      if (aspcdat.data.size()) {        // ex has size ... rhz has size ?
        if (!aspcdat.data[f].empty()) { // does ex has 512 hz ... yes! this is extra security
          ...
          if (this->ac_vals.size() != g_stat::statmap_size) {
            ...
            for (size_t j = 0; j < g_stat::statmap_size; ++j)
              this->ac_vals[i][j] = 0.0;
          }
        }
      }
    ...
    if (cplxv.size()) {
      ...
      if (j < pmt::spce_size) {
        ...
        for (const auto &cplx : cplxv) {
          ...
          if (this->ac_sel[j][k])
            this->ac[j][k++] = std::real(cplx * std::conj(cplx));
        }
      }
    }
    ++i;
        ...
        if (rhs_spc->size()) {
          ...
          if (k < pmt::spce_size) {
            ...
            if (this->ac[k].size() != lhs_spc->size()) {
              ...
              for (size_t sj = 0; sj < g_stat::statmap_size; ++sj)
                this->ac_vals[k][sj] = 0.0;
              this->ac_ampl_vals[k].reserve(g_stat::statmap_size);
              for (size_t sj = 0; sj < g_stat::statmap_size; ++sj)
                this->ac_ampl_vals[k][sj] = 0.0;
            }
            ...
            for (size_t p = 0; p < lhs_spc->size(); ++p) {
              ...
              if (this->ac_sel[k][p]) {
                ...
                if (i == j)
                  this->ac[k][p] = std::real(*lhs_iter * std::conj(*rhs_iter));
                else
                  this->ac[k][p] = (*lhs_iter * std::conj(*rhs_iter));
              }
              ++lhs_iter;
              ++rhs_iter;
            }
...
std::vector<double> acsp::ssp_spc_if(const bool real_false_imag_true, const int first_1_second_2_all_3, const size_t chan_type) const {
  ...
  if (chan_type < pmt::chan_type_size) {
    ...
    if (!real_false_imag_true) {
      do {
        if (*isit++)
          vals[i++] = std::real(*starts++);
      } while (starts != stops);
    } else {
      do {
        if (*isit++)
          vals[i++] = std::imag(*starts++);
      } while (starts != stops);
    }
  }
...
int acsp::has_which_spectra() const {
  ...
  if (!acspsz && !sspsz)
    return 0; // has no spectra
  if (acspsz && sspsz)
    return 3; // has both spectra
  if (acspsz && !sspsz)
    return 1; // has ac (auto- cross) spectra only
  if (!acspsz && sspsz)
    return 2; // has single complex spectra only
