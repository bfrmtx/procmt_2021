...
mt_gensign::mt_gensign(QWidget *parent) :
  ...
  this->ui->start_datetime->setTimeZone(QTimeZone::utc());
  this->ui->stop_datetime->setTimeZone(QTimeZone::utc());
...
void mt_gensign::create_unique_seed(bool createme) {
  ...
  if (createme) {
    ...
    for (auto &gen : this->generators) {
      if (gen->unique_noise_buffer_1 != nullptr)
        gen->unique_noise_buffer_1.reset();
      if (gen->unique_noise_buffer_2 != nullptr)
        gen->unique_noise_buffer_2.reset();
    }
...
void mt_gensign::clear() {
  ...
  for (auto &gener : this->generators) {
    if (gener != nullptr)
      gener->buffer.reset();
    if (gener != nullptr)
      gener->unique_noise_buffer_1.reset();
    if (gener != nullptr)
      gener->unique_noise_buffer_2.reset();
  }
...
generator::generator(const size_t &slices, const size_t &slice, mt_gensign *master) :
    slices(slices) {
  this->v.resize(slice);
  this->master = master;
...
generator::~generator() {
  if (this->buffer != nullptr)
    this->buffer.reset();
  if (this->unique_noise_buffer_1 != nullptr)
    this->unique_noise_buffer_1.reset();
  if (this->unique_noise_buffer_2 != nullptr)
    this->unique_noise_buffer_2.reset();
...
void generator::linear() {
  ...
  for (i = 0; i < (this->slices * this->v.size()); ++i) {
    ...
    if (j == (this->v.size() - 1)) {
      if (i < ((this->slices * this->v.size()) - 1))
        buffer->deposit(v, threadbuffer_status::running);
      else {
...
void mt_gensign::on_fsine_f1_valueChanged(double arg1) {
  if ((this->ui->checkBox_use_seconds->checkState() == Qt::Checked) && (arg1 != 0.0))
    this->fsine_f[0] = 1.0 / arg1;
  if ((this->ui->checkBox_use_seconds->checkState() == Qt::Checked) && (arg1 == 0.0))
    this->fsine_f[0] = 0.0;
  else
    this->fsine_f[0] = arg1;
...
void mt_gensign::on_checkBox_use_seconds_stateChanged(int arg1) {
  if (arg1 == 0) {
    ...
    if (this->fsine_f[0] != 0.0)
      this->fsine_f[0] = 1.0 / fsine_f[0];
    if (this->fsine_f[1] != 0.0)
      this->fsine_f[1] = 1.0 / fsine_f[1];
    if (this->fsine_f[2] != 0.0)
      this->fsine_f[2] = 1.0 / fsine_f[2];
    if (this->fsine_f[3] != 0.0)
      this->fsine_f[3] = 1.0 / fsine_f[3];
