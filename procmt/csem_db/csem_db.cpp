  ...
  if (this->csemdb.open()) {
    ...
    for (auto &str : this->tables) {
      ...
      for (auto &colstr : this->columns) {
        if (i == 0)
          qstr += " `" + colstr + "` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE";
        else
          qstr += " `" + colstr + "` TEXT";
        if (i < icols - 1)
          qstr += " ,";
...
void csem_db::close() {
  this->csemdb.close();
  for (auto &dip : this->dip_1)
    dip.clear();
  for (auto &dip : this->dip_2)
    dip.clear();
}
...
bool csem_db::db_has_data_keys(const QMap<QString, QVariant> &data) {
  ...
  for (kystr = data.cbegin(); kystr != data.cend(); ++kystr) {
    if (this->columns.contains(kystr.key()))
      ++count;
  }
