...
atsfile::atsfile(const QFileInfo &qfi, const short &header_version, QObject *parent) :
    atsheader(qfi, header_version, parent) {
}
atsfile::atsfile(const QFileInfo &qfi, const size_t &slot, const short &header_version, QObject *parent) :
    atsheader(qfi, header_version, parent) {
  this->slot = slot;
}
atsfile::atsfile(QObject *parent) :
    atsheader(parent) {
  this->clear(0);
}

atsfile::atsfile(const QString &dir, const QString &name, const short &header_version, QObject *parent) :
    atsheader(dir, name, header_version, parent) {
}

atsfile::atsfile(const atsfile &atsf) :
    atsheader(atsf) {
  this->slot = atsf.slot;
}
...
void atsfile::reset_buffer() {
  if (this->tsbuffer != nullptr)
    this->tsbuffer.reset();
  if (this->spcbuffer != nullptr)
    this->spcbuffer.reset();
  if (this->spc_amplbuffer != nullptr)
    this->spc_amplbuffer.reset();
...
std::shared_ptr<threadbuffer<double>> atsfile::get_create_buffer(const size_t &nbuffers, const atsfileout &buffer_type) {
  ...
  if (buffer_type == atsfileout::atsfileout_unscaled_timeseries)
    this->scale_me = false;
  if (buffer_type == atsfileout::atsfileout_unscaled_fir_filtered)
    this->scale_me = false;
...
bool atsfile::csvline(const QString &line_in) {
  ...
  QString remove;
  size_t i = 0;
  if (!QString::compare(this->channel_type(), "Ex"))
    remove = "\"E1\": [";
  else if (!QString::compare(this->channel_type(), "Ey"))
    remove = "\"E2\": [";
  else if (!QString::compare(this->channel_type(), "Hx"))
    remove = "\"H1\": [";
  else if (!QString::compare(this->channel_type(), "Hy"))
    remove = "\"H2\": [";
  else if (!QString::compare(this->channel_type(), "Hz"))
    remove = "\"H3\": [";
...
bool atsfile::set_window_length(const size_t &wl, const double &fcut_upper, const double &fcut_lower, const bool &last_read, const quint64 &use_samples, const quint64 &overlapping, const atsfileout &buffer_type) {
  ...
  if (buffer_type != atsfileout::atsfileout_void)
    this->buffer_type = buffer_type;
  ...
  if (this->buffer_type != atsfileout::atsfileout_unscaled_fir_filtered) {
    ...
    if (overlapping < wl)
      this->move_back_after_read = overlapping;
  }

  quint64 samples_to_use = use_samples;
  if (this->override_use_samples)
    samples_to_use = this->override_use_samples;
  ...
  ldiv_t divresult;
  if (!samples_to_use)
    divresult = ldiv((this->sztvalue("num_samples") - this->tsdata.size()), (this->tsdata.size() - this->move_back_after_read));
  else
    divresult = ldiv((samples_to_use - this->tsdata.size()), ((this->tsdata.size() - this->move_back_after_read)));
  this->max_counts = size_t(divresult.quot + 1);
  if (last_read)
    this->last_read_size = size_t(divresult.rem);
  else {
    this->last_read_size = 0;
  }
  if (this->buffer_type == atsfileout::atsfileout_unscaled_fir_filtered)
    last_read_size = 0; // dont use for fir filter
...
std::vector<double> atsfile::get_frequencies() const {
  if (this->fftreal != nullptr)
    return this->fftreal->get_frequencies();
...
QDataStream::Status atsfile::get_double_samples(std::vector<double> &v, bool scaleme) {
  ...
  if (this->scale_me && this->is_scalable)
...
QDataStream::Status atsfile::read() {
  ...
  if (this->can_read) {
    ...
    if (this->bin_atsheader.bit_indicator == 1)
      move_size = sizeof(int64_t);
...
void atsfile::read_tsplotter(const bool emits, const bool stack_all) {
  ...
  if ((this->tsplotter_buffer_type_ts > atsfileout::atsfileout_unscaled_timeseries) && this->is_scalable)
    factor *= (1000.0 / this->dipole_length);
  ...
  if (this->qds.status() != QDataStream::Ok)
    return;
  ...
  if (this->tsplotter_buffer_type_ts != atsfileout_calibrated_scaled_timeseries) {
    ...
    if (emits)
      emit this->signal_tsdata(this->tsdata, this->slot);
    if (stack_all)
      std::transform(this->tsdata_stacked.begin(), this->tsdata_stacked.end(), this->tsdata.begin(), this->tsdata_stacked.begin(), std::plus<double>());
  }
  ...
  if ((this->calib != nullptr) && this->calib->sensortype.contains("FGS", Qt::CaseInsensitive) && !this->is_scalable) {
    if (this->calib->sensortype.contains("03") || this->calib->sensortype.contains("04")) {
      for (auto &val : this->tsdata)
        val *= 10;
    } else if (this->calib->sensortype.contains("05")) {
      for (auto &val : this->tsdata)
        val *= 7;
    } else if (this->calib->sensortype.contains("02")) {
      for (auto &val : this->tsdata)
        val *= 1.33333333;
    }
  ...
  // that needs a full rectangular FFT without hanning for H channels
  else if ((this->tsplotter_buffer_type_ts == atsfileout_calibrated_scaled_timeseries) && (this->calib != nullptr) && !this->is_scalable) {
    ...
    for (auto &all : mytsdata)
      all *= -1.0;
    if (emits)
      emit this->signal_tsdata(mytsdata, this->slot);
    if (stack_all)
      std::transform(this->tsdata_stacked.begin(), this->tsdata_stacked.end(), mytsdata.begin(), this->tsdata_stacked.begin(), std::plus<double>());
  }
  ...
  if (emits)
    emit this->signal_cplx_spcdata(this->spcdata, this->slot);
  ...
  if (plf != (this->plotter_frequencies->end()))
    return;
  ...
  if (emits)
    emit this->signal_spcdata(this->spc_ampl_data, this->slot);
  if (stack_all) {
...
#ifdef QT_DEBUG
    std::cout << std::endl;
    for (size_t g = 10; g < 20; ++g)
      std::cout << this->spc_ampl_data_stacked[g] << " ";
    std::cout << std::endl;
#endif
  }
}
...
void atsfile::read_all_tsplotter() {
  ...
  for (auto &d : this->tsdata_stacked)
    d /= counts;

  // emit this->signal_tsdata(this->tsdata_stacked, this->slot);

  if (this->tsplotter_buffer_type_spc != atsfileout::atsfileout_void) {
    for (auto &d : this->spc_ampl_data_stacked)
      d /= counts;
...

{
  ...
  if (this->calib != nullptr)
    this->calib->eliminate_except_complex(); // don't need the rest here ... I think so

  // test this->emit_ats_thrd_tsdata_ready = false;

  if (this->buffer_type == atsfileout::atsfileout_calibrated_spectra)
    this->last_read_size = 0;
  ...
  while (this->ats_read_counts < this->max_counts) {
    ...
    // stack all ts  - not spectra
      ...
      for (auto &v : this->tsdata_stacked)
        v /= double(this->ats_read_counts);
      this->tsbuffer_status = threadbuffer_status::finished;
      ...
      for (auto &v : this->spc_ampl_data_stacked)
        v /= double(this->ats_read_counts);
      this->tsbuffer_status = threadbuffer_status::finished;
...
void atsfile::read_all_single(const bool inverse_fft, const bool write_immedeately) {
  if (this->can_read) {
    ...
    if (!this->is_scalable && inverse_fft && !fgs) {
...
void atsfile::set_ats_options(const prc_com &cmdline) {
  ...
  if (!this->options.svalue("scale").compare("auto"))
    this->scale_me = true;
  else
    this->scale_me = false;
}
...
size_t atsfile::queue_data_to_vector() {
  if (!this->queue_data.size())
    return 0;
  if (!this->samples_read)
    return 0;
  ...
  double lsb_mv;
  if (max > 0)
    lsb_mv = 2 * std::abs(max) / (INT_MAX);
  else
    lsb_mv = 2 * abs(min) / (INT_MAX);
...
quint64 atsfile::read_ascii_file(const QFileInfo &ascii_file, const bool isVolt, const bool is_64bit) {
  ...
  if (!is_64bit) {
    if (std::abs(min) > std::abs(max))
      this->bin_atsheader.dblLSBMV = std::abs(min) * 6 / ((double)INT32_MAX);
    else
      this->bin_atsheader.dblLSBMV = std::abs(max) * 6 / ((double)INT32_MAX);
    for (auto &d : data_in) {
    ...
    if (std::abs(min) > std::abs(max))
      this->bin_atsheader.dblLSBMV = std::abs(min) * 6 / ((double)INT64_MAX);
    else
      this->bin_atsheader.dblLSBMV = std::abs(max) * 6 / ((double)INT64_MAX);
    for (auto &d : data_in) {
...
quint64 atsfile::dump_spc_tsplotter(const QString &filename) const {
  if (!this->spc_ampl_data.size())
    return 0;
  if (this->plotter_frequencies == nullptr)
    return 0;
  if (!this->plotter_frequencies->size())
    return 0;
  if (this->spc_ampl_data.size() != this->plotter_frequencies->size())
    return 0;
  QFile data(filename);
  bool opensuccess = false;
...
quint64 atsfile::dump_spc_tsplotter_stacked(const QString &filename) const {
  if (!this->spc_ampl_data_stacked.size())
    return 0;
  if (this->plotter_frequencies == nullptr)
    return 0;
  if (!this->plotter_frequencies->size())
    return 0;
  if (this->spc_ampl_data_stacked.size() != this->plotter_frequencies->size())
    return 0;
  QFile data(filename);
  bool opensuccess = false;
...
QDataStream::Status atsfile::slot_slide_to_time(const eQDateTime &edt_time, eQDateTime *corrected_time) {
  ...
  if (slide_samples)
    return this->slot_slide_to_sample(slide_samples);
  else
    return this->qds.status();
}
...
QDataStream::Status atsfile::sync_for_max_min_time(const eQDateTime &edt_time_starts_other, const eQDateTime &edt_time_stops_other) {
  ...
  emit this->tx_adu_msg(ivalue("system_serial_number"), ivalue("channel_number"), this->slot,
                        "atsfile::sync_for_max_min_time ->  " + this->edt_start.ISO_date_time(edt_start.get_start_sample()) + "  <->  " + this->edt_start.ISO_date_time(edt_start.get_start_sample() + this->edt_start.get_use_samples()) + " use samples: " + QString::number(this->edt_start.get_use_samples()));
...
size_t atsfile::concat(std::shared_ptr<atsfile> &rhs) {
  ...
  if (this->edt_stop > rhs->edt_start)
    return 0;
