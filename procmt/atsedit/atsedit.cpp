...
atsedit::atsedit(const QString &dbname, QWidget *parent) :
  ...
  this->excludes << "sample_freq" << "num_samples" << "start_date" << "start_time" << "start_date_time" << "dipole_length" << "dipole_angle"
                 << "stop_date" << "stop_time" << "run_number"
                 << "latitude" << "longitude" << "elevation" << "UTMZone"
                                                                "GaussRefMeridian";
...
atsedit::~atsedit() {
  if (this->atsh != nullptr)
    this->atsh.reset();
  if (this->site_table != nullptr)
    this->site_table.reset();
...
void atsedit::open_urls(const QList<QUrl> urls, const QString otherwise_absolute_path) {
  ...
  if (this->sens_pos_ser != nullptr)
    this->sens_pos_ser.reset();
  ...
  if (this->system_table != nullptr)
    this->system_table.reset();
  ...
  if (this->atsh != nullptr)
    this->setWindowTitle(this->atsh->baseName());
...
void atsedit::update_coord_gui() {
  ...
  if (0 == this->LatH.compare("N"))
    ui->comboBox_NS->setCurrentIndex(0);
  else
    ui->comboBox_NS->setCurrentIndex(1);
  if (0 == this->LonH.compare("E"))
    ui->comboBox_EW->setCurrentIndex(0);
  else
    ui->comboBox_EW->setCurrentIndex(1);
...
calcfilesize::calcfilesize(const QString &dbname, QComboBox *box, atsedit *gui) :
    QObject(gui), box(box) {
...
quint64 calcfilesize::calc_filesizes() {
  if (this->gui->ui->checkBox_CEA->isChecked())
    this->headersize_used = this->headersize_cea;
  else
    this->headersize_used = this->headersize;
...
void calcfilesize::start_time_changed(const QDateTime &startime) {
  ...
  if (this->starts > this->stops)
    ...
    this->samplesecs = (this->stops - this->starts) / 1000;
  this->calc_filesizes();
}
