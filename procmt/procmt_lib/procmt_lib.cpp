...

{
  ...
  for (auto &qfixml : this->qfis_center) {
    ...
    if ((my_mttype == mttype::emap) || (my_mttype == mttype::emap_rr))
      qfiXML.append(this->qfis_emap.at(this->iqfis_emap++));
    qfiXML.append(qfixml);
    if ((my_mttype == mttype::rr) || (my_mttype == mttype::emap_rr))
      qfiXML.append(this->qfis_rr.at(this->iqfis_rr++));
    ...
    if (mxcd->open_files(qfiXML, true, my_mttype, threadctrl::threadctrl_continuous_join)) {
      ...
      if (!cmdline->contains("parzen_radius"))
        cmdline->insert_double("parzen_radius", 0);
      if (!cmdline->contains("min_lines"))
            ...
            for (auto &prc : prcs) {
              for (auto &inprc : mxcd->in_ats_prc_coms) {
                if (prc.svalue("channel_type") == inprc.svalue("channel_type")) {
                  if (prc.qint64value("start_date_time") > inprc.qint64value("start_date_time"))
                    prc.set_key_value("start_date_time", inprc.qint64value("start_date_time"));
                  if (prc.qint64value("stop_date_time") < inprc.qint64value("stop_date_time"))
                    prc.set_key_value("stop_date_time", inprc.qint64value("stop_date_time"));
                }
              }
            }
            for (auto &prc : mxcd->in_ats_prc_coms) {
      ...
      // try else mt
        ...
        if (cmdline->sztvalue("min_lines") == 0)
          mxcd->make_auto_cross_spectra_parzen();
    ...
    for (auto &prc : prcs) {
      ...
      for (auto &str : ag) {
        ...
        if (x.size() > 1) {
          ...
          if (str == "_lat")
            prc.set_key_value("latitude", median);
          if (str == "_lon")
            prc.set_key_value("longitude", median);
          if (str == "_elev")
            prc.set_key_value("elevation", median);
        }
      }
    }
  }
  ...
  if (cmdline->sztvalue("min_lines") >= 1)
    ac1_ssp2_acssp3 = 2; // line spectra
  if (this->my_mttype == mttype::csem && this->myproc_type == proc_type::csem_rxtx)
    ac1_ssp2_acssp3 = 2; // single line spectra for CSEM
