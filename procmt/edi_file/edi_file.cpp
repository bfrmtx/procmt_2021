...
edi_file::edi_file(QObject *parent) :
    QObject(parent), QFileInfo(), prc_com(false) {
  // >HEAD
  ...
  this->edi_HEAD.insert("STDVERS", "1.0");
  this->edi_HEAD.insert("PROGVERS", "2021-08-24");
  this->edi_HEAD.insert("PROGDATE", "2023-08-24");
...
edi_file::edi_file(const edi_file &edi) :
    QObject(edi.parent()), prc_com(false) {
  this->operator=(edi);
  // qDebug() << "header copy";
}
...
bool edi_file::test_full_tensor() { return this->z.test_full_tensor(); }

bool edi_file::test_tipper() const { return this->z.test_tipper(); }
...
void edi_file::set_QFileInfo(const QFileInfo &qfi) { this->setFile(qfi.absoluteFilePath()); }
...
bool edi_file::toASCII_file(const QFileInfo &qfi) const { return this->z.toASCII_file(qfi); }
...
bool edi_file::write_mtsect() {
  ...
  //    for (auto &af : z) {    // that is the f1, f2, f3 part - so af.first is
  //    the freq, af.second is the z element
  ...
  for (elm = 0; elm < this->z.d.size(); ++elm) {
    ...
    qts << Qt::endl
        << this->znames_real.at(elm) << z.freqs.size() << Qt::endl;
    i = 0;
    ...
    qts << Qt::endl
        << this->znames_imag.at(elm) << z.freqs.size() << Qt::endl;
    i = 0;
    for (auto &allf : this->z.d[elm]) {
    ...
    qts << Qt::endl
        << this->znames_var.at(elm) << z.freqs.size() << Qt::endl;
    i = 0;
    ...
    // I assume that the complete setup was rotated - according to the electrode
    // positions
  ...
  if (this->seek_key(">=DEFINEMEAS")) {
    ...
    do {
      ...
      if (this->line.startsWith(">EMEAS") || this->line.startsWith(">HMEAS")) {
        ...
        for (int i = 0; i < data.size() - 1; ++i) {
          ...
          if (!QString::compare(data.at(i), "X", Qt::CaseInsensitive))
            prc.insert_double("pos_x1", data.at(i + 1).toDouble());
          else if (!QString::compare(data.at(i), "Y", Qt::CaseInsensitive))
            prc.insert_double("pos_y1", data.at(i + 1).toDouble());
          else if (!QString::compare(data.at(i), "Z", Qt::CaseInsensitive))
            prc.insert_double("pos_z1", data.at(i + 1).toDouble());

          else if (!QString::compare(data.at(i), "X2", Qt::CaseInsensitive))
            prc.insert_double("pos_x2", data.at(i + 1).toDouble());
          else if (!QString::compare(data.at(i), "Y2", Qt::CaseInsensitive))
            prc.insert_double("pos_y2", data.at(i + 1).toDouble());
          else if (!QString::compare(data.at(i), "Z2", Qt::CaseInsensitive))
            prc.insert_double("pos_z2", data.at(i + 1).toDouble());

          else if (!QString::compare(data.at(i), "CHTYPE", Qt::CaseInsensitive)) {
            for (auto &str : pmt::channel_types) {
              if (!QString::compare(str, data.at(i + 1), Qt::CaseInsensitive))
                prc.insert("channel_type", str);
            }
            if (prc.svalue("channel_type") == "empty")
              prc.insert("channel_type", data.at(i + 1));
          }
    ...
    for (auto str : pmt::channel_types) {
      for (auto prc : in_ats_prc_coms_tmp) {
        if (prc.svalue("channel_type") == str)
          this->in_ats_prc_coms.push_back(prc);
      }
    }
    // don't know what to do
    for (auto str : pmt::channel_types) {
      for (auto prc : in_ats_prc_coms_tmp) {
        if (!pmt::channel_types.contains(str, Qt::CaseInsensitive))
          this->in_ats_prc_coms.push_back(prc);
      }
    }
...
bool edi_file::read_mtsect() {
  ...
  while (this->seek_key(">COH", true, true)) {
    ...
    this->qts.seek(pos); // first COH line was read, so we are in front of the
                         // number  block, seek back
  }
...
qint64 edi_file::read_a_coh_section() {
  ...
  if (data.contains("MEAS1", Qt::CaseInsensitive) && data.contains("MEAS2", Qt::CaseInsensitive)) {
    ...
    if ((vone != vtwo)) {
      ...
      if (((in_ats_prc_coms.at(vone).svalue("channel_type") == "Ex") && (in_ats_prc_coms.at(vtwo).svalue("channel_type") == "Hy")) || ((in_ats_prc_coms.at(vone).svalue("channel_type") == "Hy") && (in_ats_prc_coms.at(vtwo).svalue("channel_type") == "Ex"))) {
        if (this->read_number_block(tmpv)) {
          this->z.coh[xy] = tmpv;
        }
      }

      if (((in_ats_prc_coms.at(vone).svalue("channel_type") == "Ey") && (in_ats_prc_coms.at(vtwo).svalue("channel_type") == "Hx")) || ((in_ats_prc_coms.at(vone).svalue("channel_type") == "Hx") && (in_ats_prc_coms.at(vtwo).svalue("channel_type") == "Ey"))) {
        if (this->read_number_block(tmpv)) {
          this->z.coh[yx] = tmpv;
        }
      }
...
bool edi_file::read_spectrasect() { return false; }
