...
size_t atmfile::free_atm_window(const size_t &sample_start, const size_t &window_length) {
  if ((sample_start == SIZE_MAX) || (window_length == SIZE_MAX))
    return SIZE_MAX;
  if (sample_start + window_length > b.size())
    return SIZE_MAX;
  ...
  if ((sample_start + j) < window_length)
    return SIZE_MAX; // could not find a free window anymore
