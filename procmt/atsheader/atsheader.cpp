...
atsheader::atsheader(const QFileInfo &qfi, const short &header_version, QObject *parent) :
...
atsheader::atsheader(const QString &dir, const QString &name, const short &header_version, QObject *parent) :
    QObject(parent), prc_com(false) {
  this->clear(header_version);
  qds.setByteOrder(QDataStream::LittleEndian);
  if (dir.size() && dir.endsWith("/"))
    this->setFile(dir.trimmed() + name.trimmed());
  else if (dir.size() && dir.endsWith("\\"))
    this->setFile(dir.trimmed() + "/" + name.trimmed());
  else
    this->setFile(dir.trimmed() + "/" + name.trimmed());
...
atsheader::atsheader(const QFileInfo &qfi, const QByteArray &qba, const bool &scope_mode, QObject *parent) :
    QObject(parent), QFileInfo(qfi), prc_com(false) {
  if (qba.size() > 1024)
    return;
  this->clear(80);
...
atsheader::atsheader(const atsheader &atsh) :
    QObject(Q_NULLPTR), QFileInfo(atsh.absoluteFilePath()), prc_com(false) {
  this->operator=(atsh);
  qDebug() << "header copy";
}
...
bool atsheader::dip_to_pos(const double length, const double decangle) {
  ...
  if (fabs(dvalue("pos_x1")) < assume_zero_length)
    insert_double("pos_x1", 0.);
  if (fabs(dvalue("pos_x2")) < assume_zero_length)
    insert_double("pos_x2", 0.);
  if (fabs(dvalue("pos_y1")) < assume_zero_length)
    insert_double("pos_y1", 0.);
  if (fabs(dvalue("pos_y2")) < assume_zero_length)
    insert_double("pos_y2", 0.);
  if (fabs(dvalue("pos_z1")) < assume_zero_length)
    insert_double("pos_z1", 0.);
  if (fabs(dvalue("pos_z2")) < assume_zero_length)
    insert_double("pos_z2", 0.);
...
atsheader::atsheader(const int &file_version, const int header_version, const int &system_serial_number, const double &sample_freq,
  ...
  if (header_version >= 1080) {
    ...
    this->insert("header_length", quint16(1024));
...
atsheader::atsheader(QObject *parent) :
    QObject(Q_NULLPTR), QFileInfo(), prc_com(false) {
...
short int atsheader::parse_filename_header_version() {
  ...
  if (tokens.size() > 6) {
    if (tokens.at(2).contains("C") && tokens.at(3).contains("R") && tokens.at(4).contains("T")) {
      ...
      if (this->value("header_version").toInt() >= 1080) {
        ...
        qWarning() << "atsheader::parse_filename_header_version "
                   << " -> metronix ats file - but CEA header given";
  ...
  // not a metronix file - assume CEA file
    ...
    // assumed a CEA file but header wnanted metronix
      ...
      qWarning() << "atsheader::parse_filename_header_version "
                 << " -> CEA ats file - but metronix header given";
    }
  }
...
void atsheader::set_start_seconds_offset(const qint64 &start_seconds_offset) {
  if (!start_seconds_offset)
    this->filepos_offset = 0;
    ...
    if (this->is_64bit_samples()) {
      ...
      this->filepos_offset = (ifreq * start_seconds_offset * qint64(sizeof(qint32)));
  }
  ...
  //!@todo here we must avoid that we say 7 seconds but - maybe don't get 7s with 4sec samples 0,4,8 ,,,,
    ...
    if (this->is_64bit_samples()) {
      ...
      this->filepos_offset = qint64(double(start_seconds_offset * qint64(sizeof(qint32))) / double(this->bin_atsheader.rSampleFreq));
  }
...
bool atsheader::is_64bit_samples() const {
  if ((this->bin_atsheader.bit_indicator == 1) && (this->bin_atsheader.siHeaderVers > 80) && (this->bin_atsheader.siHeaderVers != 1080))
    return true;
  return false;
}
...
void atsheader::set_samples_offset(const quint64 &samples_offset) {
  if (this->is_64bit_samples()) {
    ...
    this->filepos_offset = qint64(samples_offset * sizeof(qint32));
}
...
QString atsheader::subname_file() const {
  return this->svalue_rformat("system_serial_number", 3, '0') + "_V" + this->svalue_rformat("file_version", 2, '0') + "_C" + this->svalue_rformat("channel_number", 2, '0') + "_R" + this->svalue_rformat("run_number", 3, '0') + "_T" + this->svalue("channel_type") +
...
QString atsheader::subname_xml() const {
  return this->svalue_rformat("system_serial_number", 3, '0') + "_" + this->edt_start.measdoc_part_time(this->quint64value("num_samples")) + "_R" + this->svalue_rformat("run_number", 3, '0') + "_" + this->svalue("fname_sample_period") + this->svalue("fname_sample_period_unit") + ".xml";
}
...
QString atsheader::get_syscal_string() const {
  ...
  if ((this->svalue("system_type") == "ADU08") && (this->svalue("ADB_board_type") == "LF"))
    str_settings.append("ADB08e-LF_");
  else if ((this->svalue("system_type") == "ADU08") && (this->svalue("ADB_board_type") == "HF"))
    str_settings.append("ADB08e-HF_");
  else if ((this->svalue("system_type") == "ADU07") && (this->svalue("ADB_board_type") == "LF"))
    str_settings.append("ADB07e-LF_");
  else if ((this->svalue("system_type") == "ADU07") && (this->svalue("ADB_board_type") == "HF"))
    str_settings.append("ADB07e-HF_");
  else if ((this->svalue("system_type") == "ADU07") && (this->svalue("ADB_board_type") == "MF"))
    str_settings.append("ADB07e-MF_");

  int gain_stage1_i = int(this->dvalue("gain_stage1"));
  int gain_stage2_i = int(this->dvalue("gain_stage2"));

  str_settings.append("Gain1-");
  str_settings.append(QString::number(gain_stage1_i));
  str_settings.append("_");
  str_settings.append("Gain2-");
  str_settings.append(QString::number(gain_stage2_i));
  str_settings.append("_");

  if (this->ivalue("input_divder_on") == 1)
    str_settings.append("DIV-1_");
  else if (this->ivalue("input_divder_on") == 0)
    str_settings.append("DIV-8_");
...
QVariant atsheader::get_ats_option(const QString &key) const {
  if (!this->options.keys().contains(key))
...
void atsheader::init_calibration(const int auto0_txt1_sql2_xml3, const bool enable_adb_cal) {
  ...
  if (enable_adb_cal) {
    ...
    if (this->ivalue("chopper_status"))
      this->sys_calib->chopper = cal::chopper_on;
    else
      this->sys_calib->chopper = cal::chopper_on;
  ...
  if (this->svalue("sensor_type").size() && (this->svalue("sensor_type") != "empty") && (this->options.svalue("calibration") == "theo"))
    return;
...
void atsheader::clear(const short int &header_version) {
  ...
  if (is07 || is08)
    LFFilters["LF-RF-1"] = 0x01; //! ADU07/8_LF-RF-1 filter on LF board with capacitor 22pF
  if (is07 || is08)
    LFFilters["LF-RF-2"] = 0x02; //! ADU07/8_LF-RF-2 filter on LF board with capacitor 122pF
  if (is07)
    LFFilters["LF-RF-3"] = 0x04; //! ADU07_LF-RF-3 filter on LF board with capacitor 242pF
  if (is07)
    LFFilters["LF-RF-4"] = 0x08; //! ADU07_LF-RF-4 filter on LF board with capacitor 342pF
  if (is07 || is08)
    LFFilters["LF_LP_4HZ"] = 0x10; //! ADU07/8_LF_LP_4HZ filter on LF board with 4 Hz Lowpass characteristic

  if (is07)
    LFFilters["MF-RF-1"] = 0x40; //! ADU07_MF_RF_1 filter on MF board with capacitor 470nF
  if (is07)
    LFFilters["MF-RF-2"] = 0x20; //! ADU07_MF_RF_2 filter on MF board with capacitor 4.7nF

  // HF Path
  // 1 Hz has been dropped for 08
  if (is07)
    HFFilters["HF-HP-1Hz"] = 0x01; //! ADU07_HF-HP-1Hz 1Hz filter enable for HF board
  // 500Hz is the HP for 08
  if (is08)
    HFFilters["HF-HP-500Hz"] = 0x02; //! ADU08_HF-HP-500Hz 500Hz filter enable for HF board
...
size_t atsheader::write_header(const bool rewrite_close) {
  ...
  if (this->bin_atsheader.siHeaderVers >= 1080)
    return (size_t)(sizeof(this->bin_atsheader) + C_ATS_CEA_NUM_HEADERS * sizeof(ATSSliceHeader_1080));
...

{
  ...
  if (this->atswriter_section.size()) {
    ...
    if (this->atswriter_section.contains("start_time"))
      this->atswriter_section.insert("start_time", this->edt_start.toString("hh:mm:ss"));
    if (this->atswriter_section.contains("start_date"))
      this->atswriter_section.insert("start_date", this->edt_start.toString("yyyy-MM-dd"));
    if (this->atswriter_section.contains("ts_lsb"))
      this->atswriter_section.insert("ts_lsb", this->dvalue("lsb"));
...
void atsheader::classmembers_to_prc_com() {
  ...
  double tsf = 0;
  if (this->dvalue("rOrigSampleFreq") != 0.0)
    tsf = this->dvalue("rOrigSampleFreq");
  else {
  ...
  */
  ...
  bool is07 = false;
  bool is08 = false;
  if (this->svalue("system_type") == "ADU07")
    is07 = true;
  if (this->svalue("system_type") == "ADU08")
    is08 = true;

  //! filter banks setup contains following
  if (is08)
    LFFilters.remove("LF-RF-3");
  if (is08)
    LFFilters.remove("LF-RF-4");

  if (is08)
    LFFilters.remove("MF_RF_1");
  if (is08)
    LFFilters.remove("MF_RF_2");

  // HF Path
  // 1 Hz has been dropped for 08
  if (is08)
    HFFilters.remove("HF-HP-1Hz");
  // 500Hz is the HP for 08
  if (is07)
    HFFilters.remove("HF-HP-500Hz");
  ...
  if ((this->svalue("ADB_board_type") == "LF") || (this->svalue("ADB_board_type") == "MF")) {
    ...
    if (hits) {
      ...
      for (auto &str : cpFilterstrings) {
        if (allkeys.contains(str))
          last_filter.append(str);
      }

      if (last_filter.size() != 1)
        return; // we can not set two RF filters
...
QString atsheader::clean_b_stringUTF8(const char *ins, const int s_size, const bool remove_terminator) const {
  ...
  while (rstring.size()) {
    if (rstring.endsWith(invalid) && rstring.size())
      rstring.remove(rstring.size() - 1, 1);
    else
      break;
  }
...
bool same_recording(const atsheader &lhs, const atsheader &rhs) {
  bool same = true;
  if (rhs.qint64value("start_date_time") != lhs.qint64value("start_date_time"))
    same = false;
  if (rhs.get_sample_freq() != lhs.get_sample_freq())
    same = false;
  if (lhs.get_num_samples() != rhs.get_num_samples())
    same = false;
