...
tsdata_header::tsdata_header(const QFileInfo &qfi, QObject *parent) : QObject(parent), QFileInfo(qfi), prc_com(false)
{
...
tsdata_header::tsdata_header()  : QObject(0), QFileInfo(), prc_com(false)
{
    ...
    do  {
        ...
        if (this->global_header.contains(key_val.at(0)) && key_val.size() && !key_val.contains("STARTTIME") && !key_val.contains("ENDTIME")) {
            ...
            if (key_val.contains("STARTTIME")) {
                ...
                if (start_str.startsWith(":")) start_str.remove(0,1);
            ...
            if (key_val.contains("ENDTIME")) {
                ...
                this->stop_time.setTimeSpec(Qt::UTC);
...
bool tsdata_header::guess_values()
{
    ...
    if (!this->nsamples || (this->sample_freq == 0)) return false;
    else return true;
}
...
quint64 tsdata_header::create_ats_files()
{
    ...
    double lsb;
    if (std::abs(this->minx) > std::abs(this->maxx)) lsb = std::abs(this->minx) * 6 /  ((double)INT32_MAX);
    else lsb = std::abs(this->maxx) * 6 /  ((double)INT32_MAX);
