...
mt_site::mt_site(const size_t num_threads, const mttype enum_mttype, QObject *parent) :
    QObject(parent), prc_com(true) {
  ...
  if (what == "single_amplitude_stacked_spectra") {
    ...
    for (auto &acv : this->ac_spectra.at(0).ac) {
      if (acv.size()) {
        ...
        if (f.size() && f.size() == v.size())
          emit this->signal_fde_vector_created(acsp_name, f, v, e);
      }
      ++spce;
    }
  }
  ...
  if (what == "parallel_coherency") {
    ...
    for (auto &acv : this->ac_spectra.at(0).coh) {
      if (acv != DBL_MAX) {
        ...
        if (f.size() && f.size() == v.size())
          emit this->signal_fd_vector_created(acsp_name, f, v);
      }
      ++spce;
    }
  }
...
void mt_site::clean_up_residuals() {
  if (this->ivalue("remove_rr_residuals")) {
    for (auto &ac : this->ac_spectra)
      ac.reset_cross_remote_residuals();
  }

  if (this->ivalue("remove_auto_residuals")) {
    for (auto &ac : this->ac_spectra)
      ac.reset_auto_residuals();
  }
}
...
void mt_site::run() {
  ...
  if (this->sztvalue("min_stacks") < _min_stacks)
    this->insert_szt("min_stacks", _min_stacks);
  qDebug() << "mtsite::run -> frequencies incoming:";
  for (auto &d : this->get_freqs())
    qDebug() << doublefreq2string_unit(d);
...
void mt_site::calc_z_vector(const double &f) {
  ...
  if (f_idx == SIZE_MAX)
    return;
  bool has_t = false;
  if (this->ac_spectra.at(f_idx).ac.at(pmt::hxhz).size() && this->ac_spectra.at(f_idx).ac.at(pmt::hyhz).size())
    has_t = true;
    ...
    if (denom.real() == 0.0 && denom.imag() == 0.0) {
      ...
      if (has_t) {
        this->z[f_idx].d[tx][i] = ((this->ac_spectra[f_idx].ac[pmt::hxhz][i] * this->ac_spectra[f_idx].ac[pmt::hyhy][i]) -
                                   (this->ac_spectra[f_idx].ac[pmt::hyhz][i] * conj(this->ac_spectra[f_idx].ac[pmt::hxhy][i]))) /
                                  denom;

        this->z[f_idx].d[ty][i] = ((this->ac_spectra[f_idx].ac[pmt::hyhz][i] * this->ac_spectra[f_idx].ac[pmt::hxhx][i]) -
                                   (this->ac_spectra[f_idx].ac[pmt::hxhz][i] * this->ac_spectra[f_idx].ac[pmt::hxhy][i])) /
                                  denom;
      }
    }
  }
...
void mt_site::slot_ssp_stack_all() {
  ...
  for (;;) {
    for (j = 0; j < this->num_threads; ++j) {
      if (i < all)
        this->ssp_threads.emplace_back(std::thread(&mt_site::ssp_stack_all_vector, this, freqs.at(i++)));
    }
...
void mt_site::slot_calc_auto_spc() {
  ...
  for (;;) {
    for (j = 0; j < this->num_threads; ++j) {
      if (i < all)
        this->ac_threads.emplace_back(std::thread(&mt_site::calc_auto_spc_vector, this, freqs.at(i++)));
    }
...
void mt_site::slot_calc_ac_spc() {
  ...
  for (;;) {
    for (j = 0; j < this->num_threads; ++j) {
      if (i < all)
        this->ac_threads.emplace_back(std::thread(&mt_site::calc_ac_spc_vector, this, freqs.at(i++)));
    }
    for (auto &thrd : this->ac_threads) {
      thrd.join();
    }
...
void mt_site::ssp_dump_stack_all_vector(const size_t &spce, bool error_bars) {
  ...
  if (qfi.open(QIODevice::Append | QIODevice::Text)) {
    if (error_bars) {
      for (auto &ac : this->ac_spectra) {
        if (ac.ac_vals[spce].size())
          out << ac.f << "  " << (ac.ac_vals[spce].at(g_stat::mean_x)) << "  " << (ac.ac_vals[spce].at(g_stat::stddev_x)) << Qt::endl;
      }
    } else {
      for (auto &ac : this->ac_spectra) {
        if (ac.ac_vals[spce].size())
          out << ac.f << "   " << (ac.ac_vals[spce].at(g_stat::mean_x)) << Qt::endl;
      }
    }
  }
  qfi.close();
...
void mt_site::huber(const double hlimit, const size_t iter_limit) {
  ...
  if (this->ac_spectra.at(0).ac.at(pmt::hxhz).size() && this->ac_spectra.at(0).ac.at(pmt::hyhz).size()) {
    ...
    this->zet.resize(tns::tns_size, freqs);
...
void mt_site::huber_vector(const double &f, const double hlimit, const size_t iter_limit) {
  ...
  for (int zz = 0; zz < 2; ++zz) {
    ...
    for (size_t i = 0; i < iter_limit; ++i) {
      ...
      size_t j = 0;
      for (auto &es : e) {
        if (1.58 * std::abs(sigma) > abs(es))
          w[j] = std::complex<double>(1.0, 0.0);
        else
          w[j] = 1.58 * sigma / es;
        ++j;
      }
...
void mt_site::stack_all_vector(const double &f) {
  ...
  for (auto &ztensf : this->z[f_idx].d) {
    ...
    if (this->z[f_idx].trues[i] >= this->sztvalue("min_stacks")) {
      ...
      if (!i)
        qDebug() << "processing stack all z" << doublefreq2string_unit(f) << "selected:" << selected;
...
void mt_site::ssp_xstddev_vector(const double &freq) {
  ...
  for (auto &data : this->ac_spectra[f_idx].ac) {
    if (data.size()) {
      ...
      size_t jj = 0;
      for (auto &cplx : data)
        ampls[jj++] = (std::sqrt(std::abs(cplx)));
      if (ampls.size()) {
        for (auto &amp : ampls) {
...
void mt_site::coherency_vector(const double &f, const double &cohlower, const double &cohupper) {
  ...
  for (i = 0; i < stacks; ++i) {
    if (this->z[f_idx].istrue_xx_xy_yx_yy(i)) {
      ++sel;
      cohxy = std::real(this->z[f_idx].d[xx][i] * std::conj(this->ac_spectra[f_idx].ac[pmt::exhx][i]) + this->z[f_idx].d[xy][i] * std::conj(this->ac_spectra[f_idx].ac[pmt::exhy][i])) / (std::abs(this->ac_spectra[f_idx].ac[pmt::exex][i]));

      cohyx = std::real(this->z[f_idx].d[yx][i] * std::conj(this->ac_spectra[f_idx].ac[pmt::eyhx][i]) + this->z[f_idx].d[yy][i] * std::conj(this->ac_spectra[f_idx].ac[pmt::eyhy][i])) / (std::abs(this->ac_spectra[f_idx].ac[pmt::eyey][i]));
...
void mt_site::quadrant_check(const size_t what) {
  if (this->ivalue("activate_quadrant_check") == 0)
    return;
  if ((this->mymttye >= mttype::mt) && this->mymttye <= mttype::emap_rr) {
...
void mt_site::quadrant_check_vector(const double &f) {
  ...
  if (sz >= tns_size)
    sz = tns_size; // make only sense for Z but not for tipper
