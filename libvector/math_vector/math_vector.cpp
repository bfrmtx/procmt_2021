...
math_vector::math_vector(QObject *parent) :
    QObject(parent) {
  this->channel = -1;
}
...
size_t math_vector::akima_vector(const std::vector<double> &vcalfreqs, const std::vector<std::complex<double>> &vcalentries,
  ...
  if (calibipl.size() != cols)
    calibipl.resize(cols);
  ...
  if (old_cols > 1) {
    ...
    for (i = 0; i < calibipl.size(); ++i)
      calibipl[i] = std::complex<double>(1.0, 0.);
  ...
  for (auto &v : inmerge) {
    ...
    if (v < *orig_mm.first)
      ...
      for (j = 0; j < orig.size(); ++j)
        out.push_back(orig.at(j));
      insert = false;
    } else if (v > *orig_mm.second)
      out.push_back(v);
  }
...
int math_vector::merge_sorted_low_to_high_outside_join(const std::vector<double> &forig, const std::vector<double> &finmerge, std::vector<double> &foutmerge,
  ...
  if (!forig.size() || !finmerge.size() || forig.size() < 3)
    return 0;
...
bool math_vector::remove_duplicates_from_sorted(std::vector<double> &f, std::vector<double> &a, std::vector<double> &p,
  ...
  tf.reserve(sz);
  ta.reserve(sz);
  tp.reserve(sz), tda.reserve(sz), tdp.reserve(sz);
...
size_t math_vector::slot_parzen_vector(const std::vector<double> &freqs, const std::vector<double> &target_freqs, const double &prz_radius) {
  ...
  if (prz_rad < max_parzen) {
    ...
    double take_two = 1.0 / 64.0; // for periods > 64 sec, we just take two values from the spectra, closest to the target frequency
    for (size_t i = 0; i < lower_prz_bounds.size(); ++i) {
      if ((lower_prz_bounds[i] >= *min_freq) && (upper_prz_bounds[i] <= *max_freq)) {

        found = false;

        // for short periods we take the classic parzen
        //   low = std::lower_bound(min_freq, max_freq, lower_prz_bounds[i]);
        //   high = std::upper_bound(min_freq, max_freq, upper_prz_bounds[i]);
        //   int low_pos = std::distance(lower_prz_bounds, low);   // Position of 'low' as an integer
        //   int high_pos = std::distance(upper_prz_bounds, high); // Position of 'high' as an integer
        low = std::lower_bound(min_freq, max_freq, lower_prz_bounds[i]);
        high = std::upper_bound(min_freq, max_freq, upper_prz_bounds[i]);
        qDebug() << *high << " <-> " << *low;
        ...
        if (!found) {
          // for long periods we just take the two closest values
          low = std::lower_bound(min_freq, max_freq, target_freqs[i]);
          high = std::upper_bound(min_freq, max_freq, target_freqs[i]);
          low = std::prev(low, 1);
          qDebug() << *high << " <-> " << *low;
          qDebug() << 1.0 / (*high) << " <-> " << 1.0 / (*low) << " target: " << 1.0 / target_freqs[i];
        ...
        if (!found) {
          ...
          // ok, for long periods that is not good, let us try in freqs if we have a greater and a smaller element compared to target_freqs
          // check that i > 1 and i < freqs.size() - 1
        }
      }
    }
  }
...
statmap math_vector::mk_regression_data(const statmap &x_variance_data, const statmap &y_variance_data, bool &is_ok) {
  ...
  if ((x_variance_data.at(g_stat::d_n_x) == y_variance_data.at(g_stat::d_n_x)) && (x_variance_data.at(g_stat::d_n_x) >= 4.00)) {
    ...
    for (i = 0; i < g_stat::xstat_size; ++i) {
      if (x_variance_data.find(i) != x_variance_data.cend())
        datamap[i] = x_variance_data.at(i);
    }

    for (i = 0; i < g_stat::xstat_size; ++i) {
      if (y_variance_data.find(i) != y_variance_data.cend())
...
bool regplot_data::mkplot_slope_if(std::vector<double>::const_iterator xcbeg, std::vector<double>::const_iterator xcend,
  ...
  if ((slope_type_0s_1u_2l == 1) || (slope_type_0s_1u_2l == 2)) {
    ...
    if (slope_type_0s_1u_2l == 2)
      myconfidence = this->d_upper_confidence;
    if (step == 0.0) {
  ...
  if ((slope_type_0s_1u_2l == 3) || (slope_type_0s_1u_2l == 4)) {
    ...
    if (step == 0.0) {
      ...
      if (selected_cbeg == selected_cend) {
        for (size_t i = 0; i < x.size(); ++i) {
          dist_from_slope = factor * sqrt(((pow((x[i] - this->d_meanx), 2.0) / denom) + (1.0 / this->d_n))) + extra_offsety;
          if (slope_type_0s_1u_2l == 3)
            dist_from_slope *= -1.0;
...
bool select_in_out_reg::mkselect_in_out_reg(std::vector<double>::const_iterator xcbeg, std::vector<double>::const_iterator xcend,
  ...
  if ((slope_type_0s_1u_2l == 1) || (slope_type_0s_1u_2l == 2)) {
    ...
    for (; xcbeg != xcend; ++xcbeg, ++ycbeg, ++selected_cbeg) {
      dist_from_slope = factor * sqrt(((pow((*xcbeg - this->d_meanx), 2.0) / denom) + (1.0 / this->d_n))) + extra_offsety;
...
statmap two_pass_variance_cplx::variance(std::vector<std::complex<double>>::const_iterator xcbeg, std::vector<std::complex<double>>::const_iterator xcend,
  ...
  this->d_max = *xcbeg;
  for (; xcbeg != xcend; ++xcbeg) {
    if (std::abs(*xcbeg) < std::abs(this->d_min))
      this->d_min = std::abs(*xcbeg);
    if (std::abs(*xcbeg) > std::abs(this->d_max))
      this->d_max = std::abs(*xcbeg);
    sdiffs = *xcbeg - this->d_mean;
