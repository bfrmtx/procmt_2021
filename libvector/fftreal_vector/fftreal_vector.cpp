...
bool fftreal_vector::set_buffer(const std::vector<double> &in_out_buf, std::vector<std::complex<double>> &outbuf,
  ...
  if (freq != 0.0) {
    ...
    this->f_sample = freq;
    if (bwincal)
      this->wincal = sqrt(1. / double(f_sample * (this->wl / 2)));
    ...
    if (!this->start_idx && !this->isdcfft)
      start_idx = 1;
    this->stop_idx = size_t(double(this->wl / 2) - (double(this->wl / 2) * this->fcut_upper)) + 1;
    if (stop_idx > this->wl / 2)
      stop_idx = wl / 2; // stop idx can be 1024 because STL like we do i = start && < stop
  ...
  if (this->inverse_from_forward) {
    ...
    this->n = wl / 2;
    if (!nspc)
      outbuf.resize(n - 1); // we can not use the DC for the spectra (log/log)
  }
...
bool fftreal_vector::set_mtx_calibration(std::vector<std::complex<double>> &divcal) {
  if ((divcal.size() != (this->n - 1)) && this->isdcfft)
    return false;
  if ((divcal.size() != this->n) && !this->isdcfft)
    return false;
  this->divcal = &divcal;
  this->has_calibration = true;
  return true;
...
void fftreal_vector::process(std::vector<double> &inbuf, std::vector<std::complex<double>> &outbuf, const int ctrl) {
  ...
  if (!nspc && !this->inverse_from_forward)
    outbuf.resize(n);
  if (!nspc && (this->inverse_from_forward || this->isdcfft))
    outbuf.resize(n - 1); // avoid DC
  ...
  if (this->has_calibration_external) {
    ...
    if (((this->divcal->size() == (this->n - 1)) && this->isdcfft) || ((this->divcal->size() == this->n) && !this->isdcfft)) {
      ...
      //            }
      if (i == 0)
        ++i; // all mtx function do not include the DC part
  ...
  if (!nspc) {
    if (this->wincal != 0.0) {
      j = 0;
      size_t i = this->start_idx;
      if ((this->isdcfft || this->inverse_from_forward) && (i == 0))
        ++i;
      for (; i < this->stop_idx; ++i) {
