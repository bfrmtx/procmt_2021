...
struct MinMax {
  ...
  MinMax() :
      min(1e30), max(-1e30) {}
  MinMax(double minval, double maxval) :
...
struct TSPlotInfo {
  ...
  TSPlotInfo() :
      plot_ts(nullptr), plot_spc(nullptr), plot_no(0) {}
...
private:
  ...
  std::chrono::time_point<std::chrono::high_resolution_clock> start, end, currentTime;
  int64_t plot_delay = 20; //!< plot delay in ms
