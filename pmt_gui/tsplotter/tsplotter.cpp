...
TSPlotType get_plot_type_for_channel(QString const &channel_type_string) {
  if (channel_type_string == channel_type_str_ex)
    return TSPlotType::EX;
  else if (channel_type_string == channel_type_str_ey)
    return TSPlotType::EY;
  else if (channel_type_string == channel_type_str_hx)
    return TSPlotType::HX;
  else if (channel_type_string == channel_type_str_hy)
    return TSPlotType::HY;
  else if (channel_type_string == channel_type_str_hz)
    return TSPlotType::HZ;
...
tsplotter::tsplotter(QWidget *parent) :
  ...
  info_db.setFile("info.sql3");
  master_cal_db.setFile("master_calibration.sql3");
...
void tsplotter::reload_current_sample_window(bool force_set_window_length) {
  ...
  if (m_option_calibrate) {
    ...
    if (this->m_option_system_calibrate)
      this->ui->button_action_b_calibrate->setText("CAL + SYS is ON");
    else
      this->ui->button_action_b_calibrate->setText("CAL is ON");
...
void tsplotter::set_sample_window(qulonglong position, qlonglong window_length,
  ...
  if (std::chrono::duration_cast<std::chrono::milliseconds>(this->currentTime - this->end).count() < this->plot_delay) {
    return;
  }
  ...
  if (spectra_is_visible())
    atsfileout_spectra = atsfileout::atsfileout_spectra;
  if (m_option_calibrate)
    atsfileout_spectra = atsfileout::atsfileout_calibrated_spectra;
  if (m_option_scale)
    atsfileout_timeseries = atsfileout::atsfileout_scaled_timeseries;
...
void tsplotter::recreate_plots() {
  ...
  // sort by channel name (Ex, Ey, Hx, Hy, Hz)
  std::sort(m_mcdata->in_atsfiles.begin(), m_mcdata->in_atsfiles.end(),
            [](auto &a, auto &b) { return a->channel_type() < b->channel_type(); });

  // sort by channel number
  std::sort(m_mcdata->in_atsfiles.begin(), m_mcdata->in_atsfiles.end(),
            [](auto &a, auto &b) { return a->value("channel_number").toInt() < b->value("channel_number").toInt(); });
...
void tsplotter::clear_structures() {
  ...
  for (auto &button : m_ui_tsbuttons)
    delete button;
  for (auto &button : m_ui_spcbuttons)
    delete button;
  for (auto &plot : m_ui_tsplots)
    delete plot;
  for (auto &plot : m_ui_spcplots)
...
void tsplotter::reset_frequency_vector(std::shared_ptr<atsfile> ats_file) {
  ...
  if (stop_idx > f.size())
    stop_idx = f.size();
  if (plotter_frequencies != nullptr)
    plotter_frequencies.reset();
  if (plotter_frequencies == nullptr)
    plotter_frequencies =
  ...
  for (auto qfis : files) {
    ...
    if (qfis.isDir()) {
      ...
      if (qfis.absoluteFilePath().endsWith("xml", Qt::CaseInsensitive) ||
          qfis.absoluteFilePath().endsWith("ats", Qt::CaseInsensitive))
        fileinfos.push_back(qfis);
    }
  }
...
void tsplotter::open_files() {
  ...
  for (auto &qfi : fileinfos) {
    if (qfi.absoluteFilePath().endsWith("ats", Qt::CaseInsensitive))
      fileinfos_ats.append(qfi);
    if (qfi.absoluteFilePath().endsWith("xml", Qt::CaseInsensitive))
      fileinfos_xml.append(qfi);
  }
  ...
  if (fileinfos_xml.size()) {
    ...
    if (test_open->open_files(fileinfos_xml, true, mttype::nomt)) {
      ...
      if (xmap.contains("survey"))
        title.append(xmap.value("survey").toString() + "  ");
      if (xmap.contains("site"))
        title.append(xmap.value("site").toString() + "  ");
      for (auto &str : fileinfos_xml)
        title.append(str.baseName() + " ");
  ...
  case TSPlotterPlotAddingOption::ADD: {
    ...
    for (auto &newfile : fileinfos_ats) {
      if (!this->fileinfos.contains(newfile))
        this->fileinfos.append(newfile);
    }
...
void tsplotter::inc_cursor_mode() {
  if (m_current_cursor_mode == CursorMode::DEFAULT)
    set_cursor_mode(CursorMode::SELECTION_ADD);
  else if (m_current_cursor_mode == CursorMode::SELECTION_ADD)
    set_cursor_mode(CursorMode::SELECTION_REMOVE);
  else if (m_current_cursor_mode == CursorMode::SELECTION_REMOVE)
    set_cursor_mode(CursorMode::DEFAULT);
}

void tsplotter::on_button_group_adding_mode_buttonClicked(QAbstractButton *button) {
  if (button == ui->button_plotaddingmode_add)
    set_plot_add_mode(TSPlotterPlotAddingOption::ADD);
  else if (button == ui->button_plotaddingmode_replace)
    set_plot_add_mode(TSPlotterPlotAddingOption::REPLACE);
}

void tsplotter::on_button_group_plot_mode_buttonClicked(QAbstractButton *button) {
  if (button == ui->button_plotmode_overlay)
    set_plot_view_mode(TSPlotterPlotViewOption::OVERLAY);
  else if (button == ui->button_plotmode_separate)
    set_plot_view_mode(TSPlotterPlotViewOption::SEPARATE);
  else if (button == ui->button_plotmode_ptest)
    set_plot_view_mode(TSPlotterPlotViewOption::OVERLAY_PARALLEL);
  else if (button == ui->button_plotmode_noise)
    set_plot_view_mode(TSPlotterPlotViewOption::OVERLAY_ALL);
}

void tsplotter::on_button_group_cursor_mode_buttonClicked(QAbstractButton *button) {
  if (button == ui->button_mode_default)
    set_cursor_mode(CursorMode::DEFAULT);
  else if (button == ui->button_mode_select_add)
    set_cursor_mode(CursorMode::SELECTION_ADD);
  else if (button == ui->button_mode_select_rem)
    set_cursor_mode(CursorMode::SELECTION_REMOVE);
}

void tsplotter::slot_on_actiongroup_mouse_triggered(QAction *action) {
  if (action->isChecked()) {
    if (action == ui->menu_action_mouse_readout)
      set_cursor_mode(CursorMode::READOUT);
    else if (action == ui->menu_action_mouse_selects)
      set_cursor_mode(CursorMode::SELECTION_ADD);
    else if (action == ui->menu_action_mouse_deselects)
      set_cursor_mode(CursorMode::SELECTION_REMOVE);
  }
}
...
void tsplotter::set_yaxis_range_for_plot(TSPlotterPlot *plot) {
  ...
  if (plot->plot_type() == TSPlotType::EX) {
    if (ui->button_scale_allE->isChecked()) {
      plot->yAxis->setRange(e.min, e.max);
      set = true;
    } else if (ui->button_scale_allEx->isChecked()) {
      plot->yAxis->setRange(ex.min, ex.max);
      set = true;
    } else if (ui->button_scale_all->isChecked()) {
    ...
    if (ui->button_scale_allE->isChecked()) {
      plot->yAxis->setRange(e.min, e.max);
      set = true;
    } else if (ui->button_scale_allEy->isChecked()) {
      plot->yAxis->setRange(ey.min, ey.max);
      set = true;
    } else if (ui->button_scale_all->isChecked()) {
      plot->yAxis->setRange(a.min, a.max);
      set = true;
    ...
    if (ui->button_scale_allH->isChecked()) {
      plot->yAxis->setRange(h.min, h.max);
      set = true;
    } else if (ui->button_scale_allHx->isChecked()) {
      plot->yAxis->setRange(hx.min, hx.max);
      set = true;
    } else if (ui->button_scale_allHxHy->isChecked()) {
      plot->yAxis->setRange(hxhy.min, hxhy.max);
      set = true;
    } else if (ui->button_scale_all->isChecked()) {
      plot->yAxis->setRange(a.min, a.max);
      set = true;
    ...
    if (ui->button_scale_allH->isChecked()) {
      plot->yAxis->setRange(h.min, h.max);
      set = true;
    } else if (ui->button_scale_allHy->isChecked()) {
      plot->yAxis->setRange(hy.min, hy.max);
      set = true;
    } else if (ui->button_scale_allHxHy->isChecked()) {
      plot->yAxis->setRange(hxhy.min, hxhy.max);
      set = true;
    ...
    if (ui->button_scale_allH->isChecked()) {
      plot->yAxis->setRange(h.min, h.max);
      set = true;
    } else if (ui->button_scale_allHz->isChecked()) {
      plot->yAxis->setRange(hz.min, hz.max);
      set = true;
    } else if (ui->button_scale_all->isChecked()) {
      plot->yAxis->setRange(a.min, a.max);
      set = true;
...
void tsplotter::on_menu_action_export_ascii_triggered() {
  bool is_cal = false;
  if (ui->button_action_b_calibrate->isDown())
    is_cal = true;
...
void tsplotter::re_connect() {
  if (this->m_mcdata != nullptr)
    this->m_mcdata.reset(nullptr);
...
void tsplot::tsplotter::on_pushButton_gmaps_clicked() {
  if (m_mcdata == nullptr || !m_mcdata->in_atsfiles.size())
    return;
  geocoordinates coord;
  coord.set_lat_lon_msec(m_mcdata->in_atsfiles.at(0)->ivalue("latitude"), m_mcdata->in_atsfiles.at(0)->ivalue("longitude"), (long double)(m_mcdata->in_atsfiles.at(0)->dvalue("elevation") / 100.));
