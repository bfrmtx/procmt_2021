...
ContentSubtype parseSubType(QString const &str) {
  if (str == "unix-directory")
    return ContentSubtype::UnixDirectory;
  if (str == "pdf")
    return ContentSubtype::PDF;
  if (str == "xml")
    return ContentSubtype::XML;
  if (str == "doc")
    return ContentSubtype::DOC;
  if (str == "json")
    return ContentSubtype::JSON;
  if (str == "octet-stream")
    return ContentSubtype::OctetStream;
