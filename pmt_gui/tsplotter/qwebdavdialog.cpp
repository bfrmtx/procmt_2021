...
QString trimURI(QString uri) {
    ...
    if(uri.back() == '/') uri = uri.mid(0, uri.size() - 1);
    if(uri.front() == '/') uri = uri.mid(1);
    return uri;
}
        ...
        m_network_manager->execSecuredHeadRequest(uri, [this, target, URIs](QList<QNetworkReply::RawHeaderPair> const & header) {
            ...
            if(m_pending_requests == 0) {
                ...
                for(auto const & uri : URIs) {
                    ...
                    auto rep = m_network_manager->execSecuredGetRequestObj(uri, [=](QNetworkReply * reply) {
...
QWebDavDialogItemData::QWebDavDialogItemData(QString uri, webdav::Response data) : uri(uri), data(data) {}
