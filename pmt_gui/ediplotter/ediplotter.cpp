...
bool has_coherency_data(const mt_data_res<std::complex<double>> &data) {
  ...
  if (data.coh.size() <= tns::xxxy) {
    return false;
  }
  return data.coh[tns::xxxy].size() == data.freqs.size();
}
} // namespace
...
void ediplotter::on_button_actions_toggle_map_phase_clicked(bool checked) {
  ...
  if (this->ui->button_actions_toggle_map_phase->isChecked())
    this->phase_is_mapped = true;
  else
    this->phase_is_mapped = false;
