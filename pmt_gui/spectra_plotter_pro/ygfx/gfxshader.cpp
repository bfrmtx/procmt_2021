...
void Shader::destroyShader() {
  OGL_IF_CONTEXT {
    OGL_IF_FUNCS {
      if (shader_id > 0)
        OGLFUNCS glDeleteProgram(shader_id);
    }
  }
}
...
std::shared_ptr<Shader> Shader::createShaderFromSourceCode(const std::string &fragmentcode,
  ...
  OGL_IF_CONTEXT {
    OGL_IF_FUNCS {
      ...
      if (!r_vertexcode.empty())
        OGLFUNCS glDeleteShader(pVertex);
      if (!r_fragmentcode.empty())
        OGLFUNCS glDeleteShader(pFragment);
      if (!r_geomcode.empty())
        OGLFUNCS glDeleteShader(pGeometry);
    }
    else {
