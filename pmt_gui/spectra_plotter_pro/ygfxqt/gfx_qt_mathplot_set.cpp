...
ygfxqt::YMathGfxPlotSet2D::~YMathGfxPlotSet2D() {
  ...
  if (is_y_data_owner)
    delete gfx_y_values;
  if (is_x_org_data_owner)
    delete org_x_data;
  if (is_y_org_data_owner)
    delete org_y_data;
  delete buffer_set;
}
...
void ygfxqt::YMathGfxPlotSet2D::removeLastData(size_t size) {
  if (org_x_data->size() > size)
    org_x_data->resize(org_x_data->size() - size);
  else
    org_x_data->clear();
  if (org_y_data->size() > size)
    org_y_data->resize(org_y_data->size() - size);
  else
    org_y_data->clear();
  if (gfx_x_values->size() > GLint(size))
    gfx_x_values->resize(GLuint(gfx_x_values->size()) - GLuint(size));
  else
    gfx_x_values->clear();
  if (gfx_y_values->size() > GLint(size))
    gfx_y_values->resize(GLuint(gfx_y_values->size()) - GLuint(size));
  else
    gfx_y_values->clear();
}
