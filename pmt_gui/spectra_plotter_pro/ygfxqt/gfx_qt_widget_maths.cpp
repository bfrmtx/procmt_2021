...
ygfxqt::YMathOpenGLWidget::YMathOpenGLWidget(QWidget *parent) :
...
void ygfxqt::YMathOpenGLWidget::initializeGL() {
  ...
  qDebug() << "SHARED CONTEXT ADR          :" << reinterpret_cast<void *>(context()->shareContext());
  {
    ...
    sstream << "GL_VENDOR                   : " << glGetString(GL_VENDOR);
    qDebug().noquote() << QString::fromStdString(sstream.str());
  }
  {
    std::stringstream sstream;
    sstream << "GL_RENDERER                 : " << glGetString(GL_RENDERER);
    qDebug().noquote() << QString::fromStdString(sstream.str());
  }
  {
    std::stringstream sstream;
    sstream << "GL_VERSION                  : " << glGetString(GL_VERSION);
    qDebug().noquote() << QString::fromStdString(sstream.str());
  }
  {
    std::stringstream sstream;
    sstream << "GL_SHADING_LANGUAGE_VERSION : " << glGetString(GL_SHADING_LANGUAGE_VERSION);
    qDebug().noquote() << QString::fromStdString(sstream.str());
  }

  {
    std::stringstream sstream;
    sstream << glGetString(GL_EXTENSIONS);
    qDebug().noquote() << endl
                       << "THIS GL IMPLEMENTATIONS SUPPORTS THE FOLLOWING EXTENSIONS: ";
...
void ygfxqt::YMathOpenGLWidget::paintGL() {
  ...
  painter.beginNativePainting();
  {
    ...
    qDebug().noquote() << "opengl-viewport: "
                       << "x:" << viewport[0] << "|y:" << viewport[1] << "|w:" << viewport[2] << "|h:" << viewport[3];
    qDebug().noquote() << "widget size/pos: "
                       << "x:" << this->x() << "|y:" << this->y() << "|w:" << this->width() << "|h:" << this->height();
    qDebug().noquote() << "axes offsets   : "
                       << "x:" << axes_offset.x() << "|y:" << axes_offset.y();
    qDebug().noquote() << "***************************";
#endif
  }
      ...
      if (event->button() == selector_tool->selectionButton()) {
        for (auto plot : plots) {
          if (plot->isVisible()) {
            auto plot2d = static_cast<YMathGfxPlotSet2D *>(plot);
            if (x_axis_order == AxisOrder::REVERSED)
              selector_tool->T_xaxis_reversed = true;
            selector_tool->select(plot2d->getXData(), plot2d->getYData(), plot2d);
      ...
      if (state_info.testFlag(InternalInfo::InAxisXDragMode) && options_plot.testFlag(PlotOption::EnableXAxisMove)) {
        if (std::abs(delta_position.x()) < delta_pixel_mousejump) {
          ...
          if (event->pos().x() > this->width()) {
            QCursor::setPos(mapToGlobal(QPoint(2 + axes_offset.x(), event->pos().y())));
          } else if (event->pos().x() < axes_offset.x()) {
            QCursor::setPos(mapToGlobal(QPoint(width() - 2, event->pos().y())));
          }
        }
        ...
        if (std::abs(delta_position.y()) < delta_pixel_mousejump) {
          ...
          if (event->pos().y() > this->height() - axes_offset.y()) {
            QPoint point(event->pos().x(), 2);
            QCursor::setPos(mapToGlobal(point));
          } else if (event->pos().y() < 0) {
            QPoint point(event->pos().x(), height() - axes_offset.y() - 2);
            QCursor::setPos(mapToGlobal(point));
          }
        }
      }
