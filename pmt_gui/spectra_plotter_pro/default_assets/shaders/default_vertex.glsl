...
layout(location = $loc_color$) in vec3 in_color;
...
void main() {
    test_color = vec4(in_color.rgb, 1.0);
    gl_Position.x = 1.0 - ((topright.x - in_pos.x) / resolution_plot.x) * 2.0;
    gl_Position.y = 1.0 - ((topright.y - in_pos.y) / resolution_plot.y) * 2.0;
