...
void spectra_plotter::show_frequency(double frequency, QPushButton *freqButton) {
  ...
  if (freqButton != nullptr) {
    ...
    for (auto plot : plots_single_freq)
      plot->hideAllPlotsExceptIndex(size_t(index));
    for (auto plot : plots_single_freq)
      plot->fitScaleToPlot(size_t(index));
...
void spectra_plotter::add_frequency_data(double frequency, const mt_data_t<std::complex<double>> &data) {
  ...
  auto p_shader = ui->cb_performance->isChecked() ? ygfxqt::YGFXQTAssetManager::getShader(ygfxqt::YGFXQTAssetManager::SHADER_PLOT_2D_POINTS_PERF) : ygfxqt::YGFXQTAssetManager::getShader(ygfxqt::YGFXQTAssetManager::SHADER_PLOT_2D_POINTS);
  ...
  for (size_t i = 0; i < 4; ++i) {
    gfxVector<GLfloat> *fbuffer_rhoa = new gfxVector<GLfloat>();
    fbuffer_rhoa->reserve(glsize);
    gfxVector<GLfloat> *fbuffer_phi = new gfxVector<GLfloat>();
    fbuffer_phi->reserve(glsize);
    gfxVector<GLfloat> *fbuffer_indices = new gfxVector<GLfloat>();
    fbuffer_indices->reserve(glsize);
    gfxVector<GLfloat> *fbuffer_real = new gfxVector<GLfloat>();
    fbuffer_real->reserve(glsize);
    gfxVector<GLfloat> *fbuffer_imag = new gfxVector<GLfloat>();
    fbuffer_imag->reserve(glsize);
    std::vector<double> *dbuffer_rhoa = new std::vector<double>();
    dbuffer_rhoa->reserve(glsize);
    std::vector<double> *dbuffer_phi = new std::vector<double>();
    dbuffer_phi->reserve(glsize);
    std::vector<double> *dbuffer_indices = new std::vector<double>();
    dbuffer_indices->reserve(glsize);
    std::vector<double> *dbuffer_real = new std::vector<double>();
    dbuffer_real->reserve(glsize);
    std::vector<double> *dbuffer_imag = new std::vector<double>();
    dbuffer_imag->reserve(glsize);
...
void spectra_plotter::on_button_group_visibility_buttonClicked(QAbstractButton *button) {
  if (button == ui->btn_show_all) {
    ...
    for (auto plot : plot_widgets_gauss)
      plot->setVisible(plot == ui->plot_fstack_xx);
    for (auto plot : plot_widgets_phi)
      plot->setVisible(plot == ui->plot_phi_xx);
    for (auto plot : plot_widgets_rhoa)
      plot->setVisible(plot == ui->plot_rhoa_xx);
  } else if (button == ui->btn_show_xy) {
    for (auto plot : plot_widgets_gauss)
      plot->setVisible(plot == ui->plot_fstack_xy);
    for (auto plot : plot_widgets_phi)
      plot->setVisible(plot == ui->plot_phi_xy);
    for (auto plot : plot_widgets_rhoa)
      plot->setVisible(plot == ui->plot_rhoa_xy);
  } else if (button == ui->btn_show_yx) {
    for (auto plot : plot_widgets_gauss)
      plot->setVisible(plot == ui->plot_fstack_yx);
    for (auto plot : plot_widgets_phi)
      plot->setVisible(plot == ui->plot_phi_yx);
    for (auto plot : plot_widgets_rhoa)
      plot->setVisible(plot == ui->plot_rhoa_yx);
  } else if (button == ui->btn_show_yy) {
    for (auto plot : plot_widgets_gauss)
      plot->setVisible(plot == ui->plot_fstack_yy);
    for (auto plot : plot_widgets_phi)
      plot->setVisible(plot == ui->plot_phi_yy);
    for (auto plot : plot_widgets_rhoa)
      plot->setVisible(plot == ui->plot_rhoa_yy);
  }
}
...
void spectra_plotter::replot_tab(int index) {
  if (index == 0) {
    ...
    for (auto plot : plot_widgets_time_xy_yx)
      plot->update();
  } else if (index == 4) {
    for (auto plot : plot_widgets_time_xx_yy)
      plot->update();
  }
}
...
void spectra_plotter::on_button_debug_load_data_clicked() {
  ...
  if (this->cmdline != nullptr && cmdline->svalue("survey_basedir") != "empty")
...
void spectra_plotter::on_cb_performance_stateChanged(int val) {
  auto p_shader = val > 0 ? ygfxqt::YGFXQTAssetManager::getShader(ygfxqt::YGFXQTAssetManager::SHADER_PLOT_2D_POINTS_PERF) : ygfxqt::YGFXQTAssetManager::getShader(ygfxqt::YGFXQTAssetManager::SHADER_PLOT_2D_POINTS);
  for (auto plot : plots)
    plot->setCustomShader(p_shader);
  for (auto a : plot_widgets_all)
    for (auto plot : a)
      plot->update();
}
