...
public:
    ...
    QCPSelectorItem(QCustomPlot * parent) : QCPAbstractItem(parent) {}
    GraphWidgetSelector * selector = nullptr;
    double selectTest(const QPointF & pos, bool onlySelectable, QVariant * details) const { Q_UNUSED(pos); Q_UNUSED(onlySelectable); Q_UNUSED(details); return 0; }
    void draw(QCPPainter * painter) {
        if(selector != nullptr && selector->isActive() && is_visible) { selector->paint_selection(*painter); }
    }
};
