...
void PlotWidget::set_graph_data(double freq, const std::vector<double> &keys,
  ...
  // calculating grid-data etc
  {
    ...
    for (size_t index = 0; index < keys.size(); ++index) {
      size_t grid_x = additional_data.x_one_dimensional ? 0 : static_cast<size_t>(((keys[index] - additional_data.selection_grid_min.x()) / additional_data.selection_grid_step.x()));
      size_t grid_y = additional_data.y_one_dimensional ? 0 : static_cast<size_t>(((values[index] - additional_data.selection_grid_min.y()) / additional_data.selection_grid_step.y()));
