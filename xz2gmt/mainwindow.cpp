  ...
  if (this->f_in.open(QFile::ReadOnly)) {
    ...
    do {
      ...
      if (!line.startsWith("#") && (line.size() > 2)) {
        ...
        if (this->xcol < this->line_items.size())
          this->x.append(this->line_items.at(this->xcol).toDouble());
        if (this->yzcol < this->line_items.size())
          this->yz.append(this->line_items.at(this->yzcol).toDouble());
        if (this->valcol < this->line_items.size())
          this->vals.append(this->line_items.at(this->valcol).toDouble());
    ...
    for (auto &v : this->vals) {
      this->vals_orig.append(v);
    }

    for (int j = 0; j < x.size(); ++j)
      this->prfx.append(this->x.at(j));

    if (this->mul_x_inputby != 1)
      for (auto &v : this->x) {
        v *= this->mul_x_inputby;
      }
    if (this->mul_yz_inputby != 1)
      for (auto &v : this->yz) {
        v *= this->mul_yz_inputby;
      }
    ...
    if (this->mul_profile_ycol_inputby != 1)
      for (auto &v : this->prfy) {
        v *= this->mul_profile_ycol_inputby;
      }

    if (this->ui->logx_EW_checkBox->isChecked())
      for (auto &v : this->x) {
        v = log10(v);
      }
    if (this->ui->log_y_NS_checkBox->isChecked())
      for (auto &v : this->yz) {
        v = log10(v);
      }
    // if (this->ui->log_vals_checkBox->isChecked()) for (auto  &v : this->vals) {v = log10(v);}
    if (this->ui->log_y_profile_checkBox->isChecked())
      for (auto &v : this->prfy) {
        v = log10(v);
      }
...
void mainwindow::on_one_by_one_checkBox_clicked(bool checked) {
  ...
  if (this->x.size() && this->yz.size()) {
    ...
    if (checked) {
      ...
      if (this->x_EW_scale > this->yz_NS_scale)
        this->yz_NS_scale = this->x_EW_scale;
      else
        this->x_EW_scale = this->yz_NS_scale;
    }
...
void mainwindow::polyline_bottom_top() {
  if (this->ui->bottomline_checkBox->isChecked() || this->ui->clip_below_bottomline_checkBox->isChecked() || this->ui->topline_checkBox->isChecked() || this->ui->clip_above_topline_checkBox->isChecked()) {
    ...
    for (int i = 0; i < x.size(); ++i) {
      ...
      if (x.at(i) != starts || i == x.size() - 1) {
        if (i)
          this->bx.append(x.at(i - 1));
        if (i)
          this->bx_val.append(*min_element(yvalstmp.begin(), yvalstmp.end()));
        if (i)
          this->tx.append(x.at(i - 1));
        if (i)
          this->tx_val.append(*max_element(yvalstmp.begin(), yvalstmp.end()));
        if (i && !counter) {
...
void mainwindow::on_mul_vals_inputby_doubleSpinBox_valueChanged(double arg1) {
  ...
  if (this->add_vals == 0) {
    if (this->ui->log_vals_checkBox->isChecked())
      for (auto &v : this->vals) {
        v = log10(v);
      }
  }
