    ...
    ui(new Ui::MainWindow)
{
  ...
  this->ui->dateTimeEdit_starts->setTimeSpec(Qt::UTC);
...
void MainWindow::dropEvent(QDropEvent *event)
{
  ...
  if (currentTabName == "tab_ats2xml") {
    ...
    for (int i = 0; i < urls.size(); ++i ) {
      ...
      if (strfile.endsWith(".ats", Qt::CaseInsensitive)) allfiles.append(QFileInfo(strfile));
    }
    if (allfiles.size()) {
  ...
  if (currentTabName == "tab_tsdiff") {
    ...
    if (allfiles.size()) {
      ...
      if (!ovr_samples && (samples < this->std_slice)) this->std_slice = samples;
      else if (ovr_samples < this->std_slice) this->std_slice = ovr_samples;
      ...
      for (size_t m = 0; m < allfiles.size(); ++m) this->in_mutex.emplace_back(new std::mutex() );
      auto dat = data.begin();
  ...
  if (currentTabName == "tab_json") {
    ...
    for (int i = 0; i < urls.size(); ++i ) {
      QString strfile = urls.at(i).toLocalFile();
      if (strfile.contains(".json", Qt::CaseInsensitive)) allfiles.append(QFileInfo(strfile));
    }
    if (allfiles.size()) {
      for (auto &atsf : this->atsfiles) atsf.reset();
      this->atsfiles.clear();
      if (this->edt != nullptr) this->edt.reset();
      if (this->phj_system_ts != nullptr) this->phj_system_ts.reset();
      ...
      for (auto &vbr : this->rb_sensor_names) vbr.clear();
        ...
        if (ok) {
          ...
          if (phj_system_ts->phoenix_tags.contains("Ex_dipole_length_m")) {
            if( phj_system_ts->phoenix_tags.value("Ex_dipole_length_m").toDouble() > 0.1) {
              ...
              if (phj_system_ts->phoenix_tags.contains("system_serial_number")) this->atsfiles.back()->set_key_value("system_serial_number", phj_system_ts->phoenix_tags.value("system_serial_number").toUInt());
  ...
  if (currentTabName == "tab_ascii2ats") {
    ...
    for (auto &a : this->atsfiles) a->close();
    this->atsfiles.clear();
    QString chan_name;
    QString board = "LF";
    int chan_no = -1;
    ...
    if (!ok || freq <= 0.000) freq = this->fcombo->get_frequency();
    ...
    for (auto &qfi : allfiles) {
      ...
      for (int s = 0; s < bnam.size(); ++s) {
        if (bnam.at(s).isDigit()) sfreq.append(bnam.at(s));
        if (sfreq.size() && (bnam.at(s).toLower() == QChar('h'))) sfreq.append(QChar('H'));
        if (sfreq.size() && (bnam.at(s).toLower() == QChar('s'))) sfreq.append(QChar('s'));

        if ((sfreq.size() > 1) && (sfreq.endsWith(QChar('H')) || sfreq.endsWith(QChar('s')) ) ) {
          has_sfreq = true;
          break;
        }
        else if (!bnam.at(s).isDigit()) sfreq.clear();
      }
      ...
      if (has_sfreq) {
        ...
        if (sfreq.endsWith(QChar('H'))) {
          ...
          if (success == -1) this->ui->lineEdit_sample_freq->setText(numstring.setNum(freq));
          else this->ui->lineEdit_sample_freq->setText("0");
        }
        if (sfreq.endsWith(QChar('s'))) {
          sfreq = sfreq.remove(QChar('s'));
          double dfreq = sfreq.toDouble(&isok);
          if (isok) freq = 1.0/dfreq;
          double success = this->fcombo->set_frequency(freq);
          if (success == -1) this->ui->lineEdit_sample_freq->setText(numstring.setNum(freq));
          else this->ui->lineEdit_sample_freq->setText("0");
        }
      ...
      if ( (tmp_name.contains("Bx", Qt::CaseInsensitive)) || (tmp_name.contains("Hx", Qt::CaseInsensitive)) ) {
        chan_name = "Hx"; chan_no = 2;
        if (freq < 513) this->ui->checkBox_chopperHx->setChecked(true);
        else this->ui->checkBox_chopperHx->setChecked(false);

      }
      else if ( (tmp_name.contains("By", Qt::CaseInsensitive)) || (tmp_name.contains("Hy", Qt::CaseInsensitive)) ) {
        chan_name = "Hy"; chan_no = 3;
        if (freq < 513) this->ui->checkBox_chopperHy->setChecked(true);
        else this->ui->checkBox_chopperHy->setChecked(false);

      }


      else if (tmp_name.contains("Ex", Qt::CaseInsensitive) ) {
        chan_name = "Ex"; chan_no = 0;
        if (freq < 513) this->ui->checkBox_chopperEx->setChecked(true);
        else this->ui->checkBox_chopperEx->setChecked(false);


      }

      else if (tmp_name.contains("Ey", Qt::CaseInsensitive) ) {
        chan_name = "Ey"; chan_no = 1;
        if (freq < 513) this->ui->checkBox_chopperEy->setChecked(true);
        else this->ui->checkBox_chopperEy->setChecked(false);


      }

      else if ( (tmp_name.contains("Bz", Qt::CaseInsensitive)) || (tmp_name.contains("Hz", Qt::CaseInsensitive)) ) {
        chan_name = "Hz"; chan_no = 4;
        if (freq < 513) this->ui->checkBox_chopperHz->setChecked(true);
        else this->ui->checkBox_chopperHz->setChecked(false);
  ...
  if (currentTabName == "tab_tscat") {
    ...
    for (const auto &dir : qdirs) {
      ...
      for (const auto &qfi : qfilist) {
        if (qfi.fileName().endsWith("ats", Qt::CaseInsensitive) && qfi.fileName().contains("_TEx", Qt::CaseInsensitive)){
          if (qfi.fileName().contains(this->ats_file_search_str))  this->atsex.emplace_back(std::make_shared<atsfile>(qfi, ii++, short(0), this));
        }
        if (qfi.fileName().endsWith("ats", Qt::CaseInsensitive) && qfi.fileName().contains("_TEy", Qt::CaseInsensitive)){
          if (qfi.fileName().contains(this->ats_file_search_str))  this->atsey.emplace_back(std::make_shared<atsfile>(qfi, ii++, short(0), this));
        }
        if (qfi.fileName().endsWith("ats", Qt::CaseInsensitive) && qfi.fileName().contains("_THx", Qt::CaseInsensitive)){
          if (qfi.fileName().contains(this->ats_file_search_str))  this->atshx.emplace_back(std::make_shared<atsfile>(qfi, ii++, short(0), this));
        }
        if (qfi.fileName().endsWith("ats", Qt::CaseInsensitive) && qfi.fileName().contains("_THy", Qt::CaseInsensitive)){
          if (qfi.fileName().contains(this->ats_file_search_str))  this->atshy.emplace_back(std::make_shared<atsfile>(qfi, ii++, short(0), this));
        }
        if (qfi.fileName().endsWith("ats", Qt::CaseInsensitive) && qfi.fileName().contains("_THz", Qt::CaseInsensitive)){
          if (qfi.fileName().contains(this->ats_file_search_str))  this->atshz.emplace_back(std::make_shared<atsfile>(qfi, ii++, short(0), this));
        }
    ...
    if (atsex.size()) atsh = std::make_shared<atsheader> (this->atsex.begin()->get()->get_qfile_info());
    else if (atsey.size()) atsh = std::make_shared<atsheader> (this->atsey.begin()->get()->get_qfile_info());
    else if (atshx.size()) atsh = std::make_shared<atsheader> (this->atshx.begin()->get()->get_qfile_info());
    else if (atshy.size()) atsh = std::make_shared<atsheader> (this->atshy.begin()->get()->get_qfile_info());
    else if (atshz.size()) atsh = std::make_shared<atsheader> (this->atshz.begin()->get()->get_qfile_info());
...
void MainWindow::clear(const bool clear_mxcds)
{
  this->ui->ats2xml_listWidget->clear();
  if (clear_mxcds) {
    for (auto &mxcd : this->mxcds) if (mxcd != nullptr) mxcd.reset();
    this->mxcds.clear();
  }
  if (this->mxcd != nullptr) this->mxcd.reset();
  if (this->adujob != nullptr) this->adujob.reset();
  ...
  this->tsdiffs.clear();
  for (auto &thrd : this->readerthreads) thrd.join();
  for (auto &thrd : this->writerthreads) thrd.join();
  for (auto &thrd : this->fetcherthreads) thrd.join();
  ...
  for (auto &mtx : this->in_mutex) mtx->unlock();
  for (auto &mtx : this->in_mutex) delete mtx;;
  this->in_mutex.clear();
...
void MainWindow::close_tscat_channels(bool iscancelled)
{
  ...
  if (!iscancelled) {
    if (this->atsex_cat != nullptr) this->atsex_cat->close(true);
    if (this->atsey_cat != nullptr) this->atsey_cat->close(true);
    if (this->atshx_cat != nullptr) this->atshx_cat->close(true);
    if (this->atshy_cat != nullptr) this->atshy_cat->close(true);
    if (this->atshz_cat != nullptr) this->atshz_cat->close(true);
  }

  if (this->atsex_cat != nullptr) this->atsex_cat.reset();
  if (this->atsey_cat != nullptr) this->atsey_cat.reset();
  if (this->atshx_cat != nullptr) this->atshx_cat.reset();
  if (this->atshy_cat != nullptr) this->atshy_cat.reset();
  if (this->atshz_cat != nullptr) this->atshz_cat.reset();
...
{
  for (auto &ats : this->atsex) ats->scan_header_close();
  for (auto &ats : this->atsey) ats->scan_header_close();
  for (auto &ats : this->atshx) ats->scan_header_close();
  for (auto &ats : this->atshy) ats->scan_header_close();
  for (auto &ats : this->atshz) ats->scan_header_close();
...
void MainWindow::display_start_stop_tscat()
{
  ...
  else this->tscat_continue = false;
...
void MainWindow::check_for_overlap_tscat()
{
  if (this->atsex.size()) {
    for (int i = 0; i < this->atsex.size() -1 ; ++i) {
      if (delta_stop_start_sp(atsex.at(i), atsex.at(i+1)) < 0) this->tscat_continue = false;
...
{
  if (!this->tscat_continue) return;

  if (atsex.size()) atsex_cat = std::make_shared<atsfile>(*atsex.at(0));
  if (atsey.size()) atsey_cat = std::make_shared<atsfile>(*atsey.at(0));
  if (atshx.size()) atshx_cat = std::make_shared<atsfile>(*atshx.at(0));
  if (atshy.size()) atshy_cat = std::make_shared<atsfile>(*atshy.at(0));
  if (atshz.size()) atshz_cat = std::make_shared<atsfile>(*atshz.at(0));

  if (atsex_cat != nullptr) {
    int rmax = 900, i = 0;
    do {
      QFileInfo qfir(atsex_cat->absoluteFilePath());
      if (qfir.exists()) atsex_cat->set_run(-1);
      else break;
    } while (i < rmax);
  }
  ...
  if (atsey_cat != nullptr) {
    ...
    do {
      ...
      if (qfir.exists()) atsey_cat->set_run(-1);
      else break;
    } while (i < rmax);
  }
  if (atshx_cat != nullptr) {
    ...
    do {
      ...
      if (qfir.exists()) atshx_cat->set_run(-1);
      else break;
    } while (i < rmax);
  }
  if (atshy_cat != nullptr) {
    ...
    do {
      ...
      if (qfir.exists()) atshy_cat->set_run(-1);
      else break;
    } while (i < rmax);
  }
  if (atshz_cat != nullptr) {
    ...
    do {
      ...
      if (qfir.exists()) atshz_cat->set_run(-1);
      else break;
    } while (i < rmax);
  }
  ...
  if (atsex_cat->ivalue("run_number") != atsey_cat->ivalue("run_number"))  this->tscat_continue = false;
  if (atsex_cat->ivalue("run_number") != atshx_cat->ivalue("run_number"))  this->tscat_continue = false;
  if (atsex_cat->ivalue("run_number") != atshy_cat->ivalue("run_number"))  this->tscat_continue = false;
  if (atsex_cat->ivalue("run_number") != atshz_cat->ivalue("run_number"))  this->tscat_continue = false;

  if (!this->tscat_continue) this->close_tscat_channels(true);
...
void MainWindow::tsdata_subtraction(const size_t &slot)
{
  ...
  if (can_run == this->in_mutex.size()) {
    qDebug() << "runs";
    for (auto &v : this->fetchcounts) std::cerr << v << " ";
    size_t i = 0, j = 0, k = 0;
    ...
    if ((this->buffer_status[0] == threadbuffer_status::finished) || (this->buffer_status[0] == threadbuffer_status::terminated)) {
      ...
      this->tsdiffs.clear();
      this->data.clear();
      for (auto &buf : this->tsbuffers) buf.reset();
      for (auto &buf : this->tsdiffbuffers) buf.reset();
...
{
  if (!this->qfi_base.absoluteFilePath().size()) return;
  if (!this->qfi_source.absoluteFilePath().size()) return;
      ...
      for (auto &fnam : allfiles) {
        ...
        for (auto &btn : this->chbtns) {
          if (btn->isChecked()) {
            ...
            if (prp == map.value("channel").toInt() ) files.append(fnam);
          }
        }
      }
      if (files.size()) {
      ...
      if (qfi.exists()) {
        ...
        for (auto &fnam : allfiles) {
          ...
          for (auto &btn : this->chbtns) {
            if (btn->isChecked()) {
              int prp = btn->property(prop_chan).toInt();
              if (prp == map.value("channel").toInt() ) files.append(QFileInfo(qfi.absolutePath() + "/" + fnam));
            }
          }
        }
...
{
  if (!this->atsfiles.size()) return;
  if (this->atsfiles.size() != this->inats2ascii_files.size()) return;

  if (this->qfi_base.fileName().size()) {

  }

  geocoordinates geocord; int lat, lon, elev;
  ...
  for (auto &ats : this->atsfiles) {
    ...
    if (ats->fileName().contains("TEx")) {
      ...
      if (this->ui->checkBox_chopperEx->isChecked()) ats->insert("chopper_status", quint8(1));
      else ats->insert("chopper_status", quint8(0));
    ...
    if (ats->fileName().contains("TEy")) {
      ...
      if (this->ui->checkBox_chopperEy->isChecked()) ats->insert("chopper_status", quint8(1));
      else ats->insert("chopper_status", quint8(0));
    ...
    if (ats->fileName().contains("THx")) {
      ...
      if (this->ui->checkBox_chopperHx->isChecked()) ats->insert("chopper_status", quint8(1));
      else ats->insert("chopper_status", quint8(0));
    ...
    if (ats->fileName().contains("THy")) {
      ...
      if (this->ui->checkBox_chopperHy->isChecked()) ats->insert("chopper_status", quint8(1));
      else ats->insert("chopper_status", quint8(0));
    ...
    if (ats->fileName().contains("THz")) {
      ...
      if (this->ui->checkBox_chopperHz->isChecked()) ats->insert("chopper_status", quint8(1));
      else ats->insert("chopper_status", quint8(0));
  ...
  bool is_Volt = false;
  bool is_64bit = false;
  if (this->ui->radioButton_V->isDown()) is_Volt = true;
  if (this->ui->checkBox_force_64bit->isChecked()) is_64bit = true;
  ...
  QList<QFileInfo> allfiles;
  for (auto ats : this->atsfiles)  allfiles.append(QFileInfo(ats->absoluteFilePath()));
  for (auto ats : this->atsfiles) ats.reset();
  this->mxcds.emplace_back(std::make_unique<mc_data>());
  this->mxcds.back()->open_files(allfiles, true, mttype::nomt, threadctrl::threadctrl_continuous_individual);

  for (int i = 0; i < 5; ++i) this->chbtns[i]->setChecked(true);
  for (auto &atsf : this->mxcds.back()->in_atsfiles) {
    this->ui->ats2xml_listWidget->addItem(atsf->fileName());
  }
  this->ui->pushButton_create_xml->setDisabled(false);
...
void MainWindow::on_fcombo_tscat_template_currentIndexChanged(int index)
{
  ...
  if (this->ats_file_search_str.endsWith("z", Qt::CaseInsensitive)) this->ats_file_search_str.chop(1);
...
{
  QString homeLocation = QStandardPaths::locate(QStandardPaths::HomeLocation, QString(), QStandardPaths::LocateDirectory);
  auto dialog = new QFileDialog(this, "Select target dir", homeLocation);  dialog->setViewMode(QFileDialog::Detail);
  dialog->setFileMode(QFileDialog::Directory);
  dialog->setOption(QFileDialog::DontResolveSymlinks, true);
  dialog->setOption(QFileDialog::ShowDirsOnly, true);
