...
void calib_lab::run(const prc_com &cmdline_parm, const QList<QFileInfo> &qfis) {
  ...
  *this->cmdline = cmdline_parm;
  ...
  if (!this->cmdline->contains("fcut_upper"))
    this->cmdline->insert_double("fcut_upper", 0.15);
  if (!this->cmdline->contains("fcut_lower"))
    this->cmdline->insert_double("fcut_lower", 0.05);
  if (!this->cmdline->contains("nbuffers"))
    this->cmdline->insert_szt("nbuffers", 8);
  if (!this->cmdline->contains("parzen_radius"))
    this->cmdline->insert_double("parzen_radius", 0.1);
  if (!this->cmdline->contains("force_frequency_list"))
    this->cmdline->insert_int("force_frequency_list", 1); // default MT freqs

  if (!this->cmdline->contains("nbuffers"))
    this->cmdline->insert_szt("nbuffers", 8);

  if (!this->cmdline->contains("min_lines"))
  ...
  if (this->cmdline->svalue("calib_lab_run") == "parallel_test") {
    ...
    if (!this->cmdline->contains("wl") && qfis.size()) {
      if (qfis.at(0).exists() && qfis.at(0).fileName().endsWith("ats", Qt::CaseInsensitive)) {
        ...
        if (this->f_sample < 63.99)
          this->wl = 64;
        else
          this->wl = size_t(this->f_sample);
  ...
  std::cout << std::endl
            << "cut " << this->cmdline->dvalue("fcut_lower") << " " << this->cmdline->dvalue("fcut_lower") << std::endl;
...
void calib_lab::process() {
  ...
  for (int ii = 0; ii < main_loops; ++ii) {
    ...
    // from the Insitu
      ...
      mxcd->slot_set_target_frequencies(tf);
      if (tf.at(0) < 1.0)
        this->cmdline->set_key_value("auto_bandwidth", double(tf.at(0)));
    }
