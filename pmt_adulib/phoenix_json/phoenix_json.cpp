...
bool Phoenix_json::ts_header_reader(const QFileInfo &qfi, int &nerrors) {
  ...
  do {
    ...
    if (line.startsWith("\"start_time"))
      start_time = line.simplified();
    if (line.startsWith("\"stop_time"))
      stop_time = line.simplified();

    if (!line.startsWith("\"data\": ["))
      json_bytes.append(line.toUtf8());
    if (i > 79) {
      textfile.close();
  ...
  if (json_bytes.endsWith(","))
    json_bytes.remove(json_bytes.size() - 1, 1);
  json_bytes.append("}");
  ...
  if (ok)
    this->phoenix_tags.insert("start_date_time", t);
  else
    ++this->nerrors;

  t = stop_time.toLongLong(&ok);
  if (ok)
    this->phoenix_tags.insert("stop_date_time", t);
  else
    ++this->nerrors;
  ...
  if (this->phoenix_tags.contains("sample_freq")) {
    if (this->phoenix_tags.value("sample_freq").toInt() == 0)
      return false;
    ...
    if (!first.value().isObject()) {
      ...
      if (what == 2) { // coil
        if (first.key() == "freq_Hz") {
          ...
          this->special_values_coils(first);
      }
      if (what == 3) { // system
      ...
      for (int i = 0; i < jarr.size(); ++i) {
        ...
        if (val.isObject()) {
          ...
          if (what == 2) {
            ...
            if (first.key() == "freq_Hz")
              this->freqs_coil.back().push_back(val.toDouble(-1));
            if (first.key() == "magnitude")
              this->magnitude_coil.back().push_back(val.toDouble(-1));
            if (first.key() == "phs_deg")
              this->phs_deg_coil.back().push_back(val.toDouble(-1));
          }
          if (what == 3) {
            if (first.key() == "freq_Hz")
              this->freqs_system.back().back().push_back(val.toDouble(-1));
            if (first.key() == "magnitude")
              this->magnitude_system.back().back().push_back(val.toDouble(-1));
            if (first.key() == "phs_deg")
              this->phs_deg_system.back().back().push_back(val.toDouble(-1));
          }
        }
      }
    }
...
void Phoenix_json::special_values_coils(QJsonObject::const_iterator &first) {
  ...
  if (!first.key().compare("chan_data", Qt::CaseInsensitive)) {
    ...
    if (myok)
      this->phoenix_tags.insert("sensor_serial", QVariant(ser));
    else
      this->phoenix_tags.insert("sensor_serial", 1);
...
void Phoenix_json::special_values_system_ts(QJsonObject::const_iterator &first) {
  ...
  if (!first.key().compare("E1", Qt::CaseInsensitive)) {
    ...
    if (try_serial.size() > 1) {
      ...
      if (ok)
        this->phoenix_tags.insert("system_serial_number", QVariant(ser));
      else
        this->phoenix_tags.insert("system_serial_number", 1);
      if (!ok)
        ++nerrors;
    } else
      this->phoenix_tags.insert("system_serial_number", 1);
