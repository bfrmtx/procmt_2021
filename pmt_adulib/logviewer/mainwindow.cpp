...
void MainWindow::dropEvent(QDropEvent *event)
{
    ...
    if (!this->all_datetimes.size()) return;
    this->min_max_time();
...
{
    if (this->customPlot == nullptr) return;
    if (!this->num_sats_sats.size()) return;
    if (this->num_sats_sats.size() != this->num_sats_datetimes.size()) return;
...
{
    if (this->customPlot == nullptr) return;
    if (!this->temperatures.size()) return;
    if (this->temperatures.size() != this->temperature_datetimes.size()) return;
...
{
    if (this->customPlot == nullptr) return;
    if (!this->batt_volts.size()) return;
    if (this->batt_volts.size() != this->batt_volt_datetimes.size()) return;
    this->customPlot->addGraph();
...
{
    if (this->customPlot == nullptr) return;
    if (!this->batt1_currs.size()) return;
    if (this->batt1_currs.size() != this->batt1_curr_datetimes.size()) return;
    this->customPlot->addGraph();
...
{
    if (this->customPlot == nullptr) return;
    if (!this->batt2_currs.size()) return;
    if (this->batt2_currs.size() != this->batt2_curr_datetimes.size()) return;
    this->customPlot->addGraph();
...
{
    if (query.record().count() != 7) return;
    QDateTime timestamp;
    ...
    while(this->xmlreader.readNextStartElement()){
        ...
        if (!found) xmlreader.skipCurrentElement();
...
{
    if(!this->all_datetimes.size()) return;
    this->mintime.setTimeSpec(Qt::UTC);
    this->maxtime.setTimeSpec(Qt::UTC);
